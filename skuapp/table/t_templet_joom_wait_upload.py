...
class t_templet_joom_wait_upload(models.Model):
    ...
    B_cost_weight       =   models.TextField(u'普源成本价和克重', blank=True, null=True)
    Source              =   models.CharField(u'来源', max_length=10, blank=True, null=True) # 采集箱记录来源，wish或collect（采集）
    Used_shopname       =   models.CharField(u'已铺货店铺', max_length=255, blank=True, null=True)
    Cate                =   models.CharField(u'大类', max_length=20, blank=True, null=True)
    ExportTime          =   models.DateTimeField(u'导出时间', blank=True, null=True)


    class Meta:
        verbose_name = u'JOOM待铺货'
        verbose_name_plural = verbose_name
        db_table = 't_templet_joom_wait_upload'
        ordering = ['-id']
        app_label = 'skuapp'
