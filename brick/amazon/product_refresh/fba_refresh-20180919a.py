...
"""  
 ...
 @site: 
 @software: PyCharm
 @file: fba_refresh-20180919a.py
 @time: 2018/9/19 17:16
"""
...
def delete_history_log(file_pattern, file_time_diff, scan_weekday=None):
    """
    店铺数据刷新日志按天保存，防止日志文件过多，清理历史日志文件
    :param file_pattern:  文件类型，文件名中包含的关键字，如 .log
    :param file_time_diff:  文件修改时间与当前时间差大于该值时删除文件
    :param scan_weekday:  执行日期
    """
    try:
        if scan_weekday is not None and datetime.datetime.now().weekday() != scan_weekday:
            logging.debug('Not scan time')
            return

        file_url = os.getcwd()
        pattern_file = [f for f in os.listdir(file_url) if file_pattern in f]
        for file_idx in range(len(pattern_file)):
            ...
            if time_diff_days >= file_time_diff:
                ...
                logging.debug("delete this file：%s ： %s" % (file_time, pattern_file[file_idx]))
...
def refresh_db_tables(auth_info, sql_execute_obj):
    ...
    sql_execute_obj.execute_db(sql_product_status)
...
class FinancesPublic:
    ...
    def get_last_refund_time(self):
        ...
        print 'last_order_time is : %s' % str(last_refund_time)
        logging.debug('last_order_time is : %s' % str(last_refund_time))
