...
"""  
 ...
 @site: 
 @software: PyCharm
 @file: fba_refresh-20181204.py
 @time: 2018/12/4 13:51
"""
...
def refresh_db_tables(auth_info, sql_execute_obj):
    # 库存管理表单表
    sql_ad_delete = "delete from t_amazon_cpc_ad where shop_name = '%s' and shop_site = '%s'" \
    ...
    sql_ad_insert = '''insert into t_amazon_cpc_ad(shop_name,shop_site,seller_sku,title,asin,image_url,price,quantity,
                                                      create_date,STATUS,Parent_asin,product_id_type) 
                                  select ShopName,shopsite,seller_sku,item_name,asin1,image_url,price,quantity,open_date,
                                             STATUS,Parent_asin,product_id_type 
                                  from t_online_info_amazon 
                                  where ShopName = '%s' 
                                  and ShopSite = '%s' 
                                  and refresh_status = 0;''' % (auth_info['ShopName'], auth_info['ShopSite'])
    ...
    # 部分店铺SKU绑定错误重新绑定的情况，如shop_A+shop_B+shop_C 对应 product_A+product_B+product_C
    # 后发现绑定错误，人工重新绑定为：shop_A+shop_B+shop_C <->product_D
    # 此时以shop_A+shop_B+shop_C <->product_D 为准
    ...
    sql_estimated_fba_fees = '''UPDATE t_online_info_amazon a, t_amazon_estimated_fba_fees b
                   SET a.estimated_fee = b.estimated_fee,
                          ...
                          a.package_weight = b.item_package_weight
                 WHERE a.shopname = b.shop_name
                   AND a.seller_sku = b.sku
                   and a.shopname = '%s'
              ''' % auth_info['ShopName']
    ...
    # 已关闭店铺
    sql_inactive_shop = '''update t_online_info_amazon set shop_status = 1 
                                           where shopname = '%s' 
                                           and shopname in 
                                           (select shopname from t_config_shop_alias where shopstatus = 5)
                                        ''' % auth_info['ShopName']
    ...
    sql_upload_time2 = '''UPDATE t_online_info_amazon a,
        ...
        and  a.shopname = d.shopsets
        AND a.shopname = '%s'
        and a.upload_time is null
        ''' % auth_info['ShopName']
                                           ...
                                           (select f.shopname,
                                              ...
                                              from (select aa.*,
                                                      ...
                                                      left join py_db.b_goods bb
                                                        on trim(aa.product_sku) = bb.sku) f
    ...
    sql_com_pro_relation = '''update t_online_info_amazon a, t_combination_sku_log b
        ...
        where substring_index(a.sku, '*', 1) = b.com_sku
        and a.shopname = '%s'
        ''' % auth_info['ShopName']

    # FBA清仓合并商品，商品sku指向合并后的SKU
    sql_merge_pro_relation = '''update t_online_info_amazon h,
                                                   (SELECT shop_name,
                                                           seller_sku,
                                                           new_sku,
                                                           GROUP_CONCAT(sku_each ORDER BY help_topic_id SEPARATOR '+') product_sku
                                                      from (SELECT shop_name,
                                                              ...
                                                              from (select shop_name,
                                                                           seller_sku,
                                                                           new_sku,
                                                                           if(new_sku is not null, new_sku, product_sku) product_sku,
                                                                           multiply,
                                                                           help_topic_id
                                                                      from (select shop_name,
                                                                                   seller_sku,
                                                                                   substring_index(product_sku_each, '*', 1) product_sku,
                                                                                   CASE
                                                                                     WHEN instr(product_sku_each, '*') > 0 THEN
                                                                                      substring_index(product_sku_each,
                                                                                                      '*',
                                                                                                      -1)
                                                                                     ELSE
                                                                                      1
                                                                                   END multiply,
                                                                                   help_topic_id
                                                                              from (SELECT m.shop_name,
                                                                                           seller_sku,
                                                                                           product_sku,
                                                                                           SUBSTRING_INDEX(SUBSTRING_INDEX(product_sku,
                                                                                                                           '+',
                                                                                                                           help_topic_id),
                                                                                                           '+',
                                                                                                           -1) product_sku_each,
                                                                                      ...
                                                                                      FROM (SELECT shopname shop_name,
                                                                                                   seller_sku,
                                                                                                   IF(com_pro_sku IS NULL,
                                                                                                      sku,
                                                                                                      com_pro_sku) product_sku
                                                                                              FROM t_online_info_amazon a
                                                                                             WHERE shopname =
                                                                                                   '%s'
                                                                                               and is_fba = 1) m
                                                                                      JOIN t_amazon_help_topic n
                                                                                        ON n.help_topic_id <=
                                                                                           (length(m.product_sku) -
                                                                                           length(REPLACE(m.product_sku,
                                                                                                           '+',
                                                                                                           '')) + 1)
                                                                                       and n.help_topic_id > 0) f) aa
                                                                      left join t_amazon_sku_merge bb
                                                                        on aa.product_sku = bb.old_sku) cc) dd
                                                     group by shop_name, seller_sku
                                                    having new_sku is not null) l
                                               set h.merge_pro_sku = l.product_sku
                                             where h.shopname = l.shop_name
                                               and h.seller_sku = l.seller_sku
                                               and h.shopname = '%s'
                                               and h.is_fba = 1
                                               ''' % (auth_info['ShopName'], auth_info['ShopName'])

    # FBA库龄销售员
    sql_inventory_seller = '''update t_amazon_fba_inventory_age a, t_store_configuration_file b
           set a.seller = b.seller
         WHERE a.shop_name = b.ShopName
           AND a.shop_name = '%s'
        ''' % auth_info['ShopName']
    ...
    print "sql_merge_pro_relation is : %s" % sql_merge_pro_relation
    logging.debug("sql_merge_pro_relation is : %s" % sql_merge_pro_relation)
    sql_execute_obj.execute_db(sql_merge_pro_relation)

    print "sql_inventory_seller is : %s" % sql_inventory_seller
    logging.debug("sql_inventory_seller is : %s" % sql_inventory_seller)
    sql_execute_obj.execute_db(sql_inventory_seller)
...
class ReportPublic:
    ...
    def get_report_result(self, generated_report_id):
        ...
        if self.auth_info['ShopSite'] == 'AU':  # AU站chardet识别不准
    ...
    def insert_public(self, report_type, get_report_data):
        ...
        elif report_type == '_GET_FBA_INVENTORY_AGED_DATA_':
            self.fba_inventory_age_insert(get_report_data)
        elif report_type == '_GET_RESTOCK_INVENTORY_RECOMMENDATIONS_REPORT_':
            self.restock_recommendation_insert(get_report_data)
    ...
    def order_insert(self, get_report_data):
        ...
        for idx, value in enumerate(get_report_data):
            if idx == 0:
                continue
            report_data = value.split('\t')[:29]
            if report_data[6] == 'Non-Amazon' or ('US' if report_data[6].split('.')[-1].upper() == 'COM' else report_data[6].split('.')[-1].upper()) == self.auth_info['ShopSite']:
    ...
    def fba_inventory_age_insert(self, get_report_data):
        col_placeholder = '''snapshot_date, sku, fnsku, asin, 
          product_name, condition_, avaliable_quantity_sellable, qty_with_removals_in_progress, inv_age_0_to_90_days, 
          inv_age_91_to_180_days, inv_age_181_to_270_days, inv_age_271_to_365_days, inv_age_365_plus_days, 
          currency, qty_to_be_charged_ltsf_6_mo, projected_ltsf_6_mo, qty_to_be_charged_ltsf_12_mo, 
          projected_ltsf_12_mo, units_shipped_last_7_days, units_shipped_last_30_days, units_shipped_last_60_days, 
          units_shipped_last_90_days, alert, your_price, sales_price, lowest_price_new, lowest_price_used, 
          recommended_action, healthy_inventory_level, recommended_sales_price, 
          recommended_sale_duration_days, recommended_removal_quantity, estimated_cost_savings_of_removal, 
          sell_through, cubic_feet, storage_type,shop_name'''

        if self.auth_info['ShopSite'] in ('UK', 'FR', 'DE', 'IN', 'IT', 'ES'):
            col_placeholder = '''snapshot_date, marketplace, sku, fnsku, asin, product_name, condition_, 
            avaliable_quantity_sellable, qty_with_removals_in_progress, inv_age_0_to_90_days, inv_age_91_to_180_days, 
            inv_age_181_to_270_days, inv_age_271_to_365_days, inv_age_365_plus_days, currency, 
            qty_to_be_charged_ltsf_6_mo, projected_ltsf_6_mo, qty_to_be_charged_ltsf_12_mo, projected_ltsf_12_mo, 
            units_shipped_last_7_days, units_shipped_last_30_days, units_shipped_last_60_days, 
            units_shipped_last_90_days, alert, your_price, sales_price, lowest_price_new, lowest_price_used, 
            recommended_action, healthy_inventory_level, recommended_sales_price, recommended_sale_duration_days,
             recommended_removal_quantity, estimated_cost_savings_of_removal, shop_name'''
        elif self.auth_info['ShopSite'] == 'IN':
            col_placeholder = '''snapshot_date, sku, fnsku, asin, product_name, condition_, avaliable_quantity_sellable, 
            qty_with_removals_in_progress, inv_age_0_to_90_days, inv_age_91_to_180_days, inv_age_181_to_270_days, 
            inv_age_271_to_365_days, inv_age_365_plus_days, currency, qty_to_be_charged_ltsf_6_mo, 
            projected_ltsf_6_mo, qty_to_be_charged_ltsf_12_mo, projected_ltsf_12_mo, units_shipped_last_7_days, 
            units_shipped_last_30_days, units_shipped_last_60_days, units_shipped_last_90_days, alert, your_price, 
            sales_price, lowest_price_new, lowest_price_used, recommended_action, shop_name'''
        elif self.auth_info['ShopSite'] == 'JP':
            col_placeholder = '''snapshot_date, sku, fnsku, asin, product_name, condition_, avaliable_quantity_sellable, 
            qty_with_removals_in_progress, inv_age_0_to_90_days, inv_age_91_to_180_days, inv_age_181_to_270_days, 
            inv_age_271_to_365_days, inv_age_365_plus_days, currency, qty_to_be_charged_ltsf_6_mo, 
            projected_ltsf_6_mo, qty_to_be_charged_ltsf_12_mo, projected_ltsf_12_mo, units_shipped_last_7_days, 
            units_shipped_last_30_days, units_shipped_last_60_days, units_shipped_last_90_days, alert, your_price, 
            sales_price, lowest_price_new, lowest_price_used, recommended_action, healthy_inventory_level, 
            recommended_sales_price, recommended_sale_duration_days, recommended_removal_quantity, 
            estimated_cost_savings_of_removal, shop_name'''
        value_cnt = len(col_placeholder.split(','))
        val_placeholder = ('%s, ' * value_cnt).strip()[:-1]
        sql_inventory_age_insert = '''insert into t_amazon_fba_inventory_age (%s) values (%s)''' \
                                   % (col_placeholder, val_placeholder)
        sql_inventory_age_delete = '''delete from t_amazon_fba_inventory_age where shop_name ='%s' ''' \
                                   % self.auth_info['ShopName']

        inventory_age_record_list = list()
        for idx, value in enumerate(get_report_data):
            if idx == 0:
                continue
            ...
            report_data.append(self.auth_info['ShopName'])
            if self.auth_info['ShopSite'] in ('UK', 'FR', 'DE', 'IN', 'IT', 'ES'):
                if report_data[1].split('.')[-1].upper() == self.auth_info['ShopSite']:
                    inventory_age_record_list.append(report_data)
            else:
                inventory_age_record_list.append(report_data)

        with self.db_conn.cursor() as cursor:
            logging.debug('There are %d inventory age records, begin time is %s'
                          % (len(inventory_age_record_list), datetime.datetime.now()))
            cursor.execute(sql_inventory_age_delete)
            cursor.executemany(sql_inventory_age_insert, inventory_age_record_list)
            logging.debug('There are %d inventory age records, end time is %s'
                          % (len(inventory_age_record_list), datetime.datetime.now()))
            self.db_conn.commit()

    def restock_recommendation_insert(self,get_report_data):
        col_placeholder = '''product_description, sku, asin, condition_, supplier, price, sales_last_30_days_sales, 
            sales_last_30_days_units, total_inventory, inbound_inventory, available_inventory, reserved_fc_transfer, 
            reserved_fc_processing, reserved_customer_order, unfulfillable, fulfilled_by, days_of_supply, instock_alert, 
            recommended_order_quantity, recommended_order_date, shop_name'''
        if self.auth_info['ShopSite'] in ('UK', 'FR', 'DE', 'IN', 'IT', 'ES'):
            col_placeholder = '''country, product_description, fnsku, sku, asin, condition_, supplier, supplier_part_no, 
            currency, price, sales_last_30_days_sales, sales_last_30_days_units, total_inventory, inbound_inventory, 
            available_inventory, reserved_fc_transfer, reserved_fc_processing, reserved_customer_order, unfulfillable, 
            fulfilled_by, days_of_supply, instock_alert, recommended_order_quantity, recommended_order_date, 
            shop_name'''
        elif self.auth_info['ShopSite'] == 'IN':
            col_placeholder = '''product_description, sku, ASIN, Condition_, Supplier, recommended_fulfilment_center, 
            sales_last_30_days_sales, sales_last_30_days_units, days_of_supply, available_inventory, instock_alert, 
            recommended_order_quantity, recommended_order_date, ship_to_fc_date, shop_name'''

        value_cnt = len(col_placeholder.split(','))
        val_placeholder = ('%s, ' * value_cnt).strip()[:-1]
        sql_recommendation_insert = '''insert into t_amazon_restock_inventory_recommendation 
                                                                (%s) values (%s)''' % (col_placeholder, val_placeholder)
        sql_recommendation_delete = '''delete from t_amazon_restock_inventory_recommendation 
                                                           where shop_name ='%s' ''' % self.auth_info['ShopName']

        recommendation_record_list = list()
        for idx, value in enumerate(get_report_data):
            if idx == 0:
                continue
            ...
            report_data.append(self.auth_info['ShopName'])
            if self.auth_info['ShopSite'] in ('UK', 'FR', 'DE', 'IN', 'IT', 'ES'):
                if report_data[0].upper() == self.auth_info['ShopSite']:
                    recommendation_record_list.append(report_data)
            else:
                recommendation_record_list.append(report_data)

        with self.db_conn.cursor() as cursor:
            logging.debug('There are %d recommendation records, begin time is %s'
                          % (len(recommendation_record_list), datetime.datetime.now()))
            cursor.execute(sql_recommendation_delete)
            cursor.executemany(sql_recommendation_insert, recommendation_record_list)
            logging.debug('There are %d recommendation records, end time is %s'
                          % (len(recommendation_record_list), datetime.datetime.now()))
            self.db_conn.commit()
    ...
    def update_really_table(self):
        sql_relation = '''update %s a, %s b
           set a.image_url               = b.image_url,
               ...
               a.lg_flag = b.lg_flag,
               a.RiskGrade = b.RiskGrade,
               a.tortflag = b.tortflag
        ...
        insert_column = '''item_name,item_description,listing_id,seller_sku,price,quantity,open_date,image_url,
        item_is_marketplace,product_id_type,zshop_shipping_fee,item_note,item_condition,zshop_category1,
        zshop_browse_path,zshop_storefront_feature,asin1,asin2,asin3,will_ship_internationally,expedited_shipping,
        zshop_boldface,product_id,bid_for_featured_placement,add_delete,pending_quantity,fulfillment_channel,
        merchant_shipping_group,ShopName,SKU,UpdateTime,order7days,orderydays,ordertdays,ordercdays,allorder,
        order3days,Status,deal_action,deal_result,deal_result_info,product_type,ShopSite,Parent_asin,bullet_point1,
        bullet_point2,bullet_point3,bullet_point4,bullet_point5,product_description,sale_price,sale_from_date,
        sale_end_date,generic_keywords1,generic_keywords2,generic_keywords3,generic_keywords4,
        inventory_received_date,shipping_price,estimated_fee,action_remark,lg_flag,RiskGrade,tortflag
        '''

        sql_insert = "insert into t_online_info_amazon (%s) select %s  from t_online_info_amazon_for_update  where shopname = '%s'" \
                     % (insert_column, insert_column, self.auth_info['ShopName'])
...
class FinancesPublic:
    ...
    def parse_report(self, finance_report_each, report_type, file_name):
        ...
        # 根据不同报告类型分析item信息
        if marketplace_name == 'Non-Amazon' or ('US' if marketplace_name.split('.')[-1].upper() == 'COM' else marketplace_name.split('.')[-1].upper()) == self.auth_info['ShopSite']:
...
if __name__ == '__main__':
    ...
    for key, val in auth_info_all.items():
        ...
        try:
            ...
            # FBA库龄
            get_data_public_obj.report_flow('_GET_FBA_INVENTORY_AGED_DATA_')

            # 订购建议
            get_data_public_obj.report_flow('_GET_RESTOCK_INVENTORY_RECOMMENDATIONS_REPORT_')
            ...
            # 交易汇总记录
            finace_group_obj = FinancesGroupPublic(auth_info, DATABASE)
            finace_group_obj.finance_flow(begin_time='2017-01-01')
            finace_group_obj.close_db_conn()
