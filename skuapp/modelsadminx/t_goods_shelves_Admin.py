...
class t_goods_shelves_Admin(object):
    ...
    def show_tort_status(self,obj):
        count = t_tort_aliexpress.objects.filter(Site='Wish',MainSKU=obj.MainSKU).count()
        if count >= 1:
    ...
    actions = ['do_tort_shelve_wish','to_change_status']
    ...
    def do_tort_shelve_wish(self,request,objs):
        ...
        for obj in objs:
            ...
            product_and_bottom_shelf_func.delay([record_id,],user_name,'disshopsku')

        messages.success(request, u'数据已经提交，请稍等。。正在处理。。。')
    do_tort_shelve_wish.short_description = u'一键下架'

    def to_change_status(self,request,objs):
        list_id_ll = []
        for obj in objs:
            if obj.OperationState == 'Error':
                list_id_ll.append(obj.id)
                
        objs.filter(id__in = list_id_ll).update(Status = 'Disabled')
    to_change_status.short_description = u'修改状态'
    
    def get_list_queryset(self):
        request = self.request
        qs = super(t_goods_shelves_Admin, self).get_list_queryset().filter(GoodsStatus__in=['2', '3', '4'],Status='Enabled')
