# -*- coding: utf-8 -*-
from skuapp.modelsadminx.b_goods_sales_count_Admin import b_goods_sales_count_Admin
from skuapp.modelsadminx.t_cfg_b_currencycodeAdmin import *
from skuapp.modelsadminx.t_cfg_b_currencycode_logAdmin import *
from skuapp.modelsadminx.wish_processed_order_Admin import wish_processed_order_Admin
from skuapp.modelsadminx.wish_notification_Admin import wish_notification_Admin
from skuapp.modelsadminx.t_cfg_b_emsfareAdmin import *
from skuapp.modelsadminx.t_cfg_b_emsfare_countryAdmin import *
from skuapp.modelsadminx.t_cfg_b_emsfare2Admin import *
from skuapp.modelsadminx.t_cfg_b_emsfare_country2Admin import *
from skuapp.modelsadminx.t_cfg_b_logisticwayAdmin import *
from skuapp.modelsadminx.t_cfg_platform_countryAdmin import *
from skuapp.modelsadminx.t_cfg_b_countryAdmin import *
from skuapp.modelsadminx.t_cfg_bracketAdmin import *
from skuapp.modelsadminx.t_cfg_standard_largeAdmin import *
from skuapp.modelsadminx.t_cfg_standard_smallAdmin import *
from skuapp.modelsadminx.t_cfg_standard_large_smallAdmin import *
from skuapp.modelsadminx.t_cfg_categoryAdmin import *
from skuapp.modelsadminx.t_cfg_category_infoAdmin import *
from skuapp.modelsadminx.t_cfg_extra_feeAdmin import *
from skuapp.modelsadminx.GlobalSetting import *
from skuapp.modelsadminx.t_product_b_goods_Admin import *
from skuapp.modelsadminx.t_api_schedule_ed_Admin import *
from skuapp.modelsadminx.t_api_schedule_ing_Admin import *
from skuapp.modelsadminx.t_catainfo_ebay_Admin import *
from skuapp.modelsadminx.t_config_apiurlAdmin import *
from skuapp.modelsadminx.t_config_apiurl_asinAdmin import *
from skuapp.modelsadminx.t_config_apiurl_asin_cf_Admin import *
from skuapp.modelsadminx.t_config_apiurl_asin_operation_Admin import *
from skuapp.modelsadminx.t_config_apiurl_asin_clothing_Admin import *
from skuapp.modelsadminx.t_config_apiurl_asin_kfAdmin import *
from skuapp.modelsadminx.t_config_mstsc_Admin import *
from skuapp.modelsadminx.t_config_mstscfinance_Admin import *
from skuapp.modelsadminx.t_config_online_amazon_Admin import *
from skuapp.modelsadminx.t_helpAdmin import *
from skuapp.modelsadminx.t_hotsale_proinfo_ebay_Admin import *
from skuapp.modelsadminx.v_hotsale_proinfo_ebay_cloth_Admin import *
from skuapp.modelsadminx.t_report_GET_SP_AUTO_TARGETING_REPORT_Admin import *
from skuapp.modelsadminx.t_shop_amazon_india_Admin import *
from skuapp.modelsadminx.t_online_info_Admin import *
from skuapp.modelsadminx.t_order_ListOrders_Admin import *
from skuapp.modelsadminx.t_product_Admin import *
from skuapp.modelsadminx.t_product_art_ed_Admin import *
from skuapp.modelsadminx.t_product_art_ing_Admin import *
from skuapp.modelsadminx.t_product_art_pre_ed_Admin import *
from skuapp.modelsadminx.t_product_build_ing_Admin import *
from skuapp.modelsadminx.t_product_depart_get_Admin import *
from skuapp.modelsadminx.t_product_develop_ed_Admin import *
from skuapp.modelsadminx.t_product_develop_ing_Admin import *
from skuapp.modelsadminx.t_product_develop_audit_Admin import t_product_develop_audit_Admin
from skuapp.modelsadminx.t_product_enquiry_ing_Admin import *
from skuapp.modelsadminx.t_product_enter_ed_Admin import *
from skuapp.modelsadminx.t_product_enter_ed_user_Admin import *
from skuapp.modelsadminx.t_product_depart_get_user_Admin import *
from skuapp.modelsadminx.t_product_enter_ing_Admin import *
from skuapp.modelsadminx.t_product_information_modify_Admin import *
from skuapp.modelsadminx.t_product_modify_entry_Admin import *
from skuapp.modelsadminx.t_product_modify_review_Admin import *
from skuapp.modelsadminx.t_product_modify_ed_Admin import *
from skuapp.modelsadminx.t_product_oplog_Admin import *
from skuapp.modelsadminx.t_product_photograph_Admin import *
from skuapp.modelsadminx.t_product_platform_group_Admin import *
from skuapp.modelsadminx.t_product_questions_Adimin import *
from skuapp.modelsadminx.t_product_questions_outAdmin import *
from skuapp.modelsadminx.t_product_recycle_Admin import *
from skuapp.modelsadminx.t_product_repeats_Admin import *
from skuapp.modelsadminx.t_product_survey_ed_Admin import *
from skuapp.modelsadminx.t_product_survey_history_Admin import *
from skuapp.modelsadminx.t_product_survey_ing_Admin import *
from skuapp.modelsadminx.t_product_unique_ed_Admin import *
from skuapp.modelsadminx.t_product_wait_enquiry_Admin import *
from skuapp.modelsadminx.t_report_GET_SP_MEGA_REPORT_Admin import *
from skuapp.modelsadminx.t_report_orders1days_Admin import *
from skuapp.modelsadminx.t_report_potential_wish_Admin import *
from skuapp.modelsadminx.t_report_week_Admin import *
from skuapp.modelsadminx.t_report_week_dykf_Admin import *
from skuapp.modelsadminx.t_report_week_jzl_Admin import *
from skuapp.modelsadminx.t_sys_department_Admin import *
from skuapp.modelsadminx.t_sys_department_staff_Admin import *
from skuapp.modelsadminx.t_sys_param_Admin import *
from skuapp.modelsadminx.t_sys_platform_role_Admin import *
from skuapp.modelsadminx.t_sys_shopdef_Admin import *
from skuapp.modelsadminx.t_tort_aliexpress_Admin import *
from skuapp.modelsadminx.t_yearweek_Admin import *
from skuapp.modelsadminx.v_product_allsku_Admin import *
from skuapp.modelsadminx.v_product_all_image_Admin import *
from skuapp.modelsadminx.v_product_enter_ed_Admin import *
from skuapp.modelsadminx.t_store_configuration_file_Admin import *
from skuapp.modelsadminx.t_store_marketplan_execution_Admin import *
from skuapp.modelsadminx.t_store_marketplan_execution_ebayAdmin import *
from skuapp.modelsadminx.t_store_marketplan_execution_joomAdmin import *
from skuapp.modelsadminx.t_store_marketplan_execution_aliexpressAdmin import *
from skuapp.modelsadminx.t_store_execution_aliexpressAdmin import *
from skuapp.modelsadminx.t_store_marketplan_execution_amazonAdmin import *
from skuapp.modelsadminx.t_store_marketplan_execution_evaluate_Admin import *
from skuapp.modelsadminx.t_store_weekly_statistics_Admin import *
from skuapp.modelsadminx.t_store_status_Admin import *
from skuapp.modelsadminx.t_store_summary_of_sales_profit_data_Admin import *
from skuapp.modelsadminx.t_shopsku_information_binding_Admin  import *
from skuapp.modelsadminx.b_goodsskulinkshop_Admin  import *
from skuapp.modelsadminx.t_online_info_wish_Admin  import *
from skuapp.modelsadminx.t_report_potential_wish_Admin  import *
from skuapp.modelsadminx.t_report_orders1days_Admin  import *  
from skuapp.modelsadminx.t_order_refunded_Admin  import * 
from skuapp.modelsadminx.t_download_infoAdmin  import * 
from skuapp.modelsadminx.t_importfile_Admin  import *
from skuapp.modelsadminx.t_distribution_product_to_store_Admin  import *
from skuapp.modelsadminx.t_amazon_ad_serving_status_Admin  import *
from skuapp.modelsadminx.b_packinfo_Admin  import *
from skuapp.modelsadminx.b_supplier_Admin  import *
from skuapp.modelsadminx.t_product_mainsku_sku_Admin import *
from skuapp.modelsadminx.t_product_ad_conversion_rate_Admin import *
from skuapp.modelsadminx.t_online_info_amazon_Admin import *
from skuapp.modelsadminx.t_online_info_15day_amazon_Admin import *
from skuapp.modelsadminx.t_tort_shelve_wish_Admin import *
from skuapp.modelsadminx.t_store_marketplan_execution_log_Admin   import *
from skuapp.modelsadminx.t_config_user_buyerAdmin   import *
from skuapp.modelsadminx.t_config_user_buyer_taskAdmin   import *
from skuapp.modelsadminx.t_product_price_check_Admin  import *
from skuapp.modelsadminx.t_online_shopinfo_joom2_Admin import *
from skuapp.modelsadminx.t_online_shopinfo_joom3_Admin import *
from skuapp.modelsadminx.t_online_amazon_fba_inventory_Admin import *
from skuapp.modelsadminx.t_online_info_ebay_Admin  import *
from skuapp.modelsadminx.t_report_ebay_orders1days_Admin  import *
from skuapp.modelsadminx.t_report_potential_ebay_Admin  import *
from skuapp.modelsadminx.t_distribution_product_to_store_result_Admin import *
from skuapp.modelsadminx.t_online_info_wait_publish_joom2_Admin import *
from skuapp.modelsadminx.t_online_info_wait_publish_joom3_Admin import *
from skuapp.modelsadminx.t_online_info_wait_publish_TophatterAdmin import *
from skuapp.modelsadminx.t_history_Admin import *
from skuapp.modelsadminx.t_online_info_wait_publish_UMKA_Admin import *
from skuapp.modelsadminx.t_online_info_wait_publish_vova_Admin import *
from skuapp.modelsadminx.t_online_info_wait_publish_jumia_Admin import *
from skuapp.modelsadminx.t_sys_staff_auth_Admin import *
from skuapp.modelsadminx.t_online_info_publish_joom_Admin import *
from skuapp.modelsadminx.t_paypal_payout_logAdmin import *
from skuapp.modelsadminx.t_config_paypal_accountAdmin import *
from skuapp.modelsadminx.t_product_upload_recommend_table_Admin import *
from skuapp.modelsadminx.t_product_price_check_recycle_Admin import *
from skuapp.modelsadminx.t_goods_shelves_Admin import *
from skuapp.modelsadminx.t_tort_brands_infoAdmin import *
from skuapp.modelsadminx.t_upload_to_wish_all_shop_product_Admin import *
from skuapp.modelsadminx.t_upload_shopname_Admin import *
from skuapp.modelsadminx.t_upload_to_wish_all_product_Admin import *
from skuapp.modelsadminx.p_trade_blacklist_zip_Admin import *
from skuapp.modelsadminx.t_product_up_downAdmin import *
from skuapp.modelsadminx.t_product_inventory_warnning_Admin import *
from skuapp.modelsadminx.t_product_inventory_warnning_dead_Admin import *
from skuapp.modelsadminx.t_product_inventory_warnning_stocking_Admin import *
from skuapp.modelsadminx.t_product_inventory_warnning_dead_ed_Admin import *
from skuapp.modelsadminx.t_product_inventory_warnning_dead_ing_Admin import *

from skuapp.modelsadminx.t_order_amazon_india_Admin import *
from skuapp.modelsadminx.t_upload_statistics_table_Admin import *
from skuapp.modelsadminx.t_config_ip_Admin import *
from skuapp.modelsadminx.t_config_ip_ext_Admin import *
from skuapp.modelsadminx.t_goods_shelves_dis_upload_Admin import *
from skuapp.modelsadminx.t_translation_Admin import *
from skuapp.modelsadminx.t_wish_honor_Admin import *
from skuapp.modelsadminx.t_wish_pb_Admin import *
from skuapp.modelsadminx.t_wish_screenshot_Admin import *
from skuapp.modelsadminx.t_wish_honorShop_Admin import *
from skuapp.modelsadminx.t_wish_pbShop_Admin import *
from skuapp.modelsadminx.t_distribution_staff_statistics_table_Admin import *
from skuapp.modelsadminx.t_small_platform_translation_Admin import *
from skuapp.modelsadminx.v_wish_pb_Admin import *
from skuapp.modelsadminx.t_distribution_template_table_Admin import *
from skuapp.modelsadminx.t_product_image_modify_Admin import *
from skuapp.modelsadminx.t_config_apiurl_asin_xprx_Admin import *
from skuapp.modelsadminx.t_use_productsku_apply_for_shopsku_Admin import *
from skuapp.modelsadminx.t_use_productsku_apply_for_shopsku_Admin import *
from skuapp.modelsadminx.t_stocking_demand_list_Admin import *
from skuapp.modelsadminx.t_stocking_demand_audit_Admin import t_stocking_demand_audit_Admin
from skuapp.modelsadminx.t_stocking_demand_passaudit_Admin import t_stocking_demand_passaudit_Admin
from skuapp.modelsadminx.t_stocking_purchase_order_Admin import *
from skuapp.modelsadminx.t_set_warehouse_storage_situation_list_Admin import *
from skuapp.modelsadminx.t_shipping_management_Admin import *
from skuapp.modelsadminx.t_stocking_demand_fba_Admin import t_stocking_demand_fba_Admin
from skuapp.modelsadminx.t_stocking_demand_fba_purchase_Admin import t_stocking_demand_fba_purchase_Admin
from skuapp.modelsadminx.t_stocking_demand_fba_check_Admin import t_stocking_demand_fba_check_Admin
from skuapp.modelsadminx.t_stocking_demand_fba_genbatch_Admin import t_stocking_demand_fba_genbatch_Admin
from skuapp.modelsadminx.t_stocking_demand_fba_deliver_Admin import t_stocking_demand_fba_deliver_Admin
from skuapp.modelsadminx.t_stocking_demand_fba_all_Admin import t_stocking_demand_fba_all_Admin
from skuapp.modelsadminx.t_stocking_demand_fba_detail_Admin import t_stocking_demand_fba_detail_Admin
from skuapp.modelsadminx.t_stocking_demand_fbw_Admin import t_stocking_demand_fbw_Admin
from skuapp.modelsadminx.t_stocking_demand_fbw_deliver_Admin import t_stocking_demand_fbw_deliver_Admin
from skuapp.modelsadminx.t_stocking_demand_fbw_demand_Admin import t_stocking_demand_fbw_demand_Admin
from skuapp.modelsadminx.t_stocking_demand_fbw_management_Admin import t_stocking_demand_fbw_management_Admin
from skuapp.modelsadminx.t_stocking_demand_fbw_all_Admin import t_stocking_demand_fbw_all_Admin
from skuapp.modelsadminx.t_saler_profit_config_Admin import t_saler_profit_config_Admin
from skuapp.modelsadminx.t_saler_profit_reportform_Admin import t_saler_profit_reportform_Admin
from skuapp.modelsadminx.t_stocking_reject_fba_Admin import t_stocking_reject_fba_Admin
from skuapp.modelsadminx.t_stocking_rejecting_fba_Admin import t_stocking_rejecting_fba_Admin
from skuapp.modelsadminx.t_help_banding_sku_to_ali_Admin import *


from skuapp.modelsadminx.t_joom_refund_Admin import *
from skuapp.modelsadminx.t_config_logisticAdmin import *
from skuapp.modelsadminx.t_config_logistic_expressAdmin import *
from skuapp.modelsadminx.t_online_info_logistic_Admin import *
from skuapp.modelsadminx.t_sensitive_sku_info_Admin import *
from skuapp.modelsadminx.t_sensitive_word_info_Admin import *
from skuapp.modelsadminx.t_aliexpress_refund_info_Admin import *
from skuapp.modelsadminx.t_aliexpress_refund_info_all_Admin import *
from skuapp.modelsadminx.t_aliexpress_compare_price_Admin import t_aliexpress_compare_price_Admin
from skuapp.modelsadminx.t_product_build_FBA_Admin import t_product_build_FBA_Admin
from skuapp.modelsadminx.t_product_enter_FBA_Admin import t_product_enter_FBA_Admin
from skuapp.modelsadminx.t_templet_ebay_collection_box_Admin import *
from skuapp.modelsadminx.t_templet_public_ebay_listing_Admin import *
from skuapp.modelsadminx.t_templet_public_ebay_Admin import *
from skuapp.modelsadminx.t_templet_ebay_wait_upload_Admin import *
from skuapp.modelsadminx.t_templet_ebay_upload_result_Admin import *
from skuapp.modelsadminx.t_ebay_product_image_modify_Admin import *
from skuapp.modelsadminx.t_config_wishapi_product_analyse_info_Admin import *
from skuapp.modelsadminx.t_config_wishapi_product_analyse_info_xprx_Admin import *
from skuapp.modelsadminx.t_config_wishapi_product_analyse_info_bssp_Admin import *
from skuapp.modelsadminx.t_config_wishapi_product_analyse_info_xpbs_Admin import *
from skuapp.modelsadminx.t_config_store_ebay_Admin import *
from skuapp.modelsadminx.t_config_wishapi_product_analyse_info_latest_add_Admin import *
from skuapp.modelsadminx.w_work_report_Admin import *
from skuapp.modelsadminx.d_work_report_Admin import *
from skuapp.modelsadminx.not_clothing_salesman_registration_Admin import *
from skuapp.modelsadminx.t_product_pic_completion_Admin import t_product_pic_completion_Admin
from skuapp.modelsadminx.t_product_photo_ing_Admin import t_product_photo_ing_Admin
from skuapp.modelsadminx.t_product_photo_alreadly_Admin import t_product_photo_alreadly_Admin
from skuapp.modelsadminx.paypal_tort_Admin import *
from skuapp.modelsadminx.t_aliexpress_refund_Admin import *
from skuapp.modelsadminx.t_wish_keywords_Admin import t_wish_keywords_Admin
from skuapp.modelsadminx.t_online_info_amazon_listing_Admin import *
from skuapp.modelsadminx.t_templet_amazon_collection_reverse_Admin import *
from skuapp.modelsadminx.amzon_india_price_config_Admin import amzon_india_price_config_Admin
from skuapp.modelsadminx.t_trademark_Admin import t_trademark_Admin
from skuapp.modelsadminx.t_copyright_Admin import t_copyright_Admin
from skuapp.modelsadminx.t_patents_Admin import t_patents_Admin







from skuapp.modelsadminx.t_report_sales_clothingsystem_Admin import *
from skuapp.modelsadminx.t_templet_amazon_upload_fail_Admin import t_templet_amazon_upload_fail_Admin
from skuapp.modelsadminx.t_templet_amazon_recycle_bin_Admin import t_templet_amazon_recycle_bin_Admin
from skuapp.modelsadminx.t_weekly_collar_and_publication_statistics_table_Admin import t_weekly_collar_and_publication_statistics_table_Admin
from skuapp.modelsadminx.t_combination_sku_log_Admin import t_combination_sku_log_Admin
from skuapp.modelsadminx.t_tort_info_Admin import *
from skuapp.modelsadminx.t_tort_info_audit_Admin import *
from skuapp.modelsadminx.t_tort_info_result_Admin import *
from skuapp.modelsadminx.t_tort_info_result_no_Admin import *
from skuapp.modelsadminx.t_tort_info_result_common_Admin import *
from skuapp.modelsadminx.t_tort_info_cancel_Admin import *
from skuapp.modelsadminx.t_tort_info_cancel_audit_Admin import *
from skuapp.modelsadminx.t_tort_info_image_Admin import *
from skuapp.modelsadminx.t_tort_info_sync_Admin import *
from skuapp.modelsadminx.t_tort_info_query_Admin import *
from skuapp.modelsadminx.t_config_mstscfinance_user_Admin import t_config_mstscfinance_user_Admin
from skuapp.modelsadminx.t_marketing_review_trace_Admin import t_marketing_review_trace_Admin

from skuapp.modelsadminx.t_wish_pb_campaignproductstats_Admin import *
from skuapp.modelsadminx.t_wish_pb_productdailystats_Admin import *
from skuapp.modelsadminx.t_wish_pb_report_Admin import *
from skuapp.modelsadminx.t_work_flow_of_plate_house_admin import t_work_flow_of_plate_house_admin
from skuapp.modelsadminx.t_work_battledore_admin import t_work_battledore_admin
from skuapp.modelsadminx.t_wish_pb_finance_rep_Admin import *

from skuapp.modelsadminx.t_wish_pb_earnings_show_Admin import *
from modelsadminx.t_eBay_TrendKw_Admin import *
from skuapp.modelsadminx.t_product_price_suggest_Admin import *

from skuapp.modelsadminx.t_report_sales_daily_byPlatform_Admin import *
from skuapp.modelsadminx.t_report_sales_daily_byShopName_Admin import *
from skuapp.modelsadminx.t_report_sales_daily_byProductID_Admin import *
from skuapp.modelsadminx.t_report_sales_daily_Admin import *
from skuapp.modelsadminx.t_report_sales_daily_byMainSKU_Admin import *
from skuapp.modelsadminx.t_report_sales_daily_bySKU_Admin import *
from skuapp.modelsadminx.t_wish_activerate_Admin import *


#from skuapp.plugin.CategoryPlugin import *
from skuapp.plugin.sales_count_Plugin import sales_count_Plugin

from skuapp.plugin.show_picturePlugin import *
from skuapp.plugin.marketplan_Plugin import *

xadmin.site.register_plugin(marketplan_Plugin, ListAdminView)
from skuapp.plugin.t_helpPlugin import *
from skuapp.plugin.t_product_build_ingPlugin import *
from skuapp.plugin.t_product_downloadxls_Plugin import *
from skuapp.plugin.t_product_enter_edPlugin import *
from skuapp.plugin.timePlugin import *
from skuapp.plugin.top_reportPlugin import *
from skuapp.plugin.top_top_navbarPlugin import *
from skuapp.plugin.search_cata_navPlugin import *
from skuapp.plugin.t_config_user_buyerPlugin import *
from skuapp.plugin.button_Plugin import *
from skuapp.plugin.t_paypal_payout_logPlugin import *
from skuapp.plugin.t_product_enter_ed_classificationPlugin import *
from skuapp.plugin.wish_search_barPlugin import *
from skuapp.plugin.import_joomPlugin import *
from skuapp.plugin.select_checkboxPlugin import *
from skuapp.plugin.publish_joomPlugin import *
from skuapp.plugin.publish_joom3Plugin import *
from skuapp.plugin.publish_tophatterPlugin import *
from skuapp.plugin.publish_UMKAPlugin import *
from skuapp.plugin.publish_vovaPlugin import *
from skuapp.plugin.publish_jumiaPlugin import *
from skuapp.plugin.t_upload_statistics_table_Plugin import *
from skuapp.plugin.t_goods_shelves_search_Plugin import *
from skuapp.plugin.t_amazon_cpc_ad_plugin import t_amazon_cpc_ad_plugin
from skuapp.plugin.t_online_info_amazon_listing_plugin import *
xadmin.site.register_plugin(t_online_info_amazon_listing_plugin, ListAdminView)
from skuapp.plugin.t_product_enter_ed_plugin import t_product_enter_ed_menu_Plugin
xadmin.site.register_plugin(t_product_enter_ed_menu_Plugin, ListAdminView)
from skuapp.plugin.search_boxPlugin import *
from skuapp.plugin.search_box1Plugin import *
from skuapp.plugin.search_box2Plugin import *
from skuapp.plugin.price_check_categoryPlugin import *
from skuapp.plugin.t_product_up_downPlugin import *
from skuapp.plugin.black_list_Plugin import *
from skuapp.plugin.t_prompt_developPlugin import *
from skuapp.plugin.search_sku_Plugin import *
from skuapp.plugin.wish_distribution_listing_sku_query_Plugin import *
from skuapp.plugin.site_left_menu_Plugin import *
from skuapp.plugin.amazon_site_left_menu_tree_Plugin import *
from skuapp.plugin.plateform_distribution_navigation_Plugin import *
from skuapp.plugin.variation_item_amazon_Plugin import *
from skuapp.plugin.importfile_stocking_demand_plugin import *
from skuapp.plugin.importfile_aliexpress_refundPlugin import *
from skuapp.plugin.purchase_order_button_plugin import *
from skuapp.plugin.kc_get_money import *
from skuapp.plugin.kc_jump_status import *
from skuapp.plugin.t_online_info_logisticPlugin import *
from skuapp.plugin.jump_temp_some_page import *
from skuapp.plugin.task_trunkPlugin import *
from skuapp.plugin.t_task_trunkPlugin import *
from skuapp.plugin.t_product_udPlugin import *
from skuapp.plugin.wish_distribution_order_sales__chart_Plugin import *
from skuapp.plugin.importfile_paypal_tortPlugin import *
from skuapp.plugin.shopsku_apply_plugin import shopsku_apply_plugin
from skuapp.plugin.tort_Plugin import tort_Plugin
from skuapp.plugin.t_tort_wishPlugin import t_tort_wishPlugin
from skuapp.plugin.t_aliexpress_refund_infoPlugin import t_aliexpress_refund_infoPlugin
from skuapp.plugin.t_product_build_FBAPlugin import t_product_build_FBAPlugin
from skuapp.plugin.t_product_suringPlugin import t_product_suringPlugin

from skuapp.plugin.t_product_quality_feedback_Plugin import t_product_quality_feedback_Plugin
from skuapp.plugin.kc_refresh_time import *
from skuapp.plugin.Large_Small_Plugin import Large_Small_Plugin

from skuapp.plugin.Three_Grade_Classification_Of_Clothing_Plugin import Three_Grade_Classification_Of_Clothing_Plugin
from skuapp.plugin.importfile_aliexpress_service_analysis import importfile_aliexpress_service_analysis
from skuapp.plugin.t_cloth_factory_Plugin import t_cloth_factory_Plugin
from skuapp.plugin.t_aliexpress_compare_pricePlugin import t_aliexpress_compare_pricePlugin
from skuapp.plugin.t_report_sales_clothingsystem_chart_Plugin import *
from skuapp.plugin.t_weekly_collar_and_pub_plugin import t_weekly_collar_and_pub_plugin
from skuapp.plugin.repeat_sku_plugin import repeat_sku_Plugin
from skuapp.plugin.t_config_mstsc_searchplugin import t_config_Search_Plugin
from skuapp.plugin.Main_SKU_countplugin import Main_SKU_countplugin
from skuapp.plugin.t_amazon_cpc_ad_left_menu_plugin import t_amazon_cpc_ad_left_menu_plugin
from skuapp.plugin.t_product_information_modify_Plguin import t_product_information_modify_Plugin

from skuapp.plugin.enter_ed_userPlugin import enter_ed_user_Plugin
from skuapp.plugin.depart_get_userPlugin import depart_get_user_Plugin
from skuapp.plugin.joom_refundPlugin import joom_refundPlugin
from skuapp.plugin.t_tort_info_tree_menu_Plugin import *
from skuapp.plugin.sales_show_byplatformnamePlugin import *
from skuapp.plugin.sales_show_bytypePlugin import *
from skuapp.plugin.salesshowlisting_Plugin import *
from skuapp.plugin.t_wish_pb_productdailystats_Plugin import *
from skuapp.plugin.t_wish_activerate_Plugin import *
from skuapp.plugin.t_wishpb_syncdata_Plugin import *
from skuapp.plugin.t_wish_pb_earnings_Plugin import *

from skuapp.plugin.t_wish_pb_left_menu_Plugin import *
from skuapp.plugin.t_wishpb_keywords_Plugin import *
from skuapp.plugin.t_wish_pb_report_Plugin import *
from skuapp.plugin.b_supplier_Plugin import b_supplier_Plugin
from skuapp.plugin.wish_processed_order_Plugin import wish_processed_order_Plugin
xadmin.site.register_plugin(wish_processed_order_Plugin, ListAdminView)
from skuapp.plugin.wish_notification_Plugin import wish_notification_Plugin
xadmin.site.register_plugin(wish_notification_Plugin, ListAdminView)
from skuapp.plugin.t_stocking_check_reportPlugin import t_stocking_check_reportPlugin
xadmin.site.register_plugin(t_stocking_check_reportPlugin, ListAdminView)
from skuapp.plugin.t_stocking_purchase_orderPlugin import t_stocking_purchase_orderPlugin
xadmin.site.register_plugin(t_stocking_purchase_orderPlugin, ListAdminView)
from skuapp.plugin.site_left_menu_stocking_purchase_Plugin import site_left_menu_stocking_purchase_Plugin
xadmin.site.register_plugin(site_left_menu_stocking_purchase_Plugin, ListAdminView)
from skuapp.plugin.t_stocking_demand_fba_tree_menu_Plugin import t_stocking_demand_fba_tree_menu_Plugin
xadmin.site.register_plugin(t_stocking_demand_fba_tree_menu_Plugin, ListAdminView)
from skuapp.plugin.t_stocking_demand_fbw_tree_menu_Plugin import t_stocking_demand_fbw_tree_menu_Plugin
xadmin.site.register_plugin(t_stocking_demand_fbw_tree_menu_Plugin, ListAdminView)
from skuapp.plugin.hide_page_action_Plugin import hide_page_action_Plugin
xadmin.site.register_plugin(hide_page_action_Plugin, ListAdminView)

from skuapp.plugin.t_amazon_comment_Plugin import t_amazon_comment_Plugin
from skuapp.plugin.t_amazon_comment_analy_Plugin import t_amazon_comment_analy_Plugin








from modelsadminx.t_templet_wish_collection_box_Admin import *
from modelsadminx.t_templet_public_wish_Admin import *
from modelsadminx.t_templet_wish_wait_upload_Admin import *
from modelsadminx.t_templet_wish_upload_result_Admin import *
from modelsadminx.t_templet_public_wish_listing_Admin import *
from modelsadminx.t_templet_public_amazon_listing_Admin import *
from modelsadminx.t_templet_amazon_collection_box_Admin import *
from modelsadminx.t_templet_amazon_upload_result_Admin import *
from modelsadminx.t_templet_amazon_wait_upload_Admin import *
from modelsadminx.t_templet_amazon_wait_published_Admin import *
from modelsadminx.t_templet_aliexpress_collection_box_Admin import *
from modelsadminx.t_templet_public_aliexpress_Admin import *
from modelsadminx.t_templet_aliexpress_wait_upload_Admin import *
from modelsadminx.t_templet_joom_collection_box_Admin import *
from modelsadminx.t_templet_public_joom_listing_Admin import *
from modelsadminx.t_templet_public_joom_Admin import *
from modelsadminx.t_templet_joom_wait_upload_Admin import *
from modelsadminx.t_product_wish_off_shelf_Admin import *
from skuapp.plugin.cexport_refund_to_oss_Plugin import *
from modelsadminx.t_task_trunk_Admin import *
from modelsadminx.t_task_trunk_case_Admin import *
from modelsadminx.t_task_details_Admin import *
from modelsadminx.t_task_operation_log_Admin import *
from skuapp.plugin.t_config_store_ebay_regetpes_plugin import *
from skuapp.plugin.t_wish_keywords_downloads_plugin import t_wish_keywords_downloads_plugin
from skuapp.plugin.reverse_amazon_collection_box_plugin import reverse_amazon_collection_box_plugin
from skuapp.plugin.importfile_marketing_Plugin import importfile_marketing_Plugin
from skuapp.modelsadminx.t_product_survering_admin import t_product_survering_admin

from skuapp.plugin.t_product_udPlugin import *
from skuapp.modelsadminx.z_clothing_Admin import clothing_salesman_registrationAdmin
from skuapp.modelsadminx.goods_quality_check_Admin import Newshop_feedbackAdmin, GoodsResponse_tableAdmin,BugerResponse_tableAdmin,WarehouseSpecialist_tableAdmin,GoodsCheck_tableAdmin,Result_tableAdmin
from modelsadminx.t_amazon_pro_comment_Admin import t_amazon_pro_comment_Admin

from modelsadminx.t_task_trunk_Admin import *
from modelsadminx.t_task_details_Admin import *
from modelsadminx.t_task_operation_log_Admin import *



from skuapp.table.t_product_b_goods import *
from modelsadminx.t_wish_distribution_result_statistics_everyday_Admin import *
from modelsadminx.t_wish_distribution_sales_statistics_everyday_Admin import *
from modelsadminx.t_wish_distribution_sales_statistics_total_Admin import *
from modelsadminx.t_order_out_of_stock_Admin import *
from modelsadminx.t_product_quality_feedback_submit_Admin import *
from modelsadminx.t_product_quality_feedback_cpzy_Admin import *
from modelsadminx.t_product_quality_feedback_cgy_Admin import *
from modelsadminx.t_product_quality_feedback_zjy_sh_Admin import *
from modelsadminx.t_product_quality_feedback_zjy_Admin import *
from modelsadminx.t_product_quality_feedback_ck_Admin import *
from modelsadminx.t_product_quality_feedback_final_sh_Admin import *
from modelsadminx.t_product_quality_feedback_final_result_Admin import *
from modelsadminx.b_goods_sku_status_change_Admin import *

from skuapp.modelsadminx.t_cloth_factory_dispatch_needpurchase_Admin import t_cloth_factory_dispatch_needpurchase_Admin
from skuapp.modelsadminx.t_cloth_factory_dispatch_noneedpurchase_Admin import t_cloth_factory_dispatch_noneedpurchase_Admin
from skuapp.modelsadminx.t_cloth_factory_dispatch_plan_Admin import t_cloth_factory_dispatch_plan_Admin
from skuapp.modelsadminx.t_cloth_factory_dispatch_spemanmodify_Admin import t_cloth_factory_dispatch_spemanmodify_Admin
from skuapp.modelsadminx.t_cloth_factory_dispatch_paid_audit_Admin import t_cloth_factory_dispatch_paid_audit_Admin
from skuapp.modelsadminx.t_cloth_factory_dispatch_no_audit_Admin import t_cloth_factory_dispatch_no_audit_Admin
from skuapp.modelsadminx.t_cloth_factory_dispatch_audit_Admin import t_cloth_factory_dispatch_audit_Admin
from skuapp.modelsadminx.t_cloth_factory_dispatch_commit_Admin import t_cloth_factory_dispatch_commit_Admin
from skuapp.modelsadminx.t_cloth_factory_dispatch_confirm_Admin import t_cloth_factory_dispatch_confirm_Admin
from skuapp.modelsadminx.t_cloth_factory_dispatch_overtobuild_Admin import t_cloth_factory_dispatch_overtobuild_Admin
from skuapp.modelsadminx.t_cloth_factory_dispatch_close_Admin import t_cloth_factory_dispatch_close_Admin
from skuapp.modelsadminx.t_cloth_factory_dispatch_history_Admin import t_cloth_factory_dispatch_history_Admin
from skuapp.modelsadminx.t_cloth_factory_Admin import t_cloth_factory_Admin
from skuapp.modelsadminx.t_cloth_factory_dispatch_paiding_Admin import t_cloth_factory_dispatch_paiding_Admin
from skuapp.modelsadminx.t_aliexpress_distribution_shop_Admin import t_aliexpress_distribution_shop_Admin
from skuapp.modelsadminx.t_templet_public_wish_review_Admin import t_templet_public_wish_review_Admin
from skuapp.modelsadminx.t_amazon_cpc_ad_Admin import t_amazon_cpc_ad_Admin
from skuapp.modelsadminx.t_templet_wish_upload_review_Admin import t_templet_wish_upload_review_Admin
from skuapp.modelsadminx.t_supply_chain_production_basic_Admin import t_supply_chain_production_basic_Admin
from skuapp.modelsadminx.t_config_shop_alias_Admin import t_config_shop_alias_Admin


from skuapp.table.t_joom_refund import *
from skuapp.table.t_cfg_b_currencycode import *
from skuapp.table.t_cfg_b_currencycode_log import *
from skuapp.table.t_cfg_b_emsfare import *
from skuapp.table.t_cfg_b_emsfare_country import *
from skuapp.table.t_cfg_b_emsfare2 import *
from skuapp.table.t_cfg_b_emsfare_country2 import *
from skuapp.table.t_cfg_b_logisticway import *
from skuapp.table.t_cfg_platform_country import *
from skuapp.table.t_cfg_b_country import *
from skuapp.table.t_cfg_bracket import *
from skuapp.table.t_cfg_standard_large import *
from skuapp.table.t_cfg_standard_small import *
from skuapp.table.t_cfg_standard_large_small import *
from skuapp.table.t_cfg_category import *
from skuapp.table.t_cfg_category_info import *
from skuapp.table.t_cfg_extra_fee import *
from skuapp.table.t_report_GET_SP_AUTO_TARGETING_REPORT_  import *
#from skuapp.table.B_PackInfo  import *
from pyapp.models  import xxxb_packinfo
from skuapp.table.b_goods                     import *
from skuapp.table.model_name                    import *
from skuapp.table.t_api_schedule                import *
from skuapp.table.t_api_schedule_ed             import *
from skuapp.table.t_api_schedule_ing            import *
from skuapp.table.t_base                      import *
from skuapp.table.t_catainfo_ebay               import *
from skuapp.table.t_config_apiurl               import *
from skuapp.table.t_config_apiurl_asin          import *
from skuapp.table.t_config_apiurl_asin_cf          import *
from skuapp.table.t_config_apiurl_asin_operation   import *
from skuapp.table.t_config_apiurl_asin_clothing import *
from skuapp.table.t_config_apiurl_asin_kf       import *
from skuapp.table.t_config_mstsc                import *
from skuapp.table.t_config_mstscfinance         import *
from skuapp.table.t_config_online_amazon        import *
from skuapp.table.t_help                        import *
from  skuapp.table.t_history                    import *
from skuapp.table.t_hotsale_proinfo_ebay        import *
from skuapp.table.t_online_info                 import *
from skuapp.table.t_order_ListOrders            import *
from skuapp.table.t_product_art_ed              import *
from skuapp.table.t_product_art_ing             import *
from skuapp.table.t_product_art_pre_ed          import *
from skuapp.table.t_product_build_ing           import *
from skuapp.table.t_product_depart_get          import *
from skuapp.table.t_product_develop_ed          import *
from skuapp.table.t_product_develop_ing         import *
from skuapp.table.t_product_develop_audit       import t_product_develop_audit
from skuapp.table.t_product_enquiry_ing         import *
from skuapp.table.t_product_enter_ed            import *
from skuapp.table.t_product_enter_ed_user       import *
from skuapp.table.t_product_depart_get_user     import *
from skuapp.table.t_product_enter_ing           import *
from skuapp.table.t_product_information_modify  import *
from skuapp.table.t_product_mainsku_sku         import *
from skuapp.table.t_product_modify_entry        import *
from skuapp.table.t_product_modify_review       import *
from skuapp.table.t_product_modify_ed           import *
from skuapp.table.t_product_oplog               import *
from skuapp.table.t_product_photograph          import *
from skuapp.table.t_product_platform_group      import *
from skuapp.table.t_product_questions           import *
from skuapp.table.t_product_questions_out       import *
from skuapp.table.t_product_recycle             import *
from skuapp.table.t_product_repeats             import *
from skuapp.table.t_product_survey_ed           import *
from skuapp.table.t_product_survey_history      import *
from skuapp.table.t_product_survey_ing          import *
from skuapp.table.t_product_unique_ed           import *
from skuapp.table.t_product_wait_enquiry        import *
from skuapp.table.t_report_GET_SP_MEGA_REPORT_  import *
from skuapp.table.t_report_week                 import *
from skuapp.table.t_report_week_dykf            import *
from skuapp.table.t_report_week_jzl             import *
from skuapp.table.t_store_plan_warning          import *
from skuapp.table.t_syn_tables                  import *
from skuapp.table.t_sys_department              import *
from skuapp.table.t_sys_department_staff        import *
from skuapp.table.t_sys_param                   import *
from skuapp.table.t_sys_shopdef                 import *
from skuapp.table.t_tort_aliexpress             import *
from skuapp.table.t_yearweek                    import *
from skuapp.table.v_UMG                         import *
from skuapp.table.v_app_model_name              import *
from skuapp.table.v_django_type                 import *
from skuapp.table.v_product_allsku              import *
from skuapp.table.v_product_all_image              import *
from skuapp.table.v_product_enter_ed            import *
from skuapp.table.v_user_content_type_id        import *
from skuapp.table.v_user_model                  import *
from skuapp.table.v_user_model_name             import *
from skuapp.table.v_user_permission_id          import *
from skuapp.table.t_shopsku_information_binding import *
from skuapp.table.t_store_configuration_file    import *
from skuapp.table.t_store_marketplan_execution  import *
from skuapp.table.t_store_marketplan_execution_ebay  import *
from skuapp.table.t_store_marketplan_execution_evaluate import *
from skuapp.table.t_store_marketplan_execution_joom import *
from skuapp.table.t_store_marketplan_execution_aliexpress import *
from skuapp.table.t_store_execution_aliexpress import *
from skuapp.table.t_store_marketplan_execution_amazon import *
from skuapp.table.t_store_weekly_statistics    import *
from skuapp.table.t_store_status                import *
from skuapp.table.t_store_summary_of_sales_profit_data      import *
from skuapp.table.t_online_info_wish      import *
from skuapp.table.t_report_potential_wish      import *
from skuapp.table.t_report_orders1days      import *
from skuapp.table.t_importfile      import *
from skuapp.table.v_hotsale_proinfo_ebay_cloth import *
from skuapp.table.t_order_refunded import *
from skuapp.table.t_download_info import *
from skuapp.table.t_distribution_product_to_store import *
from skuapp.table.t_ShopName_ParentSKU import *
from skuapp.table.t_amazon_ad_serving_status import *
#from skuapp.table.b_supplier import *
from pyapp.models  import B_Supplier
from skuapp.table.t_config_apiurl_asin_rank_history import *
from skuapp.table.t_online_shopinfo_amazon import *
#from skuapp.table.b_goodssku import *
from skuapp.table.t_product_mainsku_sku import *
from skuapp.table.t_product_ad_conversion_rate import *
from skuapp.table.t_online_info_amazon import *
from skuapp.table.t_online_info_15day_amazon import *
from skuapp.table.t_tort_shelve_wish import *
from skuapp.table.t_config_user_buyer  import *
from skuapp.table.t_config_user_buyer_task import *
from skuapp.table.t_store_marketplan_execution_log    import *
from skuapp.table.t_product_price_check import *
from skuapp.table.t_online_shopinfo_joom2 import *
from skuapp.table.t_online_shopinfo_joom3 import *
from skuapp.table.v_wish_pb import *
from skuapp.table.t_online_amazon_fba_inventory import *
from skuapp.table.t_online_info_ebay import *
from skuapp.table.t_report_ebay_orders1days import *
from skuapp.table.t_report_potential_ebay import *
from skuapp.table.t_online_info_wait_publish_joom2 import *
from skuapp.table.t_online_info_wait_publish_joom3 import *
from skuapp.table.t_online_info_wait_publish_tophatter import *
from skuapp.table.t_online_info_wait_publish_UMKA import *
from skuapp.table.t_online_info_wait_publish_vova import *
from skuapp.table.t_online_info_wait_publish_jumia import *
from skuapp.table.t_sys_staff_auth import *
from skuapp.table.t_online_info_publish_joom import *
from skuapp.table.t_distribution_product_to_store_result import *
from skuapp.table.t_config_paypal_account import *
from skuapp.table.t_paypal_payout_log import *
from skuapp.table.t_product_upload_recommend_table import *
from skuapp.table.t_product_price_check_recycle import *
from skuapp.table.t_goods_shelves import *
from skuapp.table.t_tort_brands_info import *
from skuapp.table.t_upload_to_wish_all_shop_product import *
from skuapp.table.t_upload_shopname import *
from skuapp.table.t_upload_to_wish_all_product import *
from skuapp.table.t_upload_statistics_table import *
from skuapp.table.t_config_ip import *
from skuapp.table.t_config_ip_ext import *
from skuapp.table.t_goods_shelves_dis_upload import *
from skuapp.table.t_translation import *
from skuapp.table.t_wish_honor import *
from skuapp.table.t_wish_pb import *
from skuapp.table.t_wish_screenshot import *
from skuapp.table.t_wish_honorShop import *
from skuapp.table.t_wish_pbShop import *
from skuapp.table.t_distribution_staff_statistics_table import *
from skuapp.table.t_small_platform_translation import *
from skuapp.table.t_distribution_template_table import *
from skuapp.table.p_trade_blacklist_zip import *
from skuapp.table.t_product_up_down import *
from skuapp.table.t_product_inventory_warnning import *
from skuapp.table.t_product_inventory_warnning_dead import *
from skuapp.table.t_product_inventory_warnning_stocking import *
from skuapp.table.t_product_inventory_warnning_dead_ed import *
from skuapp.table.t_product_inventory_warnning_dead_ing import *
from skuapp.table.t_order_amazon_india import *
from skuapp.table.t_shop_amazon_india import *
from skuapp.table.t_templet_wish_collection_box import *
from skuapp.table.t_templet_public_wish import *
from skuapp.table.t_templet_wish_wait_upload import *
from skuapp.table.t_templet_wish_upload_result import *
from skuapp.table.t_templet_public_wish_listing import *
from skuapp.table.t_product_image_modify import *
from skuapp.table.t_templet_amazon_collection_box import *
from skuapp.table.t_templet_public_amazon_listing import *
from skuapp.table.t_templet_amazon_upload_result import *
from skuapp.table.t_templet_amazon_wait_upload import *
from skuapp.table.t_templet_amazon_wait_published import *
from skuapp.table.t_templet_aliexpress_collection_box import *
from skuapp.table.t_templet_public_aliexpress import *
from skuapp.table.t_templet_aliexpress_wait_upload import *
from skuapp.table.t_config_apiurl_asin_xprx import *
from skuapp.table.t_use_productsku_apply_for_shopsku import *
from skuapp.table.t_stocking_demand_list import *
from skuapp.table.t_stocking_demand_audit import t_stocking_demand_audit
from skuapp.table.t_stocking_demand_passaudit import t_stocking_demand_passaudit
from skuapp.table.t_stocking_purchase_order import *
from skuapp.table.t_set_warehouse_storage_situation_list import *
from skuapp.table.t_shipping_management import *
from skuapp.table.t_stocking_demand_fba import t_stocking_demand_fba
from skuapp.table.t_stocking_demand_fba_purchase import t_stocking_demand_fba_purchase
from skuapp.table.t_stocking_demand_fba_check import t_stocking_demand_fba_check
from skuapp.table.t_stocking_demand_fba_genbatch import t_stocking_demand_fba_genbatch
from skuapp.table.t_stocking_demand_fba_deliver import t_stocking_demand_fba_deliver
from skuapp.table.t_stocking_demand_fbw import t_stocking_demand_fbw
from skuapp.table.t_stocking_demand_fbw_deliver import t_stocking_demand_fbw_deliver
from skuapp.table.t_stocking_demand_fbw_demand import t_stocking_demand_fbw_demand
from skuapp.table.t_stocking_demand_fbw_management import t_stocking_demand_fbw_management
from skuapp.table.t_stocking_demand_fbw_all import t_stocking_demand_fbw_all
from skuapp.table.t_stocking_demand_fba_all import t_stocking_demand_fba_all
from skuapp.table.t_stocking_demand_fba_detail import t_stocking_demand_fba_detail
from skuapp.table.t_saler_profit_config import t_saler_profit_config
from skuapp.table.t_saler_profit_reportform import t_saler_profit_reportform
from skuapp.table.t_stocking_reject_fba import t_stocking_reject_fba
from skuapp.table.t_stocking_rejecting_fba import t_stocking_rejecting_fba
from skuapp.table.t_help_banding_sku_to_ali import *
from skuapp.table.t_online_info_amazon_listing import *
from skuapp.table.t_config_logistic import *
from skuapp.table.t_config_logistic_express import *
from skuapp.table.t_online_info_logistic import *
from skuapp.table.t_sensitive_sku_info import *
from skuapp.table.t_sensitive_word_info import *

from skuapp.table.t_templet_ebay_collection_box import *
from skuapp.table.t_templet_public_ebay import *
from skuapp.table.t_templet_ebay_wait_upload import *
from skuapp.table.t_templet_ebay_upload_result import *
from skuapp.table.t_templet_public_ebay_listing import *
from skuapp.table.t_ebay_product_image_modify import *
from skuapp.table.t_config_wishapi_product_analyse_info import *
from skuapp.table.t_config_wishapi_product_analyse_info_xprx import *
from skuapp.table.t_config_wishapi_product_analyse_info_bssp import *
from skuapp.table.t_config_wishapi_product_analyse_info_xpbs import *

from skuapp.table.t_templet_joom_collection_box import *
from skuapp.table.t_templet_public_joom_listing import *
from skuapp.table.t_templet_public_joom import *
from skuapp.table.t_templet_joom_wait_upload import *
from skuapp.table.t_config_wishapi_product_analyse_info_latest_add import *
from skuapp.table.t_product_wish_off_shelf import *

from skuapp.table.t_config_store_ebay import *
from skuapp.table.w_work_report import *
from skuapp.table.d_work_report import *
from skuapp.table.t_task_trunk import *
from skuapp.table.t_task_trunk_case import *
from skuapp.table.t_task_details import *
from skuapp.table.t_task_operation_log import *
from skuapp.table.not_clothing_salesman_registration import *
from skuapp.table.t_product_pic_completion import t_product_pic_completion
from skuapp.table.t_product_photo_ing import t_product_photo_ing
from skuapp.table.t_product_photo_alreadly import t_product_photo_alreadly
from skuapp.table.t_wish_distribution_result_statistics_everyday import *
from skuapp.table.t_wish_distribution_sales_statistics_everyday import *
from skuapp.table.t_wish_distribution_sales_statistics_total import *
from skuapp.table.t_clothing import clothing_salesman_registration
from skuapp.table.t_order_out_of_stock import *
from skuapp.table.goods_quality_check import Newshop_feedback, GoodsResponse_table, BugerResponse_table, WarehouseSpecialist_table, GoodsCheck_table, Result_table
from skuapp.table.paypal_tort import *
from skuapp.table.t_aliexpress_refund import *
from skuapp.table.t_aliexpress_refund_info import *
from skuapp.table.t_aliexpress_refund_info_all import *
from skuapp.table.t_wish_keywords import t_wish_keywords
from skuapp.table.t_product_quality_feedback_submit import *
from skuapp.table.t_product_quality_feedback_cpzy import *
from skuapp.table.t_product_quality_feedback_cgy import *
from skuapp.table.t_product_quality_feedback_zjy_sh import *
from skuapp.table.t_product_quality_feedback_zjy import *
from skuapp.table.t_product_quality_feedback_ck import *
from skuapp.table.t_product_quality_feedback_final_sh import *
from skuapp.table.t_product_quality_feedback_final_result import *
from skuapp.table.t_templet_amazon_collection_reverse import *






from skuapp.table.t_amazon_pro_comment import t_amazon_pro_comment
from skuapp.table.t_trademark import t_trademark
from skuapp.table.t_copyright import t_copyright
from skuapp.table.t_patents import t_patents
from skuapp.table.amzon_india_price_config import amzon_india_price_config
from skuapp.modelsadminx.t_aliexpress_service_division_analysis_Admin import t_aliexpress_service_division_analysis_Admin
from skuapp.table.t_aliexpress_service_division_analysis import t_aliexpress_service_division_analysis
from skuapp.table.t_aliexpress_service_division_standard import t_aliexpress_service_division_standard
from skuapp.modelsadminx.t_aliexpress_service_division_standard_Admin import t_aliexpress_service_division_standard_Admin
from skuapp.table.t_product_suvering import t_product_suvering


from skuapp.table.t_product_upc_id_amazon import *
from skuapp.modelsadminx.t_product_upc_id_amazon_Admin import *
from skuapp.plugin.t_product_upc_id_Plugin import *
from skuapp.plugin.amazon_result_copy_select_sitePlugin import *
from skuapp.table.b_goods_sku_status_change import *
from skuapp.table.t_report_sales_clothingsystem import *
from skuapp.table.t_cloth_factory_dispatch_needpurchase import t_cloth_factory_dispatch_needpurchase
from skuapp.table.t_cloth_factory_dispatch_noneedpurchase import t_cloth_factory_dispatch_noneedpurchase
from skuapp.table.t_cloth_factory_dispatch_plan import t_cloth_factory_dispatch_plan
from skuapp.table.t_cloth_factory_dispatch_spemanmodify import t_cloth_factory_dispatch_spemanmodify
from skuapp.table.t_cloth_factory_dispatch_paid_audit import t_cloth_factory_dispatch_paid_audit
from skuapp.table.t_cloth_factory_dispatch_no_audit import t_cloth_factory_dispatch_no_audit
from skuapp.table.t_cloth_factory_dispatch_audit import t_cloth_factory_dispatch_audit
from skuapp.table.t_cloth_factory_dispatch_commit import t_cloth_factory_dispatch_commit
from skuapp.table.t_cloth_factory_dispatch_confirm import t_cloth_factory_dispatch_confirm
from skuapp.table.t_cloth_factory_dispatch_overtobuild import t_cloth_factory_dispatch_overtobuild
from skuapp.table.t_cloth_factory_dispatch_close import t_cloth_factory_dispatch_close
from skuapp.table.t_cloth_factory_dispatch_history import t_cloth_factory_dispatch_history
from skuapp.table.t_cloth_factory import t_cloth_factory
from skuapp.table.t_cloth_factory_dispatch_paiding import t_cloth_factory_dispatch_paiding
from skuapp.table.t_aliexpress_compare_price import t_aliexpress_compare_price

from skuapp.table.t_templet_amazon_upload_result_lose_pic import *
from skuapp.modelsadminx.t_templet_amazon_upload_result_lose_pic_Admin import *
from skuapp.table.t_templet_amazon_upload_fail import t_templet_amazon_upload_fail
from skuapp.table.t_templet_amazon_recycle_bin import t_templet_amazon_recycle_bin

from skuapp.table.t_product_build_FBA import t_product_build_FBA
from skuapp.table.t_product_enter_FBA import t_product_enter_FBA
from skuapp.table.t_weekly_collar_and_publication_statistics_table import t_weekly_collar_and_publication_statistics_table
from skuapp.table.t_combination_sku_log import t_combination_sku_log
from skuapp.table.t_aliexpress_distribution_shop import t_aliexpress_distribution_shop
from skuapp.table.t_templet_public_wish_review import t_templet_public_wish_review
from skuapp.plugin.show_per_pagePlugin import show_per_page_Plugin
from skuapp.table.t_tort_info import *
from skuapp.table.t_tort_info_audit import t_tort_info_audit
from skuapp.table.t_tort_info_result import t_tort_info_result
from skuapp.table.t_tort_info_result_no import t_tort_info_result_no
from skuapp.table.t_tort_info_result_common import t_tort_info_result_common
from skuapp.table.t_tort_info_cancel import t_tort_info_cancel
from skuapp.table.t_tort_info_cancel_audit import t_tort_info_cancel_audit
from skuapp.table.t_tort_info_image import t_tort_info_image
from skuapp.table.t_tort_info_sync import t_tort_info_sync
from skuapp.table.t_tort_info_query import t_tort_info_query
from skuapp.table.t_config_mstscfinance_user import t_config_mstscfinance_user
from skuapp.table.t_wish_pb_campaignproductstats import *
from skuapp.table.t_wish_pb_productdailystats import *
from skuapp.table.t_wish_pb_report import *
from skuapp.table.t_marketing_review_trace import t_marketing_review_trace
from skuapp.table.wish_processed_order import wish_processed_order
from skuapp.table.wish_notification import wish_notification
from skuapp.table.t_amazon_cpc_ad import t_amazon_cpc_ad
from skuapp.table.t_work_flow_of_plate_house import t_work_flow_of_plate_house
from skuapp.plugin.amazon_india_FBA_sku_Plugin import *
from skuapp.table.t_work_battledore import t_work_battledore
from skuapp.table.t_templet_wish_upload_review import t_templet_wish_upload_review
from skuapp.table.t_supply_chain_production_basic import t_supply_chain_production_basic
from skuapp.table.t_wish_pb_finance_rep import *
from skuapp.table.t_wish_pb_earnings_rep import t_wish_pb_earnings_show
from skuapp.modelsadminx.t_config_amazon_template_Admin import *
from skuapp.table.t_config_amazon_template import *
from skuapp.table.t_stocking_check_report import t_stocking_check_report
from skuapp.modelsadminx.t_stocking_check_report_Admin import t_stocking_check_report_Admin
from skuapp.table.t_config_shop_alias import t_config_shop_alias
from skuapp.table.t_eBay_TrendKw import t_eBay_TrendKw
from skuapp.table.t_product_price_suggest import t_product_price_suggest
from skuapp.table.t_report_sales_daily_byPlatform import *
from skuapp.table.t_report_sales_daily_byShopName import *
from skuapp.table.t_report_sales_daily_byProductID import *
from skuapp.table.t_report_sales_daily import *
from skuapp.table.t_report_sales_daily_byMainSKU import *
from skuapp.table.t_report_sales_daily_bySKU import *
from skuapp.table.t_wish_activerate import t_wish_activerate
from skuapp.table.b_goods_sales_count import b_goods_sales_count


xadmin.site.register(t_config_amazon_template,t_config_amazon_template_Admin)
xadmin.site.register(t_product_upc_id_amazon,t_product_upc_id_amazon_Admin)

from skuapp.table.t_product_saler_requirement_photograph import t_product_saler_requirement_photograph
from skuapp.modelsadminx.t_product_saler_requirement_photograph_Admin import t_product_saler_requirement_photograph_Admin
xadmin.site.register(t_cfg_b_currencycode, t_cfg_b_currencycodeAdmin)
xadmin.site.register(t_cfg_b_currencycode_log, t_cfg_b_currencycode_logAdmin)
xadmin.site.register(t_cfg_b_emsfare, t_cfg_b_emsfareAdmin)
xadmin.site.register(t_cfg_b_emsfare_country, t_cfg_b_emsfare_countryAdmin)
xadmin.site.register(t_cfg_b_emsfare2, t_cfg_b_emsfare2Admin)
xadmin.site.register(t_cfg_b_emsfare_country2, t_cfg_b_emsfare_country2Admin)
xadmin.site.register(t_cfg_b_logisticway, t_cfg_b_logisticwayAdmin)
xadmin.site.register(t_cfg_platform_country, t_cfg_platform_countryAdmin)
xadmin.site.register(t_cfg_b_country, t_cfg_b_countryAdmin)
xadmin.site.register(t_cfg_bracket, t_cfg_bracketAdmin)
xadmin.site.register(t_cfg_standard_large, t_cfg_standard_largeAdmin)
xadmin.site.register(t_cfg_standard_small, t_cfg_standard_smallAdmin)
xadmin.site.register(t_cfg_standard_large_small, t_cfg_standard_large_smallAdmin)
xadmin.site.register(t_cfg_category, t_cfg_categoryAdmin)
xadmin.site.register(t_cfg_category_info, t_cfg_category_infoAdmin)
xadmin.site.register(t_cfg_extra_fee, t_cfg_extra_feeAdmin)

xadmin.site.register(amzon_india_price_config, amzon_india_price_config_Admin)
xadmin.site.register(t_wish_keywords, t_wish_keywords_Admin)
xadmin.site.register(t_config_wishapi_product_analyse_info_latest_add, t_config_wishapi_product_analyse_info_latest_add_Admin)
xadmin.site.register(t_config_store_ebay, t_config_store_ebay_Admin)
xadmin.site.register(t_config_wishapi_product_analyse_info_xpbs, t_config_wishapi_product_analyse_info_xpbs_Admin)
xadmin.site.register(t_config_wishapi_product_analyse_info_bssp, t_config_wishapi_product_analyse_info_bssp_Admin)
xadmin.site.register(t_config_wishapi_product_analyse_info_xprx, t_config_wishapi_product_analyse_info_xprx_Admin)
xadmin.site.register(t_config_wishapi_product_analyse_info,t_config_wishapi_product_analyse_info_Admin)
xadmin.site.register(t_product_b_goods, t_product_b_goods_Admin)
xadmin.site.register(t_report_GET_SP_AUTO_TARGETING_REPORT_,t_report_GET_SP_AUTO_TARGETING_REPORT_Admin)
xadmin.site.register(t_importfile,t_importfile_Admin)
xadmin.site.register(t_store_status,t_store_status_Admin)
xadmin.site.register(t_store_weekly_statistics,t_store_weekly_statistics_Admin)
xadmin.site.register(t_store_summary_of_sales_profit_data,t_store_summary_of_sales_profit_data_Admin)
xadmin.site.register(t_store_marketplan_execution,t_store_marketplan_execution_Admin)
xadmin.site.register(t_store_marketplan_execution_ebay,t_store_marketplan_execution_ebayAdmin)
xadmin.site.register(t_store_marketplan_execution_amazon,t_store_marketplan_execution_amazonAdmin)
xadmin.site.register(t_store_marketplan_execution_aliexpress,t_store_marketplan_execution_aliexpressAdmin)
xadmin.site.register(t_store_execution_aliexpress,t_store_execution_aliexpressAdmin)
xadmin.site.register(t_store_marketplan_execution_joom,t_store_marketplan_execution_joomAdmin)
xadmin.site.register(t_store_configuration_file,t_store_configuration_file_Admin)
xadmin.site.register(t_online_info_wish,t_online_info_wish_Admin)
xadmin.site.register(t_report_potential_wish,t_report_potential_wish_Admin)
xadmin.site.register(t_report_orders1days,t_report_orders1days_Admin)
xadmin.site.register(t_api_schedule_ed,t_api_schedule_ed_Admin)
xadmin.site.register(t_api_schedule_ing,t_api_schedule_ing_Admin)
xadmin.site.register(t_catainfo_ebay,t_catainfo_ebay_Admin)
xadmin.site.register(v_hotsale_proinfo_ebay_cloth,v_hotsale_proinfo_ebay_cloth_Admin)
xadmin.site.register(t_config_apiurl_asin,t_config_apiurl_asinAdmin)
xadmin.site.register(t_config_apiurl_asin_cf,t_config_apiurl_asin_cf_Admin)
xadmin.site.register(t_config_apiurl_asin_operation,t_config_apiurl_asin_operation_Admin)
xadmin.site.register(t_config_apiurl_asin_clothing,t_config_apiurl_asin_clothing_Admin)
xadmin.site.register(t_config_apiurl_asin_kf,t_config_apiurl_asin_kfAdmin)
xadmin.site.register(t_config_mstsc,t_config_mstsc_Admin)
xadmin.site.register(t_config_mstscfinance,t_config_mstscfinance_Admin)
xadmin.site.register(t_config_online_amazon,t_config_online_amazon_Admin)
xadmin.site.register(t_help,t_helpAdmin)
xadmin.site.register(t_history,t_history_Admin)

xadmin.site.register(t_hotsale_proinfo_ebay,t_hotsale_proinfo_ebay_Admin)
xadmin.site.register(t_online_info,t_online_info_Admin)
xadmin.site.register(t_order_ListOrders,t_order_ListOrders_Admin)
xadmin.site.register(t_product_art_ed,t_product_art_ed_Admin)
xadmin.site.register(t_product_art_ing,t_product_art_ing_Admin)
xadmin.site.register(t_product_art_pre_ed,t_product_art_pre_ed_Admin)
xadmin.site.register(t_product_build_ing,t_product_build_ing_Admin)
xadmin.site.register(t_product_depart_get,t_product_depart_get_Admin)
xadmin.site.register(t_product_develop_ed,t_product_develop_ed_Admin)
xadmin.site.register(t_product_develop_ing,t_product_develop_ing_Admin)
xadmin.site.register(t_product_develop_audit,t_product_develop_audit_Admin)
xadmin.site.register(t_product_enquiry_ing,t_product_enquiry_ing_Admin)
xadmin.site.register(t_product_enter_ed,t_product_enter_ed_Admin)
xadmin.site.register(t_product_enter_ed_user,t_product_enter_ed_user_Admin)
xadmin.site.register(t_product_depart_get_user,t_product_depart_get_user_Admin)
xadmin.site.register(t_product_enter_ing,t_product_enter_ing_Admin)
xadmin.site.register(t_product_information_modify,t_product_information_modify_Admin)
xadmin.site.register(t_product_modify_entry,t_product_modify_entry_Admin)
xadmin.site.register(t_product_modify_review,t_product_modify_review_Admin)
xadmin.site.register(t_product_modify_ed,t_product_modify_ed_Admin)
xadmin.site.register(t_product_oplog,t_product_oplog_Admin)
xadmin.site.register(t_product_photograph,t_product_photograph_Admin)
xadmin.site.register(t_product_platform_group,t_product_platform_group_Admin)
xadmin.site.register(t_product_questions,t_product_questions_Adimin)
xadmin.site.register(t_product_questions_out,t_product_questions_outAdmin)
xadmin.site.register(t_product_recycle,t_product_recycle_Admin)
xadmin.site.register(t_product_repeats,t_product_repeats_Admin)
xadmin.site.register(t_product_survey_ed,t_product_survey_ed_Admin)
xadmin.site.register(t_product_survey_history,t_product_survey_history_Admin)
xadmin.site.register(t_product_survey_ing,t_product_survey_ing_Admin)
xadmin.site.register(t_product_unique_ed,t_product_unique_ed_Admin)
xadmin.site.register(t_product_wait_enquiry,t_product_wait_enquiry_Admin)
xadmin.site.register(t_report_GET_SP_MEGA_REPORT_,t_report_GET_SP_MEGA_REPORT_Admin)
xadmin.site.register(t_report_week,t_report_week_Admin)
xadmin.site.register(t_report_week_dykf,t_report_week_dykf_Admin)
xadmin.site.register(t_report_week_jzl,t_report_week_jzl_Admin)
xadmin.site.register(t_sys_department,t_sys_department_Admin)
xadmin.site.register(t_sys_department_staff,t_sys_department_staff_Admin)
xadmin.site.register(t_sys_param, t_sys_param_Admin)
xadmin.site.register(t_sys_shopdef,t_sys_shopdef_Admin)
xadmin.site.register(t_tort_aliexpress,t_tort_aliexpress_Admin)
xadmin.site.register(t_yearweek,t_yearweek_Admin)
xadmin.site.register(v_product_allsku,v_product_allsku_Admin)
xadmin.site.register(v_product_all_image,v_product_all_image_Admin)
xadmin.site.register(v_product_enter_ed,v_product_enter_ed_Admin)
xadmin.site.register(t_shopsku_information_binding,t_shopsku_information_binding_Admin)
xadmin.site.register(t_download_info,t_download_info_Admin)
xadmin.site.register(t_distribution_product_to_store,t_distribution_product_to_store_Admin)
xadmin.site.register(t_amazon_ad_serving_status,t_amazon_ad_serving_status_Admin)
xadmin.site.register(xxxb_packinfo,b_packinfo_Admin)
xadmin.site.register(B_Supplier,b_supplier_Admin)
xadmin.site.register(t_sys_staff_auth,t_sys_staff_auth_Admin)
xadmin.site.register(t_product_mainsku_sku,t_product_mainsku_sku_Admin)
xadmin.site.register(t_order_refunded, t_order_refunded_Admin)
xadmin.site.register(t_product_ad_conversion_rate, t_product_ad_conversion_rate_Admin)
xadmin.site.register(t_online_info_amazon, t_online_info_amazon_Admin)
xadmin.site.register(t_online_info_15day_amazon, t_online_info_15day_amazon_Admin)
xadmin.site.register(t_tort_shelve_wish, t_tort_shelve_wish_Admin)
xadmin.site.register(t_config_user_buyer,t_config_user_buyerAdmin)
xadmin.site.register(t_config_user_buyer_task,t_config_user_buyer_taskAdmin)
xadmin.site.register(t_store_marketplan_execution_log,t_store_marketplan_execution_log_Admin)
xadmin.site.register(t_store_marketplan_execution_evaluate,t_store_marketplan_execution_evaluate_Admin)
xadmin.site.register_plugin(t_config_user_buyerPlugin, ModelFormAdminView)
xadmin.site.register(t_product_price_check,t_product_price_check_admin)
xadmin.site.register(t_online_shopinfo_joom2,t_online_shopinfo_joom2_Admin)
xadmin.site.register(t_online_shopinfo_joom3,t_online_shopinfo_joom3_Admin)
xadmin.site.register(t_online_amazon_fba_inventory,t_online_amazon_fba_inventory_Admin)
xadmin.site.register(t_online_info_ebay,t_online_info_ebay_Admin)
xadmin.site.register(t_report_ebay_orders1days,t_report_ebay_orders1days_Admin)
xadmin.site.register(t_report_potential_ebay,t_report_potential_ebay_Admin)
xadmin.site.register(t_online_info_wait_publish_joom2,t_online_info_wait_publish_joom2_Admin)
xadmin.site.register(t_online_info_wait_publish_joom3,t_online_info_wait_publish_joom3_Admin)
xadmin.site.register(t_online_info_wait_publish_tophatter,t_online_info_wait_publish_TophatterAdmin)
xadmin.site.register(t_online_info_wait_publish_UMKA,t_online_info_wait_publish_UMKA_Admin)
xadmin.site.register(t_online_info_wait_publish_vova,t_online_info_wait_publish_vova_Admin)
xadmin.site.register(t_online_info_wait_publish_jumia,t_online_info_wait_publish_jumia_Admin)
xadmin.site.register(t_online_info_publish_joom,t_online_info_publish_joom_Admin)
from skuapp.plugin.t_trademark_copyright_plugin import t_trademark_copyright_plugin
xadmin.site.register_plugin(t_trademark_copyright_plugin, ListAdminView)
xadmin.site.register(t_distribution_product_to_store_result,t_distribution_product_to_store_result_Admin)
xadmin.site.register(t_config_paypal_account,t_config_paypal_accountAdmin)
xadmin.site.register(t_paypal_payout_log,t_paypal_payout_logAdmin)
xadmin.site.register(t_product_upload_recommend_table,t_product_upload_recommend_table_Admin)
xadmin.site.register(t_product_price_check_recycle,t_product_price_check_recycle_Admin)
xadmin.site.register(t_goods_shelves,t_goods_shelves_Admin)
xadmin.site.register(t_tort_brands_info,t_tort_brands_infoAdmin)
xadmin.site.register(t_upload_to_wish_all_shop_product,t_upload_to_wish_all_shop_product_Admin)
xadmin.site.register(t_upload_shopname,t_upload_shopname_Admin)
xadmin.site.register(t_upload_to_wish_all_product,t_upload_to_wish_all_product_Admin)
xadmin.site.register(t_upload_statistics_table,t_upload_statistics_table_Admin)
xadmin.site.register(t_config_ip,t_config_ip_Admin)
xadmin.site.register(t_config_ip_ext,t_config_ip_ext_Admin)
xadmin.site.register(t_goods_shelves_dis_upload,t_goods_shelves_dis_upload_Admin)
xadmin.site.register(t_translation,t_translation_Admin)
xadmin.site.register(t_product_image_modify,t_product_image_modify_Admin)
xadmin.site.register(t_wish_pb,t_wish_pb_Admin)
xadmin.site.register(t_wish_honor,t_wish_honor_Admin)
xadmin.site.register(t_wish_screenshot,t_wish_screenshot_Admin)
xadmin.site.register(t_wish_pbShop,t_wish_pbShop_Admin)
xadmin.site.register(t_wish_honorShop,t_wish_honorShop_Admin)
xadmin.site.register(t_distribution_staff_statistics_table,t_distribution_staff_statistics_table_Admin)
xadmin.site.register(t_small_platform_translation,t_small_platform_translation_Admin)
xadmin.site.register(v_wish_pb,v_wish_pb_Admin)
xadmin.site.register(t_distribution_template_table,t_distribution_template_table_Admin)
xadmin.site.register(p_trade_blacklist_zip,p_trade_blacklist_zip_Admin)
xadmin.site.register(t_product_up_down,t_product_up_downAdmin)
xadmin.site.register(t_product_inventory_warnning,t_product_inventory_warnning_Admin)
xadmin.site.register(t_product_inventory_warnning_dead_ing,t_product_inventory_warnning_dead_ing_Admin)
xadmin.site.register(t_product_inventory_warnning_dead_ed,t_product_inventory_warnning_dead_ed_Admin)
xadmin.site.register(t_product_inventory_warnning_stocking,t_product_inventory_warnning_stocking_Admin)
xadmin.site.register(t_product_inventory_warnning_dead,t_product_inventory_warnning_dead_Admin)
xadmin.site.register(t_order_amazon_india,t_order_amazon_india_Admin)
xadmin.site.register(t_shop_amazon_india,t_shop_amazon_india_Admin)
xadmin.site.register(t_templet_wish_collection_box,t_templet_wish_collection_box_Admin)
xadmin.site.register(t_templet_public_wish,t_templet_public_wish_Admin)
xadmin.site.register(t_templet_wish_wait_upload,t_templet_wish_wait_upload_Admin)
xadmin.site.register(t_templet_wish_upload_result,t_templet_wish_upload_result_Admin)
xadmin.site.register(t_templet_public_wish_listing,t_templet_public_wish_listing_Admin)
xadmin.site.register(t_templet_public_ebay_listing,t_templet_public_ebay_listing_Admin)
xadmin.site.register(t_templet_public_ebay,t_templet_public_ebay_Admin)
xadmin.site.register(t_templet_ebay_collection_box,t_templet_ebay_collection_box_Admin)
xadmin.site.register(t_templet_ebay_upload_result,t_templet_ebay_upload_result_Admin)
xadmin.site.register(t_ebay_product_image_modify,t_ebay_product_image_modify_Admin)
xadmin.site.register(t_templet_ebay_wait_upload,t_templet_ebay_wait_upload_Admin)
xadmin.site.register(t_templet_public_amazon_listing,t_templet_public_amazon_listing_Admin)
xadmin.site.register(t_templet_amazon_collection_box,t_templet_amazon_collection_box_Admin)
xadmin.site.register(t_templet_amazon_wait_upload,t_templet_amazon_wait_upload_Admin)
xadmin.site.register(t_templet_amazon_upload_result,t_templet_amazon_upload_result_Admin)
xadmin.site.register(t_templet_amazon_wait_published,t_templet_amazon_wait_published_Admin)
xadmin.site.register(t_templet_aliexpress_collection_box,t_templet_aliexpress_collection_box_Admin)
xadmin.site.register(t_templet_public_aliexpress,t_templet_public_aliexpress_Admin)
xadmin.site.register(t_templet_aliexpress_wait_upload,t_templet_aliexpress_wait_upload_Admin)
xadmin.site.register(t_online_info_amazon_listing,t_online_info_amazon_listing_Admin)

xadmin.site.register(t_config_apiurl_asin_xprx,t_config_apiurl_asin_xprx_Admin)
xadmin.site.register(t_use_productsku_apply_for_shopsku,t_use_productsku_apply_for_shopsku_Admin)
xadmin.site.register(t_stocking_demand_list,t_stocking_demand_list_Admin)
xadmin.site.register(t_stocking_demand_audit,t_stocking_demand_audit_Admin)
xadmin.site.register(t_stocking_demand_passaudit,t_stocking_demand_passaudit_Admin)
xadmin.site.register(t_stocking_purchase_order,t_stocking_purchase_order_Admin)
xadmin.site.register(t_set_warehouse_storage_situation_list,t_set_warehouse_storage_situation_list_Admin)
xadmin.site.register(t_shipping_management,t_shipping_management_Admin)
xadmin.site.register(t_stocking_demand_fba,t_stocking_demand_fba_Admin)
xadmin.site.register(t_stocking_demand_fba_purchase,t_stocking_demand_fba_purchase_Admin)
xadmin.site.register(t_stocking_demand_fba_check,t_stocking_demand_fba_check_Admin)
xadmin.site.register(t_stocking_demand_fba_genbatch,t_stocking_demand_fba_genbatch_Admin)
xadmin.site.register(t_stocking_demand_fba_deliver,t_stocking_demand_fba_deliver_Admin)
xadmin.site.register(t_stocking_demand_fba_all,t_stocking_demand_fba_all_Admin)
xadmin.site.register(t_stocking_demand_fba_detail,t_stocking_demand_fba_detail_Admin)
xadmin.site.register(t_stocking_demand_fbw,t_stocking_demand_fbw_Admin)
xadmin.site.register(t_stocking_demand_fbw_deliver,t_stocking_demand_fbw_deliver_Admin)
xadmin.site.register(t_stocking_demand_fbw_demand,t_stocking_demand_fbw_demand_Admin)
xadmin.site.register(t_stocking_demand_fbw_management,t_stocking_demand_fbw_management_Admin)
xadmin.site.register(t_stocking_demand_fbw_all,t_stocking_demand_fbw_all_Admin)
xadmin.site.register(t_saler_profit_config,t_saler_profit_config_Admin)
xadmin.site.register(t_saler_profit_reportform,t_saler_profit_reportform_Admin)
xadmin.site.register(t_stocking_reject_fba,t_stocking_reject_fba_Admin)
xadmin.site.register(t_stocking_rejecting_fba,t_stocking_rejecting_fba_Admin)
xadmin.site.register(t_help_banding_sku_to_ali,t_help_banding_sku_to_ali_Admin)

xadmin.site.register(t_config_logistic,t_config_logisticAdmin)
xadmin.site.register(t_config_logistic_express,t_config_logistic_expressAdmin)
xadmin.site.register(t_online_info_logistic,t_online_info_logistic_Admin)

xadmin.site.register(t_product_suvering, t_product_survering_admin)
xadmin.site.register(t_templet_joom_collection_box,t_templet_joom_collection_box_Admin)
xadmin.site.register(t_templet_public_joom_listing,t_templet_public_joom_listing_Admin)
xadmin.site.register(t_templet_public_joom,t_templet_public_joom_Admin)
xadmin.site.register(t_templet_joom_wait_upload,t_templet_joom_wait_upload_Admin)
xadmin.site.register(t_product_wish_off_shelf,t_product_wish_off_shelf_Admin)
xadmin.site.register(t_wish_distribution_result_statistics_everyday,t_wish_distribution_result_statistics_everyday_Admin)
xadmin.site.register(t_wish_distribution_sales_statistics_everyday,t_wish_distribution_sales_statistics_everyday_Admin)
xadmin.site.register(t_wish_distribution_sales_statistics_total,t_wish_distribution_sales_statistics_total_Admin)
xadmin.site.register(w_work_report, w_work_report_Admin)
xadmin.site.register(d_work_report, d_work_report_Admin)
xadmin.site.register(t_task_trunk,t_task_trunk_Admin)
xadmin.site.register(t_task_trunk_case,t_task_trunk_case_Admin)
xadmin.site.register(t_task_details,t_task_details_Admin)
xadmin.site.register(t_task_operation_log,t_task_operation_log_Admin)
xadmin.site.register(not_clothing_salesman_registration, not_clothing_salesman_registration_Admin)
xadmin.site.register(t_product_pic_completion,t_product_pic_completion_Admin)
xadmin.site.register(t_product_photo_ing,t_product_photo_ing_Admin)
xadmin.site.register(t_product_photo_alreadly, t_product_photo_alreadly_Admin)
xadmin.site.register(paypal_tort, paypal_tort_Admin)
xadmin.site.register(t_aliexpress_refund, t_aliexpress_refund_Admin)
xadmin.site.register(t_order_out_of_stock,t_order_out_of_stock_Admin)
xadmin.site.register(t_product_quality_feedback_submit,t_product_quality_feedback_submit_Admin)
xadmin.site.register(t_product_quality_feedback_cpzy,t_product_quality_feedback_cpzy_Admin)
xadmin.site.register(t_product_quality_feedback_cgy,t_product_quality_feedback_cgy_Admin)
xadmin.site.register(t_product_quality_feedback_zjy_sh,t_product_quality_feedback_zjy_sh_Admin)
xadmin.site.register(t_product_quality_feedback_zjy,t_product_quality_feedback_zjy_Admin)
xadmin.site.register(t_product_quality_feedback_ck,t_product_quality_feedback_ck_Admin)
xadmin.site.register(t_product_quality_feedback_final_sh,t_product_quality_feedback_final_sh_Admin)
xadmin.site.register(t_product_quality_feedback_final_result,t_product_quality_feedback_final_result_Admin)
xadmin.site.register(t_templet_amazon_collection_reverse,t_templet_amazon_collection_reverse_Admin)
xadmin.site.register(t_aliexpress_service_division_analysis,t_aliexpress_service_division_analysis_Admin)
xadmin.site.register(t_aliexpress_service_division_standard,t_aliexpress_service_division_standard_Admin)
xadmin.site.register(t_aliexpress_refund_info,t_aliexpress_refund_info_Admin)
xadmin.site.register(t_report_sales_clothingsystem, t_report_sales_clothingsystem_Admin)
xadmin.site.register(t_aliexpress_refund_info_all,t_aliexpress_refund_info_all_Admin)
xadmin.site.register(t_sensitive_sku_info,t_sensitive_sku_info_Admin)
xadmin.site.register(t_sensitive_word_info,t_sensitive_word_info_Admin)
xadmin.site.register(b_goods_sku_status_change,b_goods_sku_status_change_Admin)

xadmin.site.register(t_cloth_factory_dispatch_needpurchase,t_cloth_factory_dispatch_needpurchase_Admin)
xadmin.site.register(t_cloth_factory_dispatch_noneedpurchase,t_cloth_factory_dispatch_noneedpurchase_Admin)
xadmin.site.register(t_cloth_factory_dispatch_plan,t_cloth_factory_dispatch_plan_Admin)
xadmin.site.register(t_cloth_factory_dispatch_spemanmodify,t_cloth_factory_dispatch_spemanmodify_Admin)
xadmin.site.register(t_cloth_factory_dispatch_paid_audit,t_cloth_factory_dispatch_paid_audit_Admin)
xadmin.site.register(t_cloth_factory_dispatch_no_audit,t_cloth_factory_dispatch_no_audit_Admin)
xadmin.site.register(t_cloth_factory_dispatch_audit,t_cloth_factory_dispatch_audit_Admin)
xadmin.site.register(t_cloth_factory_dispatch_commit,t_cloth_factory_dispatch_commit_Admin)
xadmin.site.register(t_cloth_factory_dispatch_confirm,t_cloth_factory_dispatch_confirm_Admin)
xadmin.site.register(t_cloth_factory_dispatch_overtobuild,t_cloth_factory_dispatch_overtobuild_Admin)
xadmin.site.register(t_cloth_factory_dispatch_close,t_cloth_factory_dispatch_close_Admin)
xadmin.site.register(t_cloth_factory_dispatch_history,t_cloth_factory_dispatch_history_Admin)
xadmin.site.register(t_cloth_factory,t_cloth_factory_Admin)
xadmin.site.register(t_cloth_factory_dispatch_paiding,t_cloth_factory_dispatch_paiding_Admin)
xadmin.site.register(t_templet_amazon_upload_result_lose_pic,t_templet_amazon_upload_result_lose_pic_Admin)
xadmin.site.register(t_templet_amazon_upload_fail,t_templet_amazon_upload_fail_Admin)
xadmin.site.register(t_templet_amazon_recycle_bin,t_templet_amazon_recycle_bin_Admin)
xadmin.site.register(t_product_build_FBA,t_product_build_FBA_Admin)
xadmin.site.register(t_product_enter_FBA,t_product_enter_FBA_Admin)
xadmin.site.register(t_weekly_collar_and_publication_statistics_table,t_weekly_collar_and_publication_statistics_table_Admin)
xadmin.site.register(t_combination_sku_log,t_combination_sku_log_Admin)
xadmin.site.register(t_templet_public_wish_review,t_templet_public_wish_review_Admin)
xadmin.site.register(t_tort_info, t_tort_info_Admin)
xadmin.site.register(t_tort_info_audit, t_tort_info_audit_Admin)
xadmin.site.register(t_tort_info_result, t_tort_info_result_Admin)
xadmin.site.register(t_tort_info_result_no, t_tort_info_result_no_Admin)
xadmin.site.register(t_tort_info_result_common, t_tort_info_result_common_Admin)
xadmin.site.register(t_tort_info_cancel, t_tort_info_cancel_Admin)
xadmin.site.register(t_tort_info_cancel_audit, t_tort_info_cancel_audit_Admin)
xadmin.site.register(t_tort_info_image, t_tort_info_image_Admin)
xadmin.site.register(t_tort_info_sync, t_tort_info_sync_Admin)
xadmin.site.register(t_tort_info_query, t_tort_info_query_Admin)
xadmin.site.register(t_wish_pb_campaignproductstats, t_wish_pb_campaignproductstats_Admin)
xadmin.site.register(t_wish_pb_productdailystats, t_wish_pb_productdailystats_Admin)
xadmin.site.register(t_wish_pb_report, t_wish_pb_report_Admin)
xadmin.site.register_plugin(importfile_marketing_Plugin,ListAdminView)
xadmin.site.register(t_marketing_review_trace,t_marketing_review_trace_Admin)
xadmin.site.register(wish_processed_order, wish_processed_order_Admin)
xadmin.site.register(wish_notification, wish_notification_Admin)
xadmin.site.register(t_work_flow_of_plate_house,t_work_flow_of_plate_house_admin)
xadmin.site.register(t_work_battledore, t_work_battledore_admin)
xadmin.site.register(t_templet_wish_upload_review,t_templet_wish_upload_review_Admin)
xadmin.site.register(t_stocking_check_report,t_stocking_check_report_Admin)
xadmin.site.register(t_eBay_TrendKw, t_eBay_TrendKw_Admin)
xadmin.site.register(t_product_price_suggest, t_product_price_suggest_Admin)
xadmin.site.register(t_report_sales_daily_byPlatform,t_report_sales_daily_byPlatform_Admin)
xadmin.site.register(t_report_sales_daily_byShopName,t_report_sales_daily_byShopName_Admin)
xadmin.site.register(t_report_sales_daily_byProductID,t_report_sales_daily_byProductID_Admin)
xadmin.site.register(t_report_sales_daily,t_report_sales_daily_Admin)
xadmin.site.register(t_report_sales_daily_byMainSKU,t_report_sales_daily_byMainSKU_Admin)
xadmin.site.register(t_report_sales_daily_bySKU,t_report_sales_daily_bySKU_Admin)
xadmin.site.register(t_wish_activerate, t_wish_activerate_Admin)
xadmin.site.register(b_goods_sales_count, b_goods_sales_count_Admin)
xadmin.site.register(t_amazon_pro_comment, t_amazon_pro_comment_Admin)
xadmin.site.register(t_config_mstscfinance_user,t_config_mstscfinance_user_Admin)
xadmin.site.register(t_amazon_cpc_ad, t_amazon_cpc_ad_Admin)
xadmin.site.register(t_wish_pb_finance_rep, t_wish_pb_finance_rep_Admin)
xadmin.site.register(t_wish_pb_earnings_show, t_wish_pb_earnings_show_Admin)
from skuapp.plugin.amazon_advertising_report_Plugin import amazon_advertising_report_Plugin
xadmin.site.register_plugin(amazon_advertising_report_Plugin,ListAdminView)
from skuapp.plugin.amazon_advertising_shop_report_Plugin import amazon_advertising_shop_report_Plugin
xadmin.site.register_plugin(amazon_advertising_shop_report_Plugin,ListAdminView)
from skuapp.plugin.amazon_shop_search_Plugin import amazon_shop_search_Plugin
xadmin.site.register_plugin(amazon_shop_search_Plugin,ListAdminView)
xadmin.site.register_plugin(t_product_suringPlugin,ListAdminView)
xadmin.site.register(t_config_shop_alias, t_config_shop_alias_Admin)
xadmin.site.register_plugin(t_product_upc_id_amazon_Plugin, ModelFormAdminView)
xadmin.site.register(t_trademark, t_trademark_Admin)
xadmin.site.register(t_copyright, t_copyright_Admin)
xadmin.site.register(t_patents, t_patents_Admin)
xadmin.site.register_plugin(sales_count_Plugin,ListAdminView)

xadmin.site.register_plugin(Large_Small_Plugin,ModelFormAdminView)
xadmin.site.register_plugin(t_wish_keywords_downloads_plugin, BaseAdminView)
#xadmin.site.register_plugin(CategoryPlugin, ModelFormAdminView)

xadmin.site.register_plugin(reverse_amazon_collection_box_plugin, BaseAdminView)
xadmin.site.register_plugin(t_config_store_ebay_regetpes_plugin, BaseAdminView)
xadmin.site.register_plugin(show_picturePlugin, ModelFormAdminView)
xadmin.site.register_plugin(t_helpPlugin, BaseAdminView)
xadmin.site.register_plugin(search_cata_navPlugin, BaseAdminView)
xadmin.site.register_plugin(t_product_build_ingPlugin, ModelFormAdminView)
xadmin.site.register_plugin(t_product_downloadxls_Plugin, ListAdminView)
xadmin.site.register_plugin(t_product_enter_edPlugin, ModelFormAdminView)
xadmin.site.register_plugin(timePlugin,BaseAdminView)
xadmin.site.register_plugin(top_reportPlugin,BaseAdminView)
xadmin.site.register_plugin(top_top_navbarPlugin, BaseAdminView)
xadmin.site.register_plugin(t_product_up_downPlugin, ModelFormAdminView)
xadmin.site.register_plugin(black_list_Plugin, ListAdminView)
xadmin.site.register_plugin(cexport_refund_to_oss_Plugin, ListAdminView)
xadmin.site.register_plugin(task_trunkPlugin, ListAdminView)
xadmin.site.register_plugin(t_task_trunkPlugin, ModelFormAdminView)
xadmin.site.register_plugin(t_product_udPlugin, ListAdminView)
xadmin.site.register_plugin(variation_item_amazon_Plugin, ModelFormAdminView)
xadmin.site.register_plugin(t_wish_activerate_Plugin, ListAdminView)
xadmin.site.register_plugin(t_wishpb_syncdata_Plugin, ListAdminView)
   

xadmin.site.register_plugin(t_amazon_comment_Plugin, ListAdminView)
xadmin.site.register_plugin(t_amazon_cpc_ad_plugin, ListAdminView)
xadmin.site.register_plugin(t_wish_pb_productdailystats_Plugin, ListAdminView)
xadmin.site.register_plugin(t_wish_pb_report_Plugin, ListAdminView)
xadmin.site.register_plugin(button_Plugin, ModelFormAdminView)
xadmin.site.register_plugin(search_sku_Plugin, ModelFormAdminView)
xadmin.site.register_plugin(t_paypal_payout_logPlugin, ModelFormAdminView)
xadmin.site.register_plugin(wish_search_barPlugin, ListAdminView)
xadmin.site.register_plugin(import_joomPlugin, BaseAdminView)
xadmin.site.register_plugin(publish_joomPlugin, BaseAdminView)
xadmin.site.register_plugin(publish_joom3Plugin, BaseAdminView)
xadmin.site.register_plugin(publish_tophatterPlugin, BaseAdminView)
xadmin.site.register_plugin(publish_UMKAPlugin, BaseAdminView)
xadmin.site.register_plugin(publish_vovaPlugin, BaseAdminView)
xadmin.site.register_plugin(publish_jumiaPlugin, BaseAdminView)
xadmin.site.register_plugin(t_product_enter_ed_classificationPlugin, ListAdminView)
xadmin.site.register_plugin(select_checkboxPlugin, ListAdminView)
xadmin.site.register_plugin(t_upload_statistics_table_Plugin, ListAdminView)
xadmin.site.register_plugin(t_goods_shelves_search_Plugin, ListAdminView)
xadmin.site.register_plugin(amazon_india_FBA_sku_Plugin, ListAdminView)
xadmin.site.register_plugin(search_box1Plugin, ListAdminView)
xadmin.site.register_plugin(search_box2Plugin, ListAdminView)
xadmin.site.register_plugin(price_check_categoryPlugin, ListAdminView)
xadmin.site.register_plugin(plateform_distribution_navigation_Plugin, ListAdminView)
xadmin.site.register_plugin(wish_distribution_listing_sku_query_Plugin, ListAdminView)
xadmin.site.register_plugin(t_prompt_developPlugin, ModelFormAdminView)
xadmin.site.register_plugin(site_left_menu_Plugin,BaseAdminView)
xadmin.site.register_plugin(amazon_site_left_menu_tree_Plugin,BaseAdminView)
xadmin.site.register_plugin(importfile_stocking_demand_plugin, ListAdminView)
xadmin.site.register_plugin(importfile_paypal_tortPlugin, ListAdminView)
xadmin.site.register_plugin(importfile_aliexpress_refundPlugin, ListAdminView)
xadmin.site.register_plugin(purchase_order_button_plugin, ListAdminView)
xadmin.site.register_plugin(kc_get_money, BaseAdminView)
xadmin.site.register_plugin(kc_jump_status, BaseAdminView)
xadmin.site.register_plugin(jump_temp_some_page, ListAdminView)
xadmin.site.register_plugin(wish_distribution_order_sales__chart_Plugin, ListAdminView)
xadmin.site.register_plugin(wish_distribution_order_sales_total__chart_Plugin, ListAdminView)
xadmin.site.register_plugin(t_report_sales_clothingsystem_chart_Plugin, ListAdminView)
xadmin.site.register(clothing_salesman_registration, clothing_salesman_registrationAdmin)
xadmin.site.register(Result_table, Result_tableAdmin)
xadmin.site.register(Newshop_feedback, Newshop_feedbackAdmin)
xadmin.site.register(GoodsResponse_table, GoodsResponse_tableAdmin)
xadmin.site.register(BugerResponse_table, BugerResponse_tableAdmin)
xadmin.site.register(WarehouseSpecialist_table, WarehouseSpecialist_tableAdmin)
xadmin.site.register(GoodsCheck_table, GoodsCheck_tableAdmin)
xadmin.site.register(t_aliexpress_compare_price,t_aliexpress_compare_price_Admin)
xadmin.site.register_plugin(shopsku_apply_plugin,ModelFormAdminView)
xadmin.site.register_plugin(salesshowlisting_Plugin, ListAdminView)
xadmin.site.register_plugin(t_amazon_comment_analy_Plugin, ListAdminView)
xadmin.site.register_plugin(sales_show_byplatformnamePlugin, ListAdminView)
xadmin.site.register_plugin(sales_show_bytypePlugin, ListAdminView)
xadmin.site.register_plugin(t_product_quality_feedback_Plugin, ListAdminView)

from ebayapp.plugin.search_ebay_listing_plugin2 import search_ebay_listing_plugin2
xadmin.site.register_plugin(search_ebay_listing_plugin2, ListAdminView)
from ebayapp.plugin.ebay_pl_plugin import ebay_pl_plugin
xadmin.site.register_plugin(ebay_pl_plugin, ListAdminView)
from reportapp.plugin.t_saler_profit_report_menu_Plugin import t_saler_profit_report_menu_Plugin
xadmin.site.register_plugin(t_saler_profit_report_menu_Plugin, ListAdminView)

xadmin.site.register_plugin(search_boxPlugin, ListAdminView)
xadmin.site.register_plugin(kc_refresh_time,ListAdminView)
xadmin.site.register_plugin(Three_Grade_Classification_Of_Clothing_Plugin,ModelFormAdminView)
xadmin.site.register_plugin(importfile_aliexpress_service_analysis, ListAdminView)
xadmin.site.register_plugin(tort_Plugin,ListAdminView)
xadmin.site.register_plugin(t_tort_wishPlugin,ListAdminView)
xadmin.site.register_plugin(t_aliexpress_refund_infoPlugin,ModelFormAdminView)
xadmin.site.register_plugin(t_cloth_factory_Plugin,BaseAdminView)
xadmin.site.register_plugin(t_aliexpress_compare_pricePlugin,ListAdminView)
xadmin.site.register_plugin(t_product_build_FBAPlugin,ModelFormAdminView)
xadmin.site.register(t_product_saler_requirement_photograph,t_product_saler_requirement_photograph_Admin)
xadmin.site.register_plugin(t_weekly_collar_and_pub_plugin,ListAdminView)
xadmin.site.register(t_aliexpress_distribution_shop,t_aliexpress_distribution_shop_Admin)
xadmin.site.register_plugin(show_per_page_Plugin,ListAdminView)
xadmin.site.register_plugin(repeat_sku_Plugin,ModelFormAdminView)
xadmin.site.register_plugin(t_config_Search_Plugin,BaseAdminView)
xadmin.site.register_plugin(Main_SKU_countplugin,ListAdminView)
xadmin.site.register_plugin(t_tort_info_tree_menu_Plugin, ListAdminView)
xadmin.site.register_plugin(t_wish_pb_left_menu_Plugin, ListAdminView)
xadmin.site.register_plugin(t_wish_pb_earnings_Plugin, ListAdminView)
xadmin.site.register_plugin(t_wishpb_keywords_Plugin, ModelFormAdminView)
from plugin.wish_site_left_Plugin import wish_site_left_Plugin
xadmin.site.register_plugin(wish_site_left_Plugin,ListAdminView)
from plugin.wish_site_left_noti_Plugin import wish_site_left_noti_Plugin
xadmin.site.register_plugin(wish_site_left_noti_Plugin,ListAdminView)
from skuapp.table.wish_ticket import wish_ticket
from skuapp.modelsadminx.wish_ticket_Admin import wish_ticket_Admin
xadmin.site.register(wish_ticket, wish_ticket_Admin)
from skuapp.plugin.wish_ticket_Plugin import wish_ticket_Plugin
xadmin.site.register_plugin(wish_ticket_Plugin,ListAdminView)
from skuapp.plugin.wish_site_left_ti_Plugin import wish_site_left_ti_Plugin
xadmin.site.register_plugin(wish_site_left_ti_Plugin,ListAdminView)
from skuapp.plugin.wish_in_Plugin import wish_in_Plugin
xadmin.site.register_plugin(wish_in_Plugin,ListAdminView)
from skuapp.plugin.wish_site_left_in_Plugin import wish_site_left_in_Plugin
xadmin.site.register_plugin(wish_site_left_in_Plugin,ListAdminView)
from skuapp.table.wish_infractions import wish_infractions
from skuapp.modelsadminx.wish_infractions_Admin import wish_infractions_Admin
xadmin.site.register(wish_infractions, wish_infractions_Admin)
xadmin.site.register(t_supply_chain_production_basic,t_supply_chain_production_basic_Admin)
from skuapp.plugin.t_supply_chain_production_basic_plugin import t_supply_chain_production_basic_plugin
xadmin.site.register_plugin(t_supply_chain_production_basic_plugin,ListAdminView)
xadmin.site.register_plugin(t_product_information_modify_Plugin,ListAdminView)
xadmin.site.register_plugin(amazon_result_copy_select_sitePlugin, ListAdminView)
xadmin.site.register(t_joom_refund,t_joom_refund_Admin)
xadmin.site.register_plugin(joom_refundPlugin,ListAdminView)

# 克重审核模块
from skuapp.table.t_sku_weight_examine import t_sku_weight_examine
from skuapp.modelsadminx.t_sku_weight_examine_admin import t_sku_weight_examine_admin
xadmin.site.register(t_sku_weight_examine, t_sku_weight_examine_admin)


# 产品定做落地跟踪表
from skuapp.table.t_progress_tracking_of_product_customization_table import t_progress_tracking_of_product_customization_table
from skuapp.modelsadminx.t_progress_tracking_of_product_customization_table_Admin import t_progress_tracking_of_product_customization_table_Admin
xadmin.site.register(t_progress_tracking_of_product_customization_table, t_progress_tracking_of_product_customization_table_Admin)

# 产品定做落地跟踪表------插件
from skuapp.plugin.product_customization_plugin import product_customization_plugin
xadmin.site.register_plugin(product_customization_plugin,ListAdminView)

# 产品定做落地跟踪表------已完成SKU库存查询
from skuapp.table.v_product_customization_show import v_product_customization_show
from skuapp.modelsadminx.v_product_customization_show_Admin import v_product_customization_show_Admin
xadmin.site.register(v_product_customization_show, v_product_customization_show_Admin)

from skuapp.table.t_amazon_shopsku_modify import t_amazon_shopsku_modify
from skuapp.modelsadminx.t_amazon_shopsku_modify_Admin import t_amazon_shopsku_modify_Admin
from skuapp.table.t_config_wishapi_product_analyse_info_original_trans import t_config_wishapi_product_analyse_info_original_trans
from skuapp.modelsadminx.t_config_wishapi_product_analyse_info_original_trans_Admin import t_config_wishapi_product_analyse_info_original_trans_Admin
xadmin.site.register(t_amazon_shopsku_modify, t_amazon_shopsku_modify_Admin)
xadmin.site.register(t_config_wishapi_product_analyse_info_original_trans,t_config_wishapi_product_analyse_info_original_trans_Admin)
from skuapp.table.t_amazon_removal_order_detail import t_amazon_removal_order_detail
from skuapp.modelsadminx.t_amazon_removal_order_detail_Admin import t_amazon_removal_order_detail_Admin
xadmin.site.register(t_amazon_removal_order_detail, t_amazon_removal_order_detail_Admin)

from skuapp.table.t_amazon_all_orders_data import t_amazon_all_orders_data
from skuapp.modelsadminx.t_amazon_all_orders_data_Admin import t_amazon_all_orders_data_Admin
xadmin.site.register(t_amazon_all_orders_data, t_amazon_all_orders_data_Admin)

from skuapp.table.t_template_amazon_advertising_report import t_template_amazon_advertising_report
from skuapp.table.t_template_amazon_business_report import t_template_amazon_business_report
from skuapp.modelsadminx.t_template_amazon_advertising_report_Admin import t_template_amazon_advertising_report_Admin
from skuapp.modelsadminx.t_template_amazon_business_report_Admin import t_template_amazon_business_report_Admin
xadmin.site.register(t_template_amazon_advertising_report,t_template_amazon_advertising_report_Admin)
xadmin.site.register(t_template_amazon_business_report,t_template_amazon_business_report_Admin)

from skuapp.table.t_template_amazon_advertising_business_report import t_template_amazon_advertising_business_report
from skuapp.modelsadminx.t_template_amazon_advertising_business_report_Admin import t_template_amazon_advertising_business_report_Admin
xadmin.site.register(t_template_amazon_advertising_business_report,t_template_amazon_advertising_business_report_Admin)

from skuapp.table.t_template_amazon_advertising_business_count_report import t_template_amazon_advertising_business_count_report
from skuapp.modelsadminx.t_template_amazon_advertising_business_count_report_Admin import t_template_amazon_advertising_business_count_report_Admin
xadmin.site.register(t_template_amazon_advertising_business_count_report,t_template_amazon_advertising_business_count_report_Admin)
from skuapp.table.t_template_amazon_advertising_business_daily_report import t_template_amazon_advertising_business_daily_report
from skuapp.modelsadminx.t_template_amazon_advertising_business_daily_report_Admin import t_template_amazon_advertising_business_daily_report_Admin
xadmin.site.register(t_template_amazon_advertising_business_daily_report,t_template_amazon_advertising_business_daily_report_Admin)
from skuapp.table.t_template_amazon_advertising_business_count_shop_report import t_template_amazon_advertising_business_count_shop_report
from skuapp.modelsadminx.t_template_amazon_advertising_business_count_shop_report_Admin import t_template_amazon_advertising_business_count_shop_report_Admin
xadmin.site.register(t_template_amazon_advertising_business_count_shop_report,t_template_amazon_advertising_business_count_shop_report_Admin)
from skuapp.table.t_template_amazon_advertising_keywords_report import t_template_amazon_advertising_keywords_report
from skuapp.modelsadminx.t_template_amazon_advertising_keywords_report_Admin import t_template_amazon_advertising_keywords_report_Admin
xadmin.site.register(t_template_amazon_advertising_keywords_report,t_template_amazon_advertising_keywords_report_Admin)

from skuapp.plugin.site_left_menu_tree_amazon_advertising_Plugin import site_left_menu_tree_amazon_advertising_Plugin
xadmin.site.register_plugin(site_left_menu_tree_amazon_advertising_Plugin,BaseAdminView)

xadmin.site.register_plugin(b_supplier_Plugin, ListAdminView)
xadmin.site.register_plugin(b_supplier_Plugin, ModelFormAdminView)

from skuapp.plugin.saler_profit_plugin import saler_profit_plugin
xadmin.site.register_plugin(saler_profit_plugin,ListAdminView)

from skuapp.plugin.amazon_sort_bar_plugin import amazon_sort_bar_plugin
xadmin.site.register_plugin(amazon_sort_bar_plugin, BaseAdminView)

from skuapp.table.t_product_enter_ed_aliexpress import *
from skuapp.modelsadminx.t_product_enter_ed_aliexpress_Admin import *
xadmin.site.register(t_product_enter_ed_aliexpress,t_product_enter_ed_aliexpress_Admin)
from skuapp.plugin.aliexpress_left_Plugin import aliexpress_left_Plugin
xadmin.site.register_plugin(aliexpress_left_Plugin,ListAdminView)

from skuapp.table.t_config_aliexpress_pl import *
from skuapp.modelsadminx.t_config_aliexpress_pl_Admin import *
xadmin.site.register(t_config_aliexpress_pl,t_config_aliexpress_pl_Admin)


from skuapp.plugin.t_amazon_product_cost_refresh_plugin import t_amazon_product_cost_refresh_plugin
xadmin.site.register_plugin(t_amazon_product_cost_refresh_plugin, BaseAdminView)

from skuapp.table.t_amazon_product_inventory_cost import t_amazon_product_inventory_cost
from skuapp.modelsadminx.t_amazon_product_inventory_cost_Admin import t_amazon_product_inventory_cost_Admin
xadmin.site.register(t_amazon_product_inventory_cost, t_amazon_product_inventory_cost_Admin)

from skuapp.table.t_amazon_product_remove_cost import t_amazon_product_remove_cost
from skuapp.modelsadminx.t_amazon_product_remove_cost_Admin import t_amazon_product_remove_cost_Admin
xadmin.site.register(t_amazon_product_remove_cost, t_amazon_product_remove_cost_Admin)

from skuapp.table.t_amazon_product_order_pend_cost import t_amazon_product_order_pend_cost
from skuapp.modelsadminx.t_amazon_product_order_pend_cost_Admin import t_amazon_product_order_pend_cost_Admin
xadmin.site.register(t_amazon_product_order_pend_cost, t_amazon_product_order_pend_cost_Admin)

from skuapp.table.t_amazon_orders_by_receive_day_total import t_amazon_orders_by_receive_day_total
from skuapp.modelsadminx.t_amazon_orders_by_receive_day_total_Admin import t_amazon_orders_by_receive_day_total_Admin
xadmin.site.register(t_amazon_orders_by_receive_day_total, t_amazon_orders_by_receive_day_total_Admin)

from skuapp.table.t_amazon_operation_log import t_amazon_operation_log
from skuapp.modelsadminx.t_amazon_operation_log_Admin import t_amazon_operation_log_Admin
xadmin.site.register(t_amazon_operation_log, t_amazon_operation_log_Admin)

from skuapp.table.b_costprice_modify import b_costprice_modify
from skuapp.modelsadminx.b_costprice_modify_Admin import b_costprice_modify_Admin
xadmin.site.register(b_costprice_modify,b_costprice_modify_Admin)

from skuapp.plugin.t_amazon_order_type_plugin import t_amazon_order_type_plugin
xadmin.site.register_plugin(t_amazon_order_type_plugin, ListAdminView)


# 供应链进度跟踪
from skuapp.plugin.t_work_progress_tracking_plugin import t_work_progress_tracking_plugin
xadmin.site.register_plugin(t_work_progress_tracking_plugin,ListAdminView)

from skuapp.table.t_amazon_actionable_order_data import t_amazon_actionable_order_data
from skuapp.modelsadminx.t_amazon_actionable_order_data_Admin import t_amazon_actionable_order_data_Admin
xadmin.site.register(t_amazon_actionable_order_data, t_amazon_actionable_order_data_Admin)



from skuapp.modelsadminx.t_product_enter_ed_wish_Admin import t_product_enter_ed_wish_Admin
from skuapp.table.t_product_enter_ed_wish import t_product_enter_ed_wish
from skuapp.plugin.t_product_enter_ed_wish_left_Plugin import t_product_enter_ed_wish_left_Plugin
xadmin.site.register(t_product_enter_ed_wish,t_product_enter_ed_wish_Admin)
xadmin.site.register_plugin(t_product_enter_ed_wish_left_Plugin, ListAdminView)


from skuapp.modelsadminx.t_product_enter_ed_all_Admin import t_product_enter_ed_all_Admin
from skuapp.table.t_product_enter_ed_all import t_product_enter_ed_all
from skuapp.plugin.t_product_enter_ed_all_left_Plugin import t_product_enter_ed_all_left_Plugin
xadmin.site.register(t_product_enter_ed_all,t_product_enter_ed_all_Admin)
xadmin.site.register_plugin(t_product_enter_ed_all_left_Plugin, ListAdminView)


from skuapp.modelsadminx.t_product_enter_ed_ebay_Admin import t_product_enter_ed_ebay_Admin
from skuapp.table.t_product_enter_ed_ebay import t_product_enter_ed_ebay
from skuapp.plugin.t_product_enter_ed_ebay_left_Plugin import t_product_enter_ed_ebay_left_Plugin
xadmin.site.register(t_product_enter_ed_ebay,t_product_enter_ed_ebay_Admin)
xadmin.site.register_plugin(t_product_enter_ed_ebay_left_Plugin, ListAdminView)


from skuapp.modelsadminx.t_product_enter_ed_amazon_Admin import t_product_enter_ed_amazon_Admin
from skuapp.table.t_product_enter_ed_amazon import t_product_enter_ed_amazon
from skuapp.plugin.t_product_enter_ed_amazon_left_Plugin import t_product_enter_ed_amazon_left_Plugin
xadmin.site.register(t_product_enter_ed_amazon,t_product_enter_ed_amazon_Admin)
xadmin.site.register_plugin(t_product_enter_ed_amazon_left_Plugin, ListAdminView)


from skuapp.modelsadminx.t_product_enter_ed_aliex_Admin import t_product_enter_ed_aliex_Admin
from skuapp.table.t_product_enter_ed_aliex import t_product_enter_ed_aliex
from skuapp.plugin.t_product_enter_ed_aliex_left_Plugin import t_product_enter_ed_aliex_left_Plugin
xadmin.site.register(t_product_enter_ed_aliex,t_product_enter_ed_aliex_Admin)
xadmin.site.register_plugin(t_product_enter_ed_aliex_left_Plugin, ListAdminView)

from skuapp.modelsadminx.t_amazon_shop_refund_status_Admin import t_amazon_shopsku_modify_Admin
from skuapp.table.t_amazon_shop_refund_status import t_amazon_shop_refund_status
xadmin.site.register(t_amazon_shop_refund_status, t_amazon_shopsku_modify_Admin)

from skuapp.modelsadminx.t_amazon_auto_load_Admin import t_amazon_auto_load_Admin
from skuapp.table.t_amazon_auto_load import t_amazon_auto_load
xadmin.site.register(t_amazon_auto_load, t_amazon_auto_load_Admin)

from skuapp.modelsadminx.t_amazon_finance_record_Admin import t_amazon_finance_record_Admin
from skuapp.table.t_amazon_finance_record import t_amazon_finance_record
xadmin.site.register(t_amazon_finance_record, t_amazon_finance_record_Admin)

from skuapp.modelsadminx.t_amazon_fba_inventory_age_Admin import t_amazon_fba_inventory_age_Admin
from skuapp.table.t_amazon_fba_inventory_age import t_amazon_fba_inventory_age
xadmin.site.register(t_amazon_fba_inventory_age, t_amazon_fba_inventory_age_Admin)

from skuapp.modelsadminx.t_perf_amazon_refresh_status_Admin import t_perf_amazon_refresh_status_Admin
from skuapp.table.t_perf_amazon_refresh_status import t_perf_amazon_refresh_status
xadmin.site.register(t_perf_amazon_refresh_status, t_perf_amazon_refresh_status_Admin)

from skuapp.modelsadminx.t_amazon_conversion_result_Admin import t_amazon_conversion_result_Admin
from skuapp.table.t_amazon_conversion_result import t_amazon_conversion_result
xadmin.site.register(t_amazon_conversion_result, t_amazon_conversion_result_Admin)