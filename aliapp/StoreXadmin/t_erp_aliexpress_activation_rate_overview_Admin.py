...
class t_erp_aliexpress_activation_rate_overview_Admin(object):
    activation_rate_flag=True
    search_box_flag = True
    list_display_links = ('',)
    list_display=['name','rate','product_count','activation_count','gmt_time','updatetime']
    ...
    def main(self,gmt_start,gmt_end):
        ...
        for submitter in submitter_dict:
            ...
            for category in tmp:
                ...
                t_erp_aliexpress_activation_rate.objects.update_or_create(submitter=submitter, category=category,
                                                                          gmt_time=gmt_time,
    ...
    def get_list_queryset(self):
        ...
        qs = super(t_erp_aliexpress_activation_rate_overview_Admin, self).get_list_queryset()
        ...
        name=request.GET.get('submitter','').split(',')

        searchList={'type__exact':_type,'updatetime__gte': UpdateTime_start, 'updatetime__lt': UpdateTime_end,}
        if name != ['', ]:
            searchList.update({'name__in':name,})
