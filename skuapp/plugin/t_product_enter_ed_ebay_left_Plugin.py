...
class t_product_enter_ed_ebay_left_Plugin(BaseAdminPlugin):
    t_product_enter_ed_ebay_left_flag = False

    def init_request(self, *args, **kwargs):
        return bool(self.t_product_enter_ed_ebay_left_flag)
    ...
    def block_left_navbar(self, context, nodes):
        ...
        if flag == 1:
            publish_url = '/Project/admin/skuapp/t_product_enter_ed_ebay?'
            count_all = t_product_enter_ed_aliexpress.objects.filter(
                                                                     StaffID__in=user_list).count()
            count_0 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__exact=0,
                                                                   StaffID__in=user_list).count()
            tt = datetime.datetime.today() - datetime.timedelta(days=30)
            # count_0_month = t_product_enter_ed_aliexpress.objects.filter(eBay_count__exact=0,MGTime__gte=tt).count()
            count_1 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__exact=1,
                                                                   StaffID__in=user_list).count()
            count_2 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__exact=2,
                                                                   StaffID__in=user_list).count()
            count_3 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__exact=3,
                                                                   StaffID__in=user_list).count()
            count_4 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__gt=3,
                                                                   StaffID__in=user_list).count()
            ...
            if DepartmentID == '7':
                ...
                count_4 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__gt=3,
                                                                       StaffID__in=user_list).exclude(
                    sevenbuOperation__exact=2).count()
                count_5 = t_product_enter_ed_aliexpress.objects.filter(sevenbuOperation__exact=2,StaffID__in=user_list).count()
            if DepartmentID == '8':
                count_all = t_product_enter_ed_aliexpress.objects.filter(StaffID__in=user_list).count()
                count_0 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__exact=0,
                                                                       StaffID__in=user_list).exclude(
                ...
                count_4 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__gt=3,
                                                                       StaffID__in=user_list).exclude(
                    eightbuOperation__exact=2).count()
                count_5 = t_product_enter_ed_aliexpress.objects.filter(eightbuOperation__exact=2, StaffID__in=user_list).count()
            ...
            if DepartmentID == '12':
                ...
                count_4 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__gt=3,
                                                                       StaffID__in=user_list).exclude(
                    twelvebuOperation__exact=2).count()
                count_5 = t_product_enter_ed_aliexpress.objects.filter(twelvebuOperation__exact=2,StaffID__in=user_list).count()
            if DepartmentID == '13':
                count_all = t_product_enter_ed_aliexpress.objects.filter(StaffID__in=user_list).count()
                count_0 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__exact=0,
                                                                       StaffID__in=user_list).exclude(
                ...
                count_4 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__gt=3,
                                                                       StaffID__in=user_list).exclude(
                    thirteenbuOperation__exact=2).count()
                count_5 = t_product_enter_ed_aliexpress.objects.filter(thirteenbuOperation__exact=2,StaffID__in=user_list).count()

        else:
            publish_url = '/Project/admin/skuapp/t_product_enter_ed_ebay?'
            count_all = t_product_enter_ed_aliexpress.objects.filter().count()
            count_0 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__exact=0).count()
            tt = datetime.datetime.today() - datetime.timedelta(days=30)
            # count_0_month = t_product_enter_ed_aliexpress.objects.filter(eBay_count__exact=0,MGTime__gte=tt).count()
            count_1 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__exact=1).count()
            count_2 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__exact=2).count()
            count_3 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__exact=3).count()
            count_4 = t_product_enter_ed_aliexpress.objects.filter(eBay_count__gt=3).count()
        ...
        eBay_count = self.request.GET.get('pub')
        if eBay_count == '0':
            flag = '0'
        elif eBay_count == '1':
            flag = '1'
