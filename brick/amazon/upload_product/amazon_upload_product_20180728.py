...
"""  
 ...
 @site: 
 @software: PyCharm
 @file: amazon_upload_product_20180728.py
 @time: 2018/7/28 10:21
"""  
    ...
    for i in range(len(pattern_file)):
        ...
        if time_diff_days >= file_time_diff:
            try:
                ...
                print(u"已删除文件：%s ： %s" % (file_time, pattern_file[i]))
    ...
    def update_local_db(self, auth_info):
        ...
        if auth_info['update_type'] == 'load_product':
            ...
            for sku in auth_info['product_list']:
                ...
                cursor.execute('commit;')
            cursor.close()
        elif auth_info['update_type'] in ('product_price_modify'):
                ...
                cursor.execute(sql)
                cursor.execute('commit;')
        elif auth_info['update_type'] in ('product_image_modify'):
...
class Server():
    ...
    def callback(self, ch, method, properties, body):
        ...
        while True:
            ...
            if auth_info.get('update_type') and auth_info['update_type'] in ('load_product', 'unload_product', 'product_info_modify', 'product_price_modify', 'product_image_modify'):
