        ...
        while count < 5:
            ...
            try:
                ...
                if report_processing_status == '_DONE_':
                    ...
                    if self.auth_info['update_type'] == 'refresh_shop_all':
                        for i, val in enumerate(report_result_datas):
                            if i == 0:
                                continue
            ...
            if self.auth_info['update_type'] in ('refresh_shop_all', 'refresh_shop_increment'):
...
def deal_and_response_request(ch, method, props, body):
    ...
    if local_ip == auth_info['ShopIP']:
        ...
        if auth_info['update_type'] in ('refresh_shop_all', 'refresh_shop_increment', 'refresh_product'):
            # 全量刷新时先更新中间表，中间表更新完成后一次性更新实际业务表（此时保留之前的图片和主体、变体关系）
            if auth_info['update_type'] == 'refresh_shop_all':
            ...
            if auth_info['update_type'] in ('refresh_shop_all', 'refresh_shop_increment'):
                get_product_info_obj.get_parent_asin_and_image()
            else:
                get_product_info_obj.get_parent_asin_and_image(auth_info['product_list'])

        elif auth_info['update_type'] in ('load_product', 'unload_product'):
