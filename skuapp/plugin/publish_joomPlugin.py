...
class publish_joomPlugin(BaseAdminPlugin):
    pp_flag2 = False

    def init_request(self, *args, **kwargs):
        return bool(self.pp_flag2)
    ...
    def block_search_cata_nav(self, context, nodes):
        ...
        elif 'status=Destiny' in sourceURL:
            flag = 20 
        elif 'status=Grace' in sourceURL:
            flag = 21 
        elif 'status=Cheirsh' in sourceURL:
            flag = 22 
        elif 'status=Fire' in sourceURL:
            flag = 23   
        elif 'status=Cosy' in sourceURL:
            flag = 24           
        ...
        countsdict={}
        lm=[]
        oo = t_online_info_wait_publish_joom2.objects.values_list('FL',flat=True)
        ...
        for i in oo:
            ...
            if i in lm:
                ...
                if i is not None and i != '':
                    counts = t_online_info_wait_publish_joom2.objects.filter(FL='%s'%i.encode('utf-8')).count()
        ...
        nodes.append(loader.render_to_string('publish_joom.html',{'url':newurl,'flag':flag,'countsdict':countsdict}))
