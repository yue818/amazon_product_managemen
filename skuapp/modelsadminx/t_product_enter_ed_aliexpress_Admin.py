...
class t_product_enter_ed_aliexpress_Admin(t_product_Admin):
    ...
    def get_list_queryset(self, ):
        ...
        import MySQLdb
        server = '192.168.105.111'  # 测试数据库服务器IP
        user = 'root'  # 用户名
        password = 'root123'  # 密码
        # database = 'hq'  # 数据库名称

        # conn_mysql = connection
        conn_mysql = MySQLdb.connect(server, user, password, charset='utf8')
        ...
        try:
            ...
            messages.error(request, DepartmentID)
        except:
            pass
        if not  DepartmentID:
            DepartmentID = 1
        ...
        qs = super(t_product_enter_ed_aliexpress_Admin, self).get_list_queryset()
