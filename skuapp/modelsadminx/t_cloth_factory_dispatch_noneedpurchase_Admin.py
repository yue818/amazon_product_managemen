...
"""
 ...
 @site:
 @software: PyCharm
 @file: t_cloth_factory_dispatch_noneedpurchase_Admin.py
 @time: 2018/4/28 8:53
"""
...
class t_cloth_factory_dispatch_noneedpurchase_Admin(object):
    ...
    def to_back(self, request, objs):
        try:
            if request.user.is_superuser:
                ...
                messages.info(request, '非超级用员不能转采购.')
        except Exception, ex:
            messages.info(request, "转采购报错:%s" % (str(ex)))
    to_back.short_description = u'转待采购'

    def get_list_queryset(self):
        request = self.request
        qs = super(t_cloth_factory_dispatch_noneedpurchase_Admin, self).get_list_queryset()
        ...
        if request.user.is_superuser or request.user.id in userID:
            return qs.filter(currentState = 32)
        buyer = request.user.first_name
        return qs.filter(currentState = 32,buyer=buyer)
