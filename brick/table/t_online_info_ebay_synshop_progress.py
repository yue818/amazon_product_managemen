...
class t_online_info_ebay_synshop_progress():
    ...
    def getProgress(self,uuid):
        ...
        try:
            ...
            sql = "select status from t_online_info_ebay_synshop_progress where uuid = %s order by  updatetime desc limit 1"
    ...
    def insert_into_Progress(self, auth_info, status):
        ...
        try:
            cursor = self.db_conn.cursor()
            sql = "insert into t_online_info_ebay_synshop_progress(uuid, ip, shopname, status, syntype) values ('%s', '%s', '%s', '%s', '%s')" \
