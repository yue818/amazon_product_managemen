...
def get_param(parameters):
    ...
    for (k, v) in sorted(parameters.items()):
        param_str += "&" + urllib.quote(k, safe='') + "=" + urllib.quote(v.encode("gbk"), safe='')
...
if __name__ == '__main__':
    ...
    areas=['cn-qingdao','cn-beijing','cn-zhangjiakou','cn-hangzhou','cn-shanghai','cn-shenzhen','cn-hongkong','ap-southeast-1','ap-southeast-2','ap-northeast-1','us-west-1','us-east-1','eu-central-1','me-east-1']
    for area in areas:
        #print ("%s \n ")%area
        for i in range(1,5):
            ...
            if content <>'':
                ...
                for lis in contentList:
                    ...
                    #if len(ipAddress)<1:
                        ipAddress=lis["EipAddress"]["IpAddress"]
                        print area
                    else:
                        ipAddress=lis["PublicIpAddress"]["IpAddress"][0]
                    regionId=lis['RegionId']
                    instanceId=lis['InstanceId']
                    #print ("%s\t%s\t%s\t%s\t%s\t%s\t%s\n"%(ipAddress,ipAddress,type(ipAddress),regionId,type(regionId),instanceId,type(instanceId)))
                    cnxn = MySQLdb.connect('hequskuapp.mysql.rds.aliyuncs.com','by15161458383','K120Esc1','hq_db' )
                    cursor =cnxn.cursor();
                    sql = "update t_config_mstsc set  RegionId='%s',InstanceId='%s',CloudName='aliyun' where ip='%s'"%(regionId,instanceId,ipAddress)
