...
class t_tort_info_result_no_Admin(object):
    ...
    def to_reject_audit(self, request, objs):
        ...
        cnt = 0
        for obj in objs:
            t_tort_info_audit_obj = t_tort_info_result_no()
    ...
    list_display = ('ID', 'show_ProductPicUrl', 'StaffID', 'UpdateTime', 'Site',
                    'MainSKU', 'ProductTitle', 'Trademark','Intellectual', 'SourceUrl','IPForbiddenSite','IPRange',
                    'show_AttachmentUrls', 'Suggestion', 'SalerName2', 'Purchaser','ProductID'
                    )
    ...
    def save_models(self):
        ...
        post = request.POST
        post['_redirect'] = '/Project/admin/skuapp/t_tort_info_result_no/?Step=%s'%(Step,)

    def get_list_queryset(self):
        request = self.request
        qs = super(t_tort_info_result_no_Admin, self).get_list_queryset()
