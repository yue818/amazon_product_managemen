...
action_checkbox.short_description = mark_safe(
    '<input type="checkbox" name="action-toggle"onclick="swapCheck();%s"/>'%("$(document).ready(function($){$('.results input.action-select').actions();});"))
...
class ActionPlugin(BaseAdminPlugin):
    ...
    def post_response(self, response, *args, **kwargs):
        ...
        if self.actions and 'action' in request.POST:
            ...
            # av.message_user('---------------%s' % (request.POST,))
            # av.message_user('actions===============%s' % (self.actions,))
            # av.message_user('action---------------%s' % (action,))
        ...
        # else:
        #     av.message_user('===============%s'%(self.actions,))
        #     av.message_user('---------------%s'%(request.POST,))
        #     av.message_user('av---------------%s'%(av,))
        return response
    ...
    def get_action_choices(self):
        ...
        for ac, name, description, icon in self.actions.itervalues():
            if self.opts.model_name in ['t_online_info_amazon_listing','t_erp_aliexpress_online_info','t_online_info_ebay_listing']:
                permname = '{}.Can_{}_{}'.format(self.opts.app_label, self.opts.model_name, name)
                if not self.request.user.is_superuser and not self.request.user.has_perm(permname):
                    continue
