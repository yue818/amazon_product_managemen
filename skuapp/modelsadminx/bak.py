...
class t_templet_amazon_wait_upload_Admin(object):
    ...
    list_display = ('show_image', 'productSKU', 'item_name', 'show_variation_info', 'show_schedule', 'show_info', 'edit_data')
                    ...
                    if t_config_mq_info_objs:
                        ...
                        if sku_result['Code'] == 0:
                            ...
                            if t_templet_amazon_published_variation_objs:
                                ...
                                # skuCount = len(obj.productSKU.split('and'))
                                # if t_templet_amazon_published_variation_objs:
                                #     for t_templet_amazon_published_variation_obj in t_templet_amazon_published_variation_objs:
                                #         skuCount += len(t_templet_amazon_published_variation_obj.productSKU.split('and'))
                                # skuResult = generate_code_func(connection,obj.ShopSets,skuCount)
                                # if skuResult['Code'] == 0:
                                #     count = 0
                                #     link_sku_list = []
                                #     if t_templet_amazon_published_variation_objs:
                                #         paramsMQ['variation'] = 1
                                #         main_variation_count = len(t_templet_amazon_published_variation_objs) + 1
                                #         for i in range(0, main_variation_count):
                                #             if i == 0:
                                #                 item_sku, count, item_sku_list = self.conn_shopSKU(obj.productSKU, skuResult, count)
                                #                 link_sku_list.extend(item_sku_list)
                                #                 obj.item_sku = item_sku
                                #                 obj.part_number = item_sku
                                #                 obj.save()
                                #             else:
                                #                 t_templet_amazon_published_variation_obj = t_templet_amazon_published_variation_objs[i - 1]
                                #                 item_sku, count, item_sku_list = self.conn_shopSKU(t_templet_amazon_published_variation_obj.productSKU, skuResult, count)
                                #                 link_sku_list.extend(item_sku_list)
                                #                 t_templet_amazon_published_variation_obj.parent_sku = obj.item_sku
                                #                 t_templet_amazon_published_variation_obj.child_sku = item_sku
                            ...
                            messages.error(request, '刊登ID：%s申请SKU失败！！' % obj.id)
                    else:
