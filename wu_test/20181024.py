...
def refresh_db_tables(auth_info, sql_execute_obj):
    ...
    sql_product_status = '''UPDATE t_online_info_amazon a, py_db.b_goods b
       SET a.product_status = CASE
                                WHEN b.goodsstatus = '临时下架' THEN
                                 '3'
                                WHEN b.goodsstatus = '售完下架' THEN
                                 '2'
                                ...
                                ELSE
                                 '4'
                              END
     WHERE substring_index(a.sku, '*', 1) = b.sku
     AND a.shopname = '%s'
    ''' % auth_info['ShopName']
    ...
    sql_refund_rate = '''update t_online_info_amazon
       set refund_rate = round(orders_refund_total * 100 / orders_total, 2)
     where shopname = '%s'
     and orders_total > 0;
    ''' % auth_info['ShopName']
...
class FinancesPublic:
    ...
    def get_last_refund_time(self):
        ...
        if max_update_time < datetime.datetime.now() + datetime.timedelta(days=-30):
            last_refund_time.append((max_update_time + datetime.timedelta(minutes=-5)).strftime('%Y-%m-%dT%H:%M:%S'))
            ...
            last_refund_time.append(max_update_time.strftime('%Y-%m-%d'))
    ...
    def parse_item(self, posted_date, amazon_order_id, marketplace_name, shipment_item, report_type, file_num):
        # 公共信息
        seller_sku = shipment_item.get('SellerSKU').get('value').strip()
        ...
        if fee_component_dict:
            ...
            if isinstance(fee_component, list):
                for ind, val in enumerate(fee_component):
                    ...
                    self.insert_finance_record(data_list_feed, file_num)
            else:
        ...
        if charge_component_dict:
            ...
            if isinstance(charge_component, list):
                for ind, val in enumerate(charge_component):
                    ...
                    self.insert_finance_record(data_list_charge, file_num)
            else:
    ...
    def parse_report(self, finance_report_each, report_type, file_num):
        ...
        if ('US' if marketplace_name.split('.')[-1].upper() == 'COM' else marketplace_name.split('.')[-1].upper()) == self.auth_info['ShopSite']:
            ...
            if isinstance(shipment_item, list):
                for ship_item in shipment_item:
                    self.parse_item(posted_date, amazon_order_id, marketplace_name, ship_item, report_type, file_num)
            else:
                self.parse_item(posted_date, amazon_order_id, marketplace_name, shipment_item, report_type, file_num)

    def deal_different_report(self, report_list, file_num):
        for report_body_type_tuple in report_list:
            ...
            if isinstance(report_body, list):
                for report in report_body:
                    self.parse_report(report, report_type, file_num)
            else:
                self.parse_report(report_body, report_type, file_num)

    def finance_flow(self, begin_time, end_time, retry_cnt=0, file_num=None):
        try:
            ...
            self.deal_different_report(report_list, file_num)
    ...
    try:
        ...
        while retry_cnt <= 4 and exe_result != 'error':  # 防止因next_token过期抛出异常，进行4次重试
            ...
            for i in range(len(max_finance_time_list) - 1):
                file_num = 'C:\Users\Administrator\Desktop\API\\finance_record_%s.txt' % datetime.datetime.now().strftime("%Y%m%d%H%M%S")
                start_date = max_finance_time_list[i]
                end_date = max_finance_time_list[i + 1]
                finance_return = finace_obj.finance_flow(begin_time=start_date, end_time=end_date, retry_cnt=retry_cnt, file_num=file_num)
                ...
                if finance_return[0] == 'retry':  # 跳出for循环，进入while，重新获取交易时间列表进行数据抓取
                    ...
                    finace_obj.insert_finance_record_file(file_num)
