...
class t_online_info_wait_publish_joom3_Admin(object):
    downloadxls = True
    import_flag = False
    pp_flag3 = True
    ...
    def get_list_queryset(self,):
        ...
        qs = super(t_online_info_wait_publish_joom3_Admin, self).get_list_queryset()
    ...
    def to_excel1(self, request, queryset):
        ...
        sheet.write(0, 15, u'Shipping Time(enter without " ",just the estimated days)')
        ...
        logger = logging.getLogger('sourceDns.webdns.views')
        joom_shopSKU = t_online_info_joom_shopSKU.objects.filter(plateform='joom3')
        if joom_shopSKU.exists():
            title = joom_shopSKU[0].title
            num = joom_shopSKU[0].index
        #写数据
        row = 0
        for qs in queryset:
            ...
            if t_online_info_objs.exists():
                for t_online_info_obj in t_online_info_objs:
                    num = num + 1
                    JoomShopSKU = title + str(num)
                    row = row + 1
                    column = 0
                    sheet.write(row, column, JoomShopSKU)
                    ...
                    t_online_info_wait_publish_joom3.objects.filter(ProductID=qs.ProductID).update(ispublished='已刊登')
