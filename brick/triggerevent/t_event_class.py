...
class t_event_class():
    ...
    说明:商品状态insert触发事件
    '''
    def t_online_info_insert_trigger(self,productID):
        result = {}
        if productID is None or productID == '':
            result['errorcode'] = 0
            return result
        strEventKey = '{"productID":"' + productID + '"}'
        result = self.insert_t_event('t_online_info_wish_goodsflag','I',strEventKey)
        return result

    '''
        说明:商品状态update触发事件
    '''
    def t_online_info_update_trigger(self,productID):
        result = {}
        if productID is None or productID == '':
            result['errorcode'] = 0
            return result
        strEventKey = '{"productID":"' + productID + '"}'
        result = self.insert_t_event('t_online_info_wish_goodsflag', 'U', strEventKey)
        return result

    '''
        说明:商品状态delete触发事件
     '''
    def t_online_info_delete_trigger(self,productID):
        result = {}
        if productID is None or productID == '':
            result['errorcode'] = 0
            return result
        strEventKey = '{"productID":"' + productID + '"}'
        result = self.insert_t_event('t_online_info_wish_goodsflag', 'D', strEventKey)
        return result
