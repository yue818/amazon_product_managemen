...
"""
 ...
 @site:
 @software: PyCharm
 @file: fba_refresh-20181101a.py
 @time: 2018/11/1 15:26
"""
...
class FinancesPublic:
    ...
    def get_last_refund_time(self):
        cursor = self.db_conn.cursor()
        sql_max_time = "select max(posted_date) from t_amazon_finance_record_20181101 where shop_name = '%s' " % (self.auth_info['ShopName'])
        print 'sql_max_time is: %s' % sql_max_time
        cursor.execute(sql_max_time)
        max_time_obj = cursor.fetchone()
        cursor.close()
    ...
    def insert_finance_record_file(self, file_name):
        ...
        sql_insert = '''insert into t_amazon_finance_record_20181101
        ...
        if promotion_component_dict:
            ...
            if isinstance(promotion_component, list):
                for ind, val in enumerate(promotion_component):
                    ...
                    data_list_promotion = [posted_date, amazon_order_id, marketplace_name, seller_sku, quantity_shipped, order_adjustment_item_id, feed_type, promotion_currency_code, promotion_currency_amount,
