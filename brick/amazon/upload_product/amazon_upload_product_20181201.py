...
"""  
 ...
 @site: 
 @software: PyCharm
 @file: amazon_upload_product_20181128.py
 @time: 2018/11/28 9:16
"""
...
class UpdateProductInfo:
    ...
    def update_db_when_error(self, auth_info, error_msg, error_desc_dict=None):
        cursor = self.db_conn.cursor()
        ...
        try:
            ...
            if submit_feed_result \
                ...
                if submit_feed_result and submit_feed_result.get('Message').get('ProcessingReport').get('Result') and auth_info.get('product_list'):
                    ...
                    self.update_db_when_error(auth_info, 'MessagesWithError is not 0 in mws API processing report ', error_desc_dict)
                else:
...
class Server():
    ...
    def listen_client(self):
        ...
        if self.realIP == '47.254.83.145':
            queue = '210.16.103.56_amazon_upload_toy'  # 160
        elif self.realIP == '47.251.3.95':
            queue = '103.95.13.105_amazon_upload_toy'  # 201
        else:
            pass
    ...
    def callback(self, ch, method, properties, body):
        ...
        while True:
            ...
            if auth_info.get('update_type') and auth_info['update_type'] in ('auto_load_product', 'auto_unload_product', 'load_product', 'unload_product', 'product_info_modify', 'product_price_modify', 'product_image_modify', 'product_price_modify_multi'):
            ...
            if auth_info['IP'] != self.realIP:
                if self.realIP == '47.254.83.145' and auth_info['IP'] == '210.16.103.56':
                    pass  # 0160
                elif self.realIP == '47.251.3.95' and auth_info['IP'] == '103.95.13.105':
                    pass  # 201
                else:
