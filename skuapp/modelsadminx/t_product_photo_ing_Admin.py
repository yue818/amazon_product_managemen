...
class t_product_photo_ing_Admin(t_product_Admin):
    enter_ed_classification = True
    search_box_flag = True
    sku_count= True
    actions = ['to_next','to_makepic']
    ...
    def to_next(self, request, objs):
        ...
        for obj in objs:
            ...
            t_product_pic_completion_obj.PZTime = datetime.now()
            t_product_pic_completion_obj.MGTime = datetime.now()
            t_product_pic_completion_obj.PZStaffName = request.user.first_name
            ...
            t_product_enter_ed.objects.filter(id=obj.pid,MainSKU=obj.MainSKU).update(PZTime=datetime.now(),PZStaffName=request.user.first_name,MGTime=datetime.now(),MGStaffName=request.user.first_name,MGProcess='2')
            t_product_depart_get.objects.filter(pid=obj.pid,MainSKU=obj.MainSKU).update(PZTime=datetime.now(),PZStaffName=request.user.first_name,MGTime=datetime.now(),MGStaffName=request.user.first_name,MGProcess='2')
            t_product_oplog.objects.create(pid=obj.pid, MainSKU=obj.MainSKU,Name=obj.Name, Name2=obj.Name2, OpID=request.user.username,OpName=request.user.first_name,StepID=u'WCSP', StepName='完成拍照', BeginTime=datetime.now())
            obj.delete()

    to_next.short_description = u'完成拍照'

    def to_makepic(self,request, objs):
        for obj in objs:
            t_product_develop_ed_obj = t_product_develop_ed()
            t_product_develop_ed_obj.__dict__ = obj.__dict__
            t_product_develop_ed_obj.MGProcess = '7'
            t_product_develop_ed_obj.Entertime=datetime.now()
            t_product_develop_ed_obj.id = obj.id
            t_product_develop_ed_obj.PZTime = datetime.now()
            t_product_develop_ed_obj.PZStaffName = request.user.first_name
            t_product_develop_ed_obj.save()

            t_product_oplog.objects.create(pid=obj.pid, MainSKU=obj.MainSKU,Name=obj.Name, Name2=obj.Name2, OpID=request.user.username,OpName=request.user.first_name,StepID=u'SPZZ', StepName='完成去制作', BeginTime=datetime.now())

            obj.delete()

    to_makepic.short_description = u'完成去制作'
    ...
    def show_skuattrs(self,obj) :
        ...
        if obj.PZRemake == '1':
            ...
            for t_product_mainsku_sku_obj in t_product_mainsku_sku_objs:
                rt =  '%s <tr><td>%s</td><td>%s</td></tr> '%(rt,t_product_mainsku_sku_obj.ProductSKU,t_product_mainsku_sku_obj.SKUATTRS)
        elif obj.PZRemake == '0':
    ...
    def show_messages1(self, obj):
        rt = u'拍照申请人:%s <br> 拍照申请时间:%s <br>拍照员:%s <br>拍照时间:%s' % (
        obj.PZStaffNameing, obj.PZTimeing, obj.PZStaffName, obj.PZTime)
        return mark_safe(rt)

    show_messages1.short_description = u'拍照信息'

    def show_messages2(self, obj):
        rt = u'业绩归属人2:%s <br>采购员:%s<br>建资料员:%s <br> 建资料时间:%s' % (
        obj.YJGS2StaffName, obj.Buyer, obj.JZLStaffName, obj.JZLTime)
        return mark_safe(rt)
    ...
    list_display= ('pid','MainSKU','show_skuattrs','PZRemake','show_Request','LargeCategory','MGProcess',
                   'PPosition','PZTimeing','show_messages1','show_messages2', 'show_SourcePicPath',
                   'show_messages3','show_SourcePicPath2','SalesApplicant','Entertime','show_urls','show_oplog')
    ...
    def get_list_queryset(self,):
        ...
        qs = super(t_product_photo_ing_Admin, self).get_list_queryset()
        ...
        flag = 0
        try:
            flag = t_sys_staff_auth.objects.filter(StaffID=request.user.username,urltable="t_product_photo_ing").count()
        except:
            pass 
        if request.user.is_superuser or flag != 0:
            qs = qs
        else:
            qs = qs.filter(PZStaffName=request.user.first_name)
