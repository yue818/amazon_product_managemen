...
#
# import os,errno, base64, datetime, urllib,time
import urllib2
# ,urllib,httplib,json
from bs4 import BeautifulSoup

# from ebayTypes import Pagination,EBayOrder,EBayItemInfo,EBaySummary,Money,EBayUnissuedItem,Picture
...
#                         '<ShippingSurcharge>%s</ShippingSurcharge>'%(option.shippingSurcharge) +\
#                         '<ShippingServicePriority>%s</ShippingServicePriority></ShippingServiceOptions>'%(option.shippingServicePriority)
#
...
#                 #Site UK
#                 if self.header['X-EBAY-API-SITEID'] == 3:
...
#         priceXml = '<StartPrice>%s</StartPrice>'%(item.startPrice)
#         #Site UK
#         if self.header['X-EBAY-API-SITEID'] == 3:
...
#             '<Description><![CDATA[%s]]></Description>'%(item.description)+\
...
#             '<Description>%s</Description>'%(item.returnPolicy.description)+\
