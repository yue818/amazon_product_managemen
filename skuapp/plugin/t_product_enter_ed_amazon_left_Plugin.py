...
class t_product_enter_ed_amazon_left_Plugin(BaseAdminPlugin):
    t_product_enter_ed_amazon_left_flag = False

    def init_request(self, *args, **kwargs):
        return bool(self.t_product_enter_ed_amazon_left_flag)
    ...
    def block_left_navbar(self, context, nodes):
        ...
        if flag == 1:
            publish_url = '/Project/admin/skuapp/t_product_enter_ed_amazon?'
            count_all = t_product_enter_ed_aliexpress.objects.filter(
                                                                     StaffID__in=user_list).count()
            count_0 = t_product_enter_ed_aliexpress.objects.filter(Amazon_count__exact=0, 
                                                                   StaffID__in=user_list).count()
            tt = datetime.datetime.today() - datetime.timedelta(days=30)
            # count_0_month = t_product_enter_ed_aliexpress.objects.filter(Amazon_count__exact=0,MGTime__gte=tt).count()
            count_1 = t_product_enter_ed_aliexpress.objects.filter(Amazon_count__exact=1, 
                                                                   StaffID__in=user_list).count()
            count_2 = t_product_enter_ed_aliexpress.objects.filter(Amazon_count__exact=2, 
                                                                   StaffID__in=user_list).count()
            count_3 = t_product_enter_ed_aliexpress.objects.filter(Amazon_count__exact=3, 
                                                                   StaffID__in=user_list).count()
            count_4 = t_product_enter_ed_aliexpress.objects.filter(Amazon_count__gt=3, 
                                                                   StaffID__in=user_list).count()
            ...
            if DepartmentID == '3':
                ...
                count_4 = t_product_enter_ed_aliexpress.objects.filter(Amazon_count__gt=3, 
                                                                       StaffID__in=user_list).exclude(
                    threebuOperation__exact=2).count()
                count_5 = t_product_enter_ed_aliexpress.objects.filter(threebuOperation__exact=2,
            ...
            count_0 = t_product_enter_ed_aliexpress.objects.filter(Amazon_count__exact=0, ).count()
            tt = datetime.datetime.today() - datetime.timedelta(days=30)
            # count_0_month = t_product_enter_ed_aliexpress.objects.filter(Amazon_count__exact=0,MGTime__gte=tt).count()
            count_1 = t_product_enter_ed_aliexpress.objects.filter(Amazon_count__exact=1, ).count()
            count_2 = t_product_enter_ed_aliexpress.objects.filter(Amazon_count__exact=2, ).count()
            count_3 = t_product_enter_ed_aliexpress.objects.filter(Amazon_count__exact=3, ).count()
            count_4 = t_product_enter_ed_aliexpress.objects.filter(Amazon_count__gt=3, ).count()
        ...
        Amazon_count = self.request.GET.get('pub')
        if Amazon_count == '0':
            flag = '0'
        elif Amazon_count == '1':
            flag = '1'
