...
class t_online_info_publish_joom_Admin(object):
    ...
    def show_pic(self,obj) :
        ...
        picUrl =  obj.Image
		#picUrl =  t_online_info.objects.filter(SKU=obj.SKU,ShopSKU=obj.ShopSKU).values_list('Image',flat=True)[0]
        rt =  '<img src="%s"  width="150" height="150"  alt = "%s"  title="%s"  />  '%(picUrl,picUrl,picUrl)
        return mark_safe(rt)
    show_pic.short_description = u'图片'
    list_per_page=20
    list_display=('id','ProductID','Title','Tags','SKU','Price','Quantity','ShopName','Description',)

    list_filter = ('PlatformName','Title','SKU','ShopSKU','Price','Quantity','Status','ShopName','Description','Tags',)
    list_editable = ('PlatformName','Title','SKU','ShopSKU','Price','Quantity','Status','ShopName','Description','Tags','ProductID',)
    search_fields = ('id','PlatformName','ProductID','Title','SKU','ShopSKU','Price','Quantity','ShopName','Description','Tags',)
    fields =  ('id',)
