...
class t_online_info_wait_publish_UMKA_Admin(object):
    downloadxls = True
    import_flag = False
    umka_flag = True
    ...
    def get_list_queryset(self,):
        ...
        qs = super(t_online_info_wait_publish_UMKA_Admin, self).get_list_queryset()
    ...
    def to_excel(self, request, queryset):
        ...
        sheet.write(0, 4, u'商品名称')#商品名称
        sheet.write(0, 5, u'包装重量')#包装重量
        sheet.write(0, 6, u'包装规格')#包装规格
        sheet.write(0, 7, u'备注')#备注
        sheet.write(0, 8, u'Product Unique ID') #Wish店铺主SKU
        sheet.write(0, 9, u'Variation Unique ID') #Wish店铺子SKU
        sheet.write(0, 10, u'Category') #类目(空)
        sheet.write(0, 11, u'SKU')#SKU(空)
        sheet.write(0, 12, u'group_id')#空
        sheet.write(0, 13, u'enable')#True
        sheet.write(0, 14, u'stock')#999
        sheet.write(0, 15, u'name')#标题
        sheet.write(0, 16, u'price')#空
        sheet.write(0, 17, u'old_price')#空
        sheet.write(0, 18, u'color')#颜色
        sheet.write(0, 19, u'size')#尺寸
        sheet.write(0, 20, u'weight')#重量(kg)
        sheet.write(0, 21, u'packaging_size')#空
        sheet.write(0, 22, u'brand')#空
        sheet.write(0, 23, u'tags')#标签
        sheet.write(0, 24, u'upc')#空
        sheet.write(0, 25, u'description')#描述       
        sheet.write(0, 26, u'main_image_url')#主图
        sheet.write(0, 27, u'Extra_Image_URL_1')#附图1
        sheet.write(0, 28, u'Extra_Image_URL_2')#2
        sheet.write(0, 29, u'Extra_Image_URL_3')#3
        sheet.write(0, 30, u'Extra_Image_URL_4')#4
        sheet.write(0, 31, u'Extra_Image_URL_5')#5
        sheet.write(0, 32, u'Extra_Image_URL_6')#6
        sheet.write(0, 33, u'Extra_Image_URL_7')#7
        sheet.write(0, 34, u'Extra_Image_URL_8')#8
        sheet.write(0, 35, u'Extra_Image_URL_9')#9
        sheet.write(0, 36, u'Extra_Image_URL_10')#10
        sheet.write(0, 37, u'shipping_template_id')#主图
        sheet.write(0, 38, u'shipping_time')#附图 
        sheet.write(0, 39, u'langing_page_url')#附图 
        ...
        for qs in queryset:
            ...
            for t_online_info_obj in info_objs:
                ...
                column = column + 1
                try:
                    PackWeight = py_b_goods.objects.get(SKU=t_online_info_obj['SKU']).PackWeight
                except:
                    PackWeight = ''
                sheet.write(row,column,PackWeight)
                
                column = column + 1
                try:
                    PackName = py_b_goods.objects.get(SKU=t_online_info_obj['SKU']).PackName
                except:
                    PackName = ''
                sheet.write(row,column,PackName)  
                   
                column = column + 1                 
                try:
                    Notes = py_b_goods.objects.get(SKU=t_online_info_obj['SKU']).Notes
                except:
                    Notes = ''
                sheet.write(row,column,Notes)
                ...
                column = column + 1
                sheet.write(row,column,'')
            t_online_info_wait_publish_UMKA.objects.filter(ProductID=qs.ProductID).update(ispublished='已刊登')
