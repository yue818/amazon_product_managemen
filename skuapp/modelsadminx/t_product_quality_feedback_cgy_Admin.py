...
from t_product_quality_feedback_submit_Admin import PREFIX, ACCESS_KEY_ID, ACCESS_KEY_SECRET, BUCKETNAME, ENDPOINT_out
...
class t_product_quality_feedback_cgy_Admin(object):
    ...
    def show_img(self, obj):
        ...
        if obj.Picture_2 or obj.Picture_3 or obj.Picture_4 or obj.Picture_5:
            rt = u"%s<br><a id=pic_%s>全部图片</a><script>$('#pic_%s').on('click',function(){layer.open({type:2,skin:'layui-layer-lan'," \
                 ...
                 u"content:'/show_feedback_picture/?id=%s&page=cgy',});});</script>" % (rt, obj.id, obj.id, obj.id)
        return mark_safe(rt)
    ...
    def show_img_zjy(self, obj):
        ...
        rt = u"%s<br><a id=img_%s>全部图片</a><script>$('#img_%s').on('click',function(){layer.open({type:2,skin:'layui-layer-lan'," \
             u"title:'全部图片',fix:false,shadeClose: true,maxmin:true,area:['800px','600px']," \
             u"content:'/feedback_zjy_picture/?id=%s&page=cgy',});});</script>" % (rt, obj.id, obj.id, obj.id)
        return mark_safe(rt)
    ...
    def show_operation(self, obj):
        ...
        if obj.Step:
            rt = u"<a id=chart_%s>查看进度</a><script>$('#chart_%s').on('click',function(){layer.open({type:2,skin:'layui-layer-lan'," \
                 ...
                 u"content:'/show_feedback_step/?id=%s&page=cgy',});});</script>" % (obj.id, obj.id, obj.id)
        return mark_safe(rt)
    ...
    list_display = ('id', 'show_product', 'show_submit', 'show_img', 'show_img_zjy', 'ProblemType', 'show_detail',
                    'ZJY', 'ZJY_Note', 'CGY', 'CGY_Note', 'State', 'Reject_Reason', 'show_operation')
    list_display_links = ('',)
    fields = (
        'Plateform', 'OrderID', 'SKU', 'ProblemType', 'Detail', 'Picture_1', 'Picture_2', 'Picture_3', 'Picture_4',
        'Picture_5')
    list_editable = ('CGY_Note', 'Reject_Reason')
    ...
    def set_step(self, query, next_step, next_name, next_state, this_state):
        ...
        table_dict = {'submit': feedback_submit, 'cpzy': feedback_cpzy}
        ...
        if table_dict.has_key(query.Source_Origin):
            ...
            if table_objs:
                ...
                if step_list:
                    ...
                    last_step_dict['name'] = query.CGY
                    last_step_dict['note'] = query.CGY_Note
            ...
            if query.State in ['wcl', 'bbh']:
                if query.CGY_Note:
                    feedback_final_sh.objects.create(SubmitTime=query.SubmitTime, Submitter=query.Submitter,
                                                     ...
                                                     Last_Source='cgy', ClothingFlag=query.ClothingFlag)
                    query.State = 'tjdzzsh'
                    query.save()
    ...
    def get_list_queryset(self, ):
        ...
        qs = super(t_product_quality_feedback_cgy_Admin, self).get_list_queryset()
        cate = request.GET.get('cate', '')

        flag = 0
        try:
            flag = t_sys_staff_auth.objects.filter(StaffID=request.user.username,urltable="t_product_quality_feedback_cgy").count()
        except:
            pass
        if request.user.is_superuser or flag != 0:
            ...
            if cate == '':
                qs = qs.filter(CGY=request.user.first_name, State='wcl')
            elif cate == 'all':
                qs = qs.filter(Submitter=request.user.first_name)
            else:
                qs = qs.filter(CGY=request.user.first_name, State=cate)
