...
"""  
 ...
 @site: 
 @software: PyCharm
 @file: amazon_advertising_shop_report_Plugin.py
...
class amazon_advertising_shop_report_Plugin(BaseAdminPlugin):
    amazon_advertising_shop_report_flag = False

    def init_request(self, *args, **kwargs):
        return bool(self.amazon_advertising_shop_report_flag)

    def getdata(self, param):

        sql_day = '''select date_format(advertising_business_date,'%%Y-%%m-%%d') as advertising_business_date,sum(ordered_sales) as ordered_sales,
        ...
        from t_template_amazon_advertising_business_daily_report
        %s
        group by advertising_business_date
        order by 1 '''
        ...
        query = " where shopname = '%s'" % (param['shopname'])
        # if param['createuser'] != '':
    ...
    def block_search_cata_nav(self, context, nodes):
        ...
        if is_viewed and is_viewed == '1':
            ...
            if len(data) > 0:
                ...
                if param['datatype'] == 'week':
                    title = u'%s店铺%s统计报表(%s~%s)' % (param['shopname'], trans[param['datatype']], min(datelist)[:10], max(datelist)[11:],)
                else:
                    title = u'%s店铺%s统计报表(%s~%s)' % (param['shopname'], trans[param['datatype']], min(datelist), max(datelist),)
            else:
