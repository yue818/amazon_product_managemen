...
#import logging
import datetime
class CreateOnlineinfoTask():
    def __init__(self,db_conn):
        self.db_conn = db_conn
        self.cmdid ='GetShopSKUInfo'
        self.PlatformName = 'Wish'

    def CreateTasks(self):
        cursor =self.db_conn.cursor();
        sql= "insert into t_api_schedule_ing(ShopName,PlatformName,CMDID,Status,ScheduleTime,inserttime,updatetime,timedelta,retrycount) select  name, \'Wish\',\'GetShopSKUInfo\',0, now(),now(),now(),1200,0 from t_config_online_amazon where K= \'access_token\' ";
        cursor.execute(sql)
        cursor.close()
        self.db_conn.commit()
