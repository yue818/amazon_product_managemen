...
class t_product_enter_ed_wish_Admin(t_product_Admin):
    t_product_enter_ed_menu_flag = True
    t_product_enter_ed_wish_left_flag = True
    search_box1_flag = True
    # search_box_flag = True
    # aliexpress_left_flag = True
    list_display = ('id', 'MainSKU', 'show_sku_publish', 'DYStaffName', 'MGProcess', 'MGTime',
                    'show_SourcePicPath', 'SpecialSell', 'show_more_information', 'ClothingNote', 'ShelveDay',
                    'OrdersLast7Days',
    ...
    def show_sku_publish(self, obj):
        ...
        for t_log_sku_shopsku_apply_obj in rows:
            try:
                user_name = \
                User.objects.filter(first_name=t_log_sku_shopsku_apply_obj[1]).values_list('username', flat=True)[0]
                DepartmentID = \
                t_sys_department_staff.objects.filter(StaffID=user_name).values_list('DepartmentID', flat=True)[0]
            except:
                DepartmentID = '未知部门'
        ...
        if obj.elevenbuOperation == '2':
            info = '%s十一部弃用<br/>' % info
        rt = '%s</table>' % info
        rt = '%s<br/><span style="color:green;cursor: pointer;" id="Editcategory_%s_%s">修改wish品类</span>' % (
        rt, obj.MainSKU, obj.id)
        rt = "%s<script>$('#Editcategory_%s_%s').on('click',function()" \
             "{layer.open({type:2,skin:'layui-layer-lan',title:'修改wish品类'," \
    ...
    def batch_Edit_Category(self, request, objs):
        ...
        information_title = ['业务流水号', '主SKU', '普元品类', '原Wish品类(以逗号隔开)', '操作', '新Wish品类']
    ...
    batch_Edit_Category.short_description = u'批量修改Wish品类'
    ...
    def get_list_queryset(self, ):
        ...
        qs = super(t_product_enter_ed_wish_Admin, self).get_list_queryset()
        ...
        if flag == 1:
            if pub == '':
                qs = qs.filter(StaffID__in=user_list)
            if pub == '0':
                if DepartmentID == '1':
                    qs = qs.filter(Wish_count__exact=0, StaffID__in=user_list).exclude(onebuOperation__exact=2)
                if DepartmentID == '2':
                    qs = qs.filter(Wish_count__exact=0, StaffID__in=user_list).exclude(twobuOperation__exact=2)
                if DepartmentID == '3':
                    qs = qs.filter(Wish_count__exact=0, StaffID__in=user_list).exclude(threebuOperation__exact=2)
                if DepartmentID == '4':
                    qs = qs.filter(Wish_count__exact=0, StaffID__in=user_list).exclude(fourbuOperation__exact=2)
                if DepartmentID == '5':
                    qs = qs.filter(Wish_count__exact=0, StaffID__in=user_list).exclude(fivebuOperation__exact=2)
                if DepartmentID == '6':
                    qs = qs.filter(Wish_count__exact=0, StaffID__in=user_list).exclude(sixbuOperation__exact=2)
                if DepartmentID == '7':
                    qs = qs.filter(Wish_count__exact=0, StaffID__in=user_list).exclude(sevenbuOperation__exact=2)
                if DepartmentID == '8':
                    qs = qs.filter(Wish_count__exact=0, StaffID__in=user_list).exclude(eightbuOperation__exact=2)
                if DepartmentID == '9':
                    qs = qs.filter(Wish_count__exact=0, StaffID__in=user_list).exclude(ninebuOperation__exact=2)
                if DepartmentID == '10':
                    qs = qs.filter(Wish_count__exact=0, StaffID__in=user_list).exclude(tenbuOperation__exact=2)
                if DepartmentID == '11':
                    qs = qs.filter(Wish_count__exact=0, StaffID__in=user_list).exclude(elevenbuOperation__exact=2)
                if DepartmentID == '12':
                    qs = qs.filter(Wish_count__exact=0, StaffID__in=user_list).exclude(twelvebuOperation__exact=2)
                if DepartmentID == '13':
                    qs = qs.filter(Wish_count__exact=0, StaffID__in=user_list).exclude(thirteenbuOperation__exact=2)
                # qs = qs.filter(publish_count='0',StaffID__in = user_list,PlatformName__exact='Wish')
            elif pub == '1':
                if DepartmentID == '1':
                    qs = qs.filter(Wish_count__exact=1, StaffID__in=user_list).exclude(onebuOperation__exact=2)
                if DepartmentID == '2':
                    qs = qs.filter(Wish_count__exact=1, StaffID__in=user_list).exclude(twobuOperation__exact=2)
                if DepartmentID == '3':
                    qs = qs.filter(Wish_count__exact=1, StaffID__in=user_list).exclude(threebuOperation__exact=2)
                if DepartmentID == '4':
                    qs = qs.filter(Wish_count__exact=1, StaffID__in=user_list).exclude(fourbuOperation__exact=2)
                if DepartmentID == '5':
                    qs = qs.filter(Wish_count__exact=1, StaffID__in=user_list).exclude(fivebuOperation__exact=2)
                if DepartmentID == '6':
                    qs = qs.filter(Wish_count__exact=1, StaffID__in=user_list).exclude(sixbuOperation__exact=2)
                if DepartmentID == '7':
                    qs = qs.filter(Wish_count__exact=1, StaffID__in=user_list).exclude(sevenbuOperation__exact=2)
                if DepartmentID == '8':
                    qs = qs.filter(Wish_count__exact=1, StaffID__in=user_list).exclude(eightbuOperation__exact=2)
                if DepartmentID == '9':
                    qs = qs.filter(Wish_count__exact=1, StaffID__in=user_list).exclude(ninebuOperation__exact=2)
                if DepartmentID == '10':
                    qs = qs.filter(Wish_count__exact=1, StaffID__in=user_list).exclude(tenbuOperation__exact=2)
                if DepartmentID == '11':
                    qs = qs.filter(Wish_count__exact=1, StaffID__in=user_list).exclude(elevenbuOperation__exact=2)
                if DepartmentID == '12':
                    qs = qs.filter(Wish_count__exact=1, StaffID__in=user_list).exclude(twelvebuOperation__exact=2)
                if DepartmentID == '13':
                    qs = qs.filter(Wish_count__exact=1, StaffID__in=user_list).exclude(thirteenbuOperation__exact=2)
                # qs = qs.filter(publish_count='1',StaffID__in = user_list,PlatformName__exact='Wish')
            elif pub == '2':
                if DepartmentID == '1':
                    qs = qs.filter(Wish_count__exact=2, StaffID__in=user_list).exclude(onebuOperation__exact=2)
                if DepartmentID == '2':
                    qs = qs.filter(Wish_count__exact=2, StaffID__in=user_list).exclude(twobuOperation__exact=2)
                if DepartmentID == '3':
                    qs = qs.filter(Wish_count__exact=2, StaffID__in=user_list).exclude(threebuOperation__exact=2)
                if DepartmentID == '4':
                    qs = qs.filter(Wish_count__exact=2, StaffID__in=user_list).exclude(fourbuOperation__exact=2)
                if DepartmentID == '5':
                    qs = qs.filter(Wish_count__exact=2, StaffID__in=user_list).exclude(fivebuOperation__exact=2)
                if DepartmentID == '6':
                    qs = qs.filter(Wish_count__exact=2, StaffID__in=user_list).exclude(sixbuOperation__exact=2)
                if DepartmentID == '7':
                    qs = qs.filter(Wish_count__exact=2, StaffID__in=user_list).exclude(sevenbuOperation__exact=2)
                if DepartmentID == '8':
                    qs = qs.filter(Wish_count__exact=2, StaffID__in=user_list).exclude(eightbuOperation__exact=2)
                if DepartmentID == '9':
                    qs = qs.filter(Wish_count__exact=2, StaffID__in=user_list).exclude(ninebuOperation__exact=2)
                if DepartmentID == '10':
                    qs = qs.filter(Wish_count__exact=2, StaffID__in=user_list).exclude(tenbuOperation__exact=2)
                if DepartmentID == '11':
                    qs = qs.filter(Wish_count__exact=2, StaffID__in=user_list).exclude(elevenbuOperation__exact=2)
                if DepartmentID == '12':
                    qs = qs.filter(Wish_count__exact=2, StaffID__in=user_list).exclude(twelvebuOperation__exact=2)
                if DepartmentID == '13':
                    qs = qs.filter(Wish_count__exact=2, StaffID__in=user_list).exclude(thirteenbuOperation__exact=2)
                # qs = qs.filter(publish_count='2',StaffID__in = user_list,PlatformName__exact='Wish')
            elif pub == '3':
                if DepartmentID == '1':
                    qs = qs.filter(Wish_count__exact=3, StaffID__in=user_list).exclude(onebuOperation__exact=2)
                if DepartmentID == '2':
                    qs = qs.filter(Wish_count__exact=3, StaffID__in=user_list).exclude(twobuOperation__exact=2)
                if DepartmentID == '3':
                    qs = qs.filter(Wish_count__exact=3, StaffID__in=user_list).exclude(threebuOperation__exact=2)
                if DepartmentID == '4':
                    qs = qs.filter(Wish_count__exact=3, StaffID__in=user_list).exclude(fourbuOperation__exact=2)
                if DepartmentID == '5':
                    qs = qs.filter(Wish_count__exact=3, StaffID__in=user_list).exclude(fivebuOperation__exact=2)
                if DepartmentID == '6':
                    qs = qs.filter(Wish_count__exact=3, StaffID__in=user_list).exclude(sixbuOperation__exact=2)
                if DepartmentID == '7':
                    qs = qs.filter(Wish_count__exact=3, StaffID__in=user_list).exclude(sevenbuOperation__exact=2)
                if DepartmentID == '8':
                    qs = qs.filter(Wish_count__exact=3, StaffID__in=user_list).exclude(eightbuOperation__exact=2)
                if DepartmentID == '9':
                    qs = qs.filter(Wish_count__exact=3, StaffID__in=user_list).exclude(ninebuOperation__exact=2)
                if DepartmentID == '10':
                    qs = qs.filter(Wish_count__exact=3, StaffID__in=user_list).exclude(tenbuOperation__exact=2)
                if DepartmentID == '11':
                    qs = qs.filter(Wish_count__exact=3, StaffID__in=user_list).exclude(elevenbuOperation__exact=2)
                if DepartmentID == '12':
                    qs = qs.filter(Wish_count__exact=3, StaffID__in=user_list).exclude(twelvebuOperation__exact=2)
                if DepartmentID == '13':
                    qs = qs.filter(Wish_count__exact=3, StaffID__in=user_list).exclude(thirteenbuOperation__exact=2)
                # qs = qs.filter(publish_count='3',StaffID__in = user_list,PlatformName__exact='Wish')
            elif pub == '4':
                if DepartmentID == '1':
                    qs = qs.filter(Wish_count__gt=3, StaffID__in=user_list).exclude(onebuOperation__exact=2)
                if DepartmentID == '2':
                    qs = qs.filter(Wish_count__gt=3, StaffID__in=user_list).exclude(twobuOperation__exact=2)
                if DepartmentID == '3':
                    qs = qs.filter(Wish_count__gt=3, StaffID__in=user_list).exclude(threebuOperation__exact=2)
                if DepartmentID == '4':
                    qs = qs.filter(Wish_count__gt=3, StaffID__in=user_list).exclude(fourbuOperation__exact=2)
                if DepartmentID == '5':
                    qs = qs.filter(Wish_count__gt=3, StaffID__in=user_list).exclude(fivebuOperation__exact=2)
                if DepartmentID == '6':
                    qs = qs.filter(Wish_count__gt=3, StaffID__in=user_list).exclude(sixbuOperation__exact=2)
                if DepartmentID == '7':
                    qs = qs.filter(Wish_count__gt=3, StaffID__in=user_list).exclude(sevenbuOperation__exact=2)
                if DepartmentID == '8':
                    qs = qs.filter(Wish_count__gt=3, StaffID__in=user_list).exclude(eightbuOperation__exact=2)
                if DepartmentID == '9':
                    qs = qs.filter(Wish_count__gt=3, StaffID__in=user_list).exclude(ninebuOperation__exact=2)
                if DepartmentID == '10':
                    qs = qs.filter(Wish_count__gt=3, StaffID__in=user_list).exclude(tenbuOperation__exact=2)
                if DepartmentID == '11':
                    qs = qs.filter(Wish_count__gt=3, StaffID__in=user_list).exclude(elevenbuOperation__exact=2)
                if DepartmentID == '12':
                    qs = qs.filter(Wish_count__gt=3, StaffID__in=user_list).exclude(twelvebuOperation__exact=2)
                if DepartmentID == '13':
                    qs = qs.filter(Wish_count__gt=3, StaffID__in=user_list).exclude(thirteenbuOperation__exact=2)
                # qs = qs.filter(publish_count__gt ='3',StaffID__in = user_list,PlatformName__exact='Wish')
            elif pub == '5':
                if DepartmentID == '1':
                    qs = qs.filter(onebuOperation__exact=2,StaffID__in=user_list)
                if DepartmentID == '2':
                    qs = qs.filter(twobuOperation__exact=2, StaffID__in=user_list)
                if DepartmentID == '3':
                    qs = qs.filter(threebuOperation__exact=2, StaffID__in=user_list)
                if DepartmentID == '4':
                    qs = qs.filter(fourbuOperation__exact=2, StaffID__in=user_list)
                if DepartmentID == '5':
                    qs = qs.filter(fivebuOperation__exact=2, StaffID__in=user_list)

                if DepartmentID == '6':
                    qs = qs.filter(sixbuOperation__exact=2, StaffID__in=user_list)
                if DepartmentID == '7':
                    qs = qs.filter(sevenbuOperation__exact=2, StaffID__in=user_list)
                if DepartmentID == '8':
                    qs = qs.filter(eightbuOperation__exact=2, StaffID__in=user_list)
                if DepartmentID == '9':
                    qs = qs.filter(ninebuOperation__exact=2, StaffID__in=user_list)
                if DepartmentID == '10':
                    qs = qs.filter(tenbuOperation__exact=2, StaffID__in=user_list)
                if DepartmentID == '11':
                    qs = qs.filter(elevenbuOperation__exact=2, StaffID__in=user_list)
                if DepartmentID == '12':
                    qs = qs.filter(twelvebuOperation__exact=2, StaffID__in=user_list)
                if DepartmentID == '13':
                    qs = qs.filter(thirteenbuOperation__exact=2, StaffID__in=user_list)

        else:
            if pub == '':
                qs = qs.filter()
            if pub == '0':
                qs = qs.filter(Wish_count='0')
            elif pub == '1':
                qs = qs.filter(Wish_count='1')
            elif pub == '2':
                qs = qs.filter(Wish_count='2')
            elif pub == '3':
                qs = qs.filter(Wish_count='3')
            elif pub == '4':
                qs = qs.filter(publish_count__gt=3)
            elif pub == '5':
                qs = qs.filter().filter(Q(onebuOperation__exact=2) | Q(twobuOperation__exact=2) | Q(threebuOperation__exact=2) | Q(fourbuOperation__exact=2) | Q(fivebuOperation__exact=2) | Q(sixbuOperation__exact=2) | Q(sevenbuOperation__exact=2) | Q(eightbuOperation__exact=2) | Q(ninebuOperation__exact=2) | Q(tenbuOperation__exact=2) | Q(elevenbuOperation__exact=2) | Q(twelvebuOperation__exact=2) | Q(thirteenbuOperation__exact=2))
