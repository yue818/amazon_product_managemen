...
class t_product_depart_get_Admin(t_product_Admin):
    enter_ed = True
    ...
    list_display= ('pid','StaffName','show_SalesAttr','show_Pub_Info','UpdateTime','DepartmentID','MainSKU','MGProcess','MGTime','show_SourcePicPath','show_more_information','show_SourcePicPath2','SpecialSell','SpecialRemark','show_urls','show_Secondary_research',)
    ...
    def get_list_queryset(self):
        ...
        qs = super(t_product_depart_get_Admin, self).get_list_queryset()
        ...
        if request.user.is_superuser:
            ...
            if t_sys_department_staff_objs.count() <=0 :
                messages.error(request,u'你没有部门数据,请联系管理员正确录入部门信息！')
                return qs.none()
            else:
                DepartmentID = t_sys_department_staff_objs[0].DepartmentID
                return qs.filter(DepartmentID=DepartmentID)
