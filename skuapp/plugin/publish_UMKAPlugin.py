...
class publish_UMKAPlugin(BaseAdminPlugin):
    umka_flag = False

    def init_request(self, *args, **kwargs):
        return bool(self.umka_flag)
    ...
    def block_search_cata_nav(self, context, nodes):
        ...
        countsdict={}
        lm=[]
        oo = t_online_info_wait_publish_UMKA.objects.values_list('FL',flat=True)
        ...
        for i in oo:
            ...
            if i in lm:
                ...
                if i is not None and i != '':
                    counts = t_online_info_wait_publish_UMKA.objects.filter(FL='%s'%i.encode('utf-8'),ispublished='未刊登').count()
        ...
        nodes.append(loader.render_to_string('publish_UMKA.html',{'url':newurl,'flag':flag,'countsdict':countsdict}))
