...
class ReportPublic:
    ...
    def report_flow(self, report_type, start_date=None, end_date=None):
        try:
            ...
            report_request_id = '83784017855' #self.submit_report_request(report_type, start_date, end_date)
            time_sleep = 20
            count = 0
...
class GetLocalIPAndAuthInfo:
    ...
    def get_auth_info_by_ip(self, ip):
        cursor = self.db_conn.cursor()
        sql = "select IP,Name,K,V, site from t_config_online_amazon  where IP= '%s' and site = 'US'" % ip
        print sql
        cursor.execute(sql)
...
if __name__ == '__main__':
    ...
    for key, val in auth_info_all.items():
        ...
        try:
            ...
            # get_data_public_obj.report_flow('_GET_FLAT_FILE_ALL_ORDERS_DATA_BY_ORDER_DATE_')
