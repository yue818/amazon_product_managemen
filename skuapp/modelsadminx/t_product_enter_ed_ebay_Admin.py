...
class t_product_enter_ed_ebay_Admin(t_product_Admin):
    t_product_enter_ed_menu_flag = True
    t_product_enter_ed_ebay_left_flag = True
    ...
    def show_sku_publish(self,obj):
        ...
        if obj.elevenbuOperation == '2':
            info = '%s十一部弃用<br/>' % info
        rt = '%s</table>' % info
        rt = '%s<br/><span style="color:green;cursor: pointer;" id="Editcategory_%s_%s">修改ebay品类</span>' % (
        rt, obj.MainSKU, obj.id)
        rt = "%s<script>$('#Editcategory_%s_%s').on('click',function()" \
             "{layer.open({type:2,skin:'layui-layer-lan',title:'修改ebay品类'," \
    ...
    def batch_Edit_Category(self, request, objs):
        ...
        information_title = ['业务流水号', '主SKU', '普元品类', '原eBay品类(以逗号隔开)', '操作', '新eBay品类']
        ...
        return render(request, 'skuapp_editcategoryMore.html', context=result)
    batch_Edit_Category.short_description = u'批量修改eBay品类'
    ...
    def get_list_queryset(self, ):
        ...
        qs = super(t_product_enter_ed_ebay_Admin, self).get_list_queryset()
        ...
        if flag == 1:
            ...
            if pub == '0':
                if DepartmentID == '1':
                    qs = qs.filter(eBay_count__exact=0, StaffID__in=user_list).exclude(onebuOperation__exact=2)
                if DepartmentID == '2':
                    qs = qs.filter(eBay_count__exact=0, StaffID__in=user_list).exclude(twobuOperation__exact=2)
                if DepartmentID == '3':
                    qs = qs.filter(eBay_count__exact=0, StaffID__in=user_list).exclude(threebuOperation__exact=2)
                if DepartmentID == '4':
                    qs = qs.filter(eBay_count__exact=0, StaffID__in=user_list).exclude(fourbuOperation__exact=2)
                if DepartmentID == '5':
                    qs = qs.filter(eBay_count__exact=0, StaffID__in=user_list).exclude(fivebuOperation__exact=2)
                if DepartmentID == '6':
                    qs = qs.filter(eBay_count__exact=0, StaffID__in=user_list).exclude(sixbuOperation__exact=2)
                if DepartmentID == '7':
                    qs = qs.filter(eBay_count__exact=0, StaffID__in=user_list).exclude(sevenbuOperation__exact=2)
                if DepartmentID == '8':
                    qs = qs.filter(eBay_count__exact=0, StaffID__in=user_list).exclude(eightbuOperation__exact=2)
                if DepartmentID == '9':
                    qs = qs.filter(eBay_count__exact=0, StaffID__in=user_list).exclude(ninebuOperation__exact=2)
                if DepartmentID == '10':
                    qs = qs.filter(eBay_count__exact=0, StaffID__in=user_list).exclude(tenbuOperation__exact=2)
                if DepartmentID == '11':
                    qs = qs.filter(eBay_count__exact=0, StaffID__in=user_list).exclude(elevenbuOperation__exact=2)
                if DepartmentID == '12':
                    qs = qs.filter(eBay_count__exact=0, StaffID__in=user_list).exclude(twelvebuOperation__exact=2)
                if DepartmentID == '13':
                    qs = qs.filter(eBay_count__exact=0, StaffID__in=user_list).exclude(thirteenbuOperation__exact=2)
                # qs = qs.filter(eBay_count='0',StaffID__in = user_list,PlatformName__exact='eBay')
            elif pub == '1':
                if DepartmentID == '1':
                    qs = qs.filter(eBay_count__exact=1, StaffID__in=user_list).exclude(onebuOperation__exact=2)
                if DepartmentID == '2':
                    qs = qs.filter(eBay_count__exact=1, StaffID__in=user_list).exclude(twobuOperation__exact=2)
                if DepartmentID == '3':
                    qs = qs.filter(eBay_count__exact=1, StaffID__in=user_list).exclude(threebuOperation__exact=2)
                if DepartmentID == '4':
                    qs = qs.filter(eBay_count__exact=1, StaffID__in=user_list).exclude(fourbuOperation__exact=2)
                if DepartmentID == '5':
                    qs = qs.filter(eBay_count__exact=1, StaffID__in=user_list).exclude(fivebuOperation__exact=2)
                if DepartmentID == '6':
                    qs = qs.filter(eBay_count__exact=1, StaffID__in=user_list).exclude(sixbuOperation__exact=2)
                if DepartmentID == '7':
                    qs = qs.filter(eBay_count__exact=1, StaffID__in=user_list).exclude(sevenbuOperation__exact=2)
                if DepartmentID == '8':
                    qs = qs.filter(eBay_count__exact=1, StaffID__in=user_list).exclude(eightbuOperation__exact=2)
                if DepartmentID == '9':
                    qs = qs.filter(eBay_count__exact=1, StaffID__in=user_list).exclude(ninebuOperation__exact=2)
                if DepartmentID == '10':
                    qs = qs.filter(eBay_count__exact=1, StaffID__in=user_list).exclude(tenbuOperation__exact=2)
                if DepartmentID == '11':
                    qs = qs.filter(eBay_count__exact=1, StaffID__in=user_list).exclude(elevenbuOperation__exact=2)
                if DepartmentID == '12':
                    qs = qs.filter(eBay_count__exact=1, StaffID__in=user_list).exclude(twelvebuOperation__exact=2)
                if DepartmentID == '13':
                    qs = qs.filter(eBay_count__exact=1, StaffID__in=user_list).exclude(thirteenbuOperation__exact=2)
                # qs = qs.filter(eBay_count='1',StaffID__in = user_list,PlatformName__exact='eBay')
            elif pub == '2':
                if DepartmentID == '1':
                    qs = qs.filter(eBay_count__exact=2, StaffID__in=user_list).exclude(onebuOperation__exact=2)
                if DepartmentID == '2':
                    qs = qs.filter(eBay_count__exact=2, StaffID__in=user_list).exclude(twobuOperation__exact=2)
                if DepartmentID == '3':
                    qs = qs.filter(eBay_count__exact=2, StaffID__in=user_list).exclude(threebuOperation__exact=2)
                if DepartmentID == '4':
                    qs = qs.filter(eBay_count__exact=2, StaffID__in=user_list).exclude(fourbuOperation__exact=2)
                if DepartmentID == '5':
                    qs = qs.filter(eBay_count__exact=2, StaffID__in=user_list).exclude(fivebuOperation__exact=2)
                if DepartmentID == '6':
                    qs = qs.filter(eBay_count__exact=2, StaffID__in=user_list).exclude(sixbuOperation__exact=2)
                if DepartmentID == '7':
                    qs = qs.filter(eBay_count__exact=2, StaffID__in=user_list).exclude(sevenbuOperation__exact=2)
                if DepartmentID == '8':
                    qs = qs.filter(eBay_count__exact=2, StaffID__in=user_list).exclude(eightbuOperation__exact=2)
                if DepartmentID == '9':
                    qs = qs.filter(eBay_count__exact=2, StaffID__in=user_list).exclude(ninebuOperation__exact=2)
                if DepartmentID == '10':
                    qs = qs.filter(eBay_count__exact=2, StaffID__in=user_list).exclude(tenbuOperation__exact=2)
                if DepartmentID == '11':
                    qs = qs.filter(eBay_count__exact=2, StaffID__in=user_list).exclude(elevenbuOperation__exact=2)
                if DepartmentID == '12':
                    qs = qs.filter(eBay_count__exact=2, StaffID__in=user_list).exclude(twelvebuOperation__exact=2)
                if DepartmentID == '13':
                    qs = qs.filter(eBay_count__exact=2, StaffID__in=user_list).exclude(thirteenbuOperation__exact=2)
                # qs = qs.filter(eBay_count='2',StaffID__in = user_list,PlatformName__exact='eBay')
            elif pub == '3':
                if DepartmentID == '1':
                    qs = qs.filter(eBay_count__exact=3, StaffID__in=user_list).exclude(onebuOperation__exact=2)
                if DepartmentID == '2':
                    qs = qs.filter(eBay_count__exact=3, StaffID__in=user_list).exclude(twobuOperation__exact=2)
                if DepartmentID == '3':
                    qs = qs.filter(eBay_count__exact=3, StaffID__in=user_list).exclude(threebuOperation__exact=2)
                if DepartmentID == '4':
                    qs = qs.filter(eBay_count__exact=3, StaffID__in=user_list).exclude(fourbuOperation__exact=2)
                if DepartmentID == '5':
                    qs = qs.filter(eBay_count__exact=3, StaffID__in=user_list).exclude(fivebuOperation__exact=2)
                if DepartmentID == '6':
                    qs = qs.filter(eBay_count__exact=3, StaffID__in=user_list).exclude(sixbuOperation__exact=2)
                if DepartmentID == '7':
                    qs = qs.filter(eBay_count__exact=3, StaffID__in=user_list).exclude(sevenbuOperation__exact=2)
                if DepartmentID == '8':
                    qs = qs.filter(eBay_count__exact=3, StaffID__in=user_list).exclude(eightbuOperation__exact=2)
                if DepartmentID == '9':
                    qs = qs.filter(eBay_count__exact=3, StaffID__in=user_list).exclude(ninebuOperation__exact=2)
                if DepartmentID == '10':
                    qs = qs.filter(eBay_count__exact=3, StaffID__in=user_list).exclude(tenbuOperation__exact=2)
                if DepartmentID == '11':
                    qs = qs.filter(eBay_count__exact=3, StaffID__in=user_list).exclude(elevenbuOperation__exact=2)
                if DepartmentID == '12':
                    qs = qs.filter(eBay_count__exact=3, StaffID__in=user_list).exclude(twelvebuOperation__exact=2)
                if DepartmentID == '13':
                    qs = qs.filter(eBay_count__exact=3, StaffID__in=user_list).exclude(thirteenbuOperation__exact=2)
                # qs = qs.filter(eBay_count='3',StaffID__in = user_list,PlatformName__exact='eBay')
            elif pub == '4':
                if DepartmentID == '1':
                    qs = qs.filter(eBay_count__gt=3, StaffID__in=user_list).exclude(onebuOperation__exact=2)
                if DepartmentID == '2':
                    qs = qs.filter(eBay_count__gt=3, StaffID__in=user_list).exclude(twobuOperation__exact=2)
                if DepartmentID == '3':
                    qs = qs.filter(eBay_count__gt=3, StaffID__in=user_list).exclude(threebuOperation__exact=2)
                if DepartmentID == '4':
                    qs = qs.filter(eBay_count__gt=3, StaffID__in=user_list).exclude(fourbuOperation__exact=2)
                if DepartmentID == '5':
                    qs = qs.filter(eBay_count__gt=3, StaffID__in=user_list).exclude(fivebuOperation__exact=2)
                if DepartmentID == '6':
                    qs = qs.filter(eBay_count__gt=3, StaffID__in=user_list).exclude(sixbuOperation__exact=2)
                if DepartmentID == '7':
                    qs = qs.filter(eBay_count__gt=3, StaffID__in=user_list).exclude(sevenbuOperation__exact=2)
                if DepartmentID == '8':
                    qs = qs.filter(eBay_count__gt=3, StaffID__in=user_list).exclude(eightbuOperation__exact=2)
                if DepartmentID == '9':
                    qs = qs.filter(eBay_count__gt=3, StaffID__in=user_list).exclude(ninebuOperation__exact=2)
                if DepartmentID == '10':
                    qs = qs.filter(eBay_count__gt=3, StaffID__in=user_list).exclude(tenbuOperation__exact=2)
                if DepartmentID == '11':
                    qs = qs.filter(eBay_count__gt=3, StaffID__in=user_list).exclude(elevenbuOperation__exact=2)
                if DepartmentID == '12':
                    qs = qs.filter(eBay_count__gt=3, StaffID__in=user_list).exclude(twelvebuOperation__exact=2)
                if DepartmentID == '13':
                    qs = qs.filter(eBay_count__gt=3, StaffID__in=user_list).exclude(thirteenbuOperation__exact=2)
                # qs = qs.filter(eBay_count__gt ='3',StaffID__in = user_list,PlatformName__exact='eBay')
            elif pub == '5':
                if DepartmentID == '1':
            ...
            if pub == '':
                qs = qs.filter()
            if pub == '0':
                qs = qs.filter(eBay_count=0)
            elif pub == '1':
                qs = qs.filter(eBay_count=1)
            elif pub == '2':
                qs = qs.filter(eBay_count=2)
            elif pub == '3':
                qs = qs.filter(eBay_count=3)
            elif pub == '4':
                qs = qs.filter(eBay_count__gt=3)
            elif pub == '5':
