...
class t_product_inventory_warnning_dead_ing_Admin(object):
    search_box_flag = True
    #kc_flag = True
    search_flag = False
    #show_kc = True
    
    actions =  ['modify_sh_to_self','to_return_refund',]
    ...
    def modify_sh_to_self(self, request, queryset):
        for querysetid in queryset.all():
            if querysetid.SHMstatus == 'DSH':
                if request.user.has_perm('skuapp.change_t_product_inventory_warnning_dead_ing'):
                    t_product_inventory_warnning_dead_ing.objects.filter(id=querysetid.id).update(SHMstatus = 'WCSH',SHTime = datetime.datetime.now(),SHName = request.user.first_name)
                else:
                    messages.error(request, '对不起！您没有审核的权限！ ID：%s'%querysetid.id)
    modify_sh_to_self.short_description = u'审核'
    
    def to_return_refund(self, request, queryset):
        for querysetid in queryset.all():
            if querysetid.SHMstatus == 'DSH' :#or querysetid.Mstatus is null:
                if request.user.has_perm('skuapp.change_t_product_inventory_warnning_dead_ing'):
                    t_product_inventory_warnning_dead_ing.objects.filter(id=querysetid.id).update(SHMstatus = 'DCL')
                else:
                    messages.error(request, '对不起！您没有驳回的权限！ ID：%s'%querysetid.id)
    to_return_refund.short_description = u'驳回'
    
    list_display = ('show_Picture','show_MainSKU','show_tortInfo','order7daysAll','order15daysAll','order30daysAll','show_AvailableNumber','UnitPrice','Weight','show_Money','CreateTime','radio','Discount','SQName','SQTime','SHMstatus','SHName','SHTime',)
    #list_editable = ('HandleResults','Remark','Discount')
    #readonly_fields = ('handleTime','Status')
    
    def get_list_queryset(self,):
        request = self.request
        qs = super(t_product_inventory_warnning_dead_ing_Admin, self).get_list_queryset()
        ...
        return qs.filter(SHMstatus = 'DSH')