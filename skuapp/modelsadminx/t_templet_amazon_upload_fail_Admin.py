...
"""  
 ...
 @site: 
 @software: PyCharm
 @file: t_templet_amazon_upload_fail_Admin.py
 @time: 2018-04-11 13:20
"""  
...
class t_templet_amazon_upload_fail_Admin(object):
    ...
    def to_recycle(self, request, queryset):
        ...
        for obj in queryset:
            ...
            obj.is_display = '0'
            obj.save()
    to_recycle.short_description = u'转到草稿箱'

    def get_list_queryset(self,):
        """显示可显示的，自己本人的"""
        request = self.request
        qs = super(t_templet_amazon_upload_fail_Admin, self).get_list_queryset()
        if request.user.is_superuser:
            qs = qs.filter(is_display='1')
        else:
            qs = qs.filter(createUser = request.user.username,is_display='1')
        return qs